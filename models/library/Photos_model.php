...
class Photos_model extends CI_Model
{
    ...
    public function insert_photos($file)
    {
        ...
        $data['img'] = $file['img'];
        $this->db->insert('photos_library', $data);
    ...
    public function insert_library_imgs($all_img, $id)
    {
        foreach ($all_img as $key => $value) {
            if (!empty($value)) {
                ...
                $this->db->insert("photos_library_imgs", $data);
            } else {
                continue;
            }
        }
    ...
    public function count_all($id){
        $this->db->select('*');
        $this->db->from('photos_library_imgs');
    ...
    public function update_library($id,$file){
        ...
        $this->db->where('id', $id);
        $this->db->update('photos_library', $data);
    }

    public function delete($id){
        $this->db->where('id',$id);
        $this->db->delete('photos_library');

    }

    public function delete_img($id){
        $this->db->where('library_id_fk',$id);
        $this->db->delete('photos_library_imgs');
