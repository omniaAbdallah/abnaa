...
class Setting_model extends CI_Model{
    ...
    public function get_code($from_code)
    {
        ...
        $query = $this->db->get('osr_talbat_setting');
        if ($query->num_rows() > 0) {
    ...
    public function insert_setting($type)
    {
        ...
        $this->db->insert('osr_talbat_setting', $data);
    }
    ...
    public function update_setting($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('osr_talbat_setting', $data);
    }
    public function getById($id)
    {
        return $this->db->get_where('osr_talbat_setting', array('id' => $id))->row();
    }
    public function delete_setting($id)
    {
        $this->db->where('id', $id)->delete('osr_talbat_setting');
    }
    public function get_by_id($id)
    {
        $this->db->where('id', $id);
        return $this->db->get('osr_talbat_setting')->row();
    }
