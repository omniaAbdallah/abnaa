...
class Serv_setting_model extends CI_Model{
    ...
    public function add_setting($id=''){
        $data['khdma_name'] = $this->input->post('khdma_name');
         $data['fe2a_id'] = $this->input->post('fe2a_id');
         
         $data['sarf_method'] = $this->input->post('sarf_method');
         $data['need_talab'] = $this->input->post('need_talab');
...
$data['mostwa_khdma']=$this->input->post('mostwa_khdma');
    ...
    public function get_setting($id=''){
        ...
        if ($query->num_rows() > 0) {
            if (!empty($id)){
              //  return $query->row();
                ...
                $data->fe2a_titles = $this->get_specific_data(1,'fe2a_title');
      ...
      public function get_all_data(){
              ...
              $this->db->select('*');
              $this->db->from('osr_serv_khdmat_settings');
        ...
        foreach ($query->result() as $row) {
            $data[$x] = $row;
            $data[$x]->fe2a_title = $this->get_specific_data($row->fe2a_id,'fe2a_title');
        ...
        return $data;
         ...
         public function get_specific_data($fe2a_id,$data_need)
    {
        $this->db->where('id', $fe2a_id);
        $query = $this->db->get('osr_serv_khdmat_fe2a_setting');
        if ($query->num_rows() > 0) {
    ...
    public function select_all_fe2at(){
           $this->db->select('*');
           $this->db->where('fe2a_khdma_id_fk',0);
           $this->db->from('osr_serv_khdmat_fe2a_setting');
           $this->db->order_by('id',"ASC");
           $query = $this->db->get();
           if ($query->num_rows() > 0) {
               foreach ($query->result() as $row) {
