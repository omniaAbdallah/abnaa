...
class Talb_electric_device_model extends CI_Model
{
    ...
    {
        $this->db->select('osr_talbat_electric_device.*,osr_talbat_main.*')
            ->from("osr_talbat_electric_device")
            ->join('osr_talbat_main', 'osr_talbat_main.id=osr_talbat_electric_device.main_id_fk')
            ...
            if (!empty($query)) {
              ...
              //  $query['type_sevice_name'] = $this->get_by('osr_serv_khdmat_fe2a_setting', array('id' => $query['type_sevice']), 'fe2a_title');
               ...
               $query['type_sevice_name'] = $this->get_by('osr_serv_khdmat_settings', array('id' => $query['type_sevice']), 'khdma_name');
                ...
                $query['type_device_name'] = $this->get_by('osr_talbat_setting', array('id' => $query['type_device_id_fk']), 'title');
    ...
    public function select_last_talab_rkm()
    {
        ...
        $this->db->from("osr_talbat_electric_device");
    ...
    public function Delete($rkm)
    {
        ...
        $this->db->delete("osr_talbat_electric_device");
    ...
    public function insert_attach($id, $images)
    {
        if (isset($images) && !empty($images)) {
            ...
            for ($x = 0; $x < $count; $x++) {
                ...
                $this->db->insert('osr_talbat_electric_device_files', $data);
            }
        }
    }
    ...
    {
        $this->db->where('id', $id)->delete('osr_talbat_electric_device_files');
    }

    public function get_morfq_by_id($id)
    {
        $this->db->where('talab_rkm_fk', $id);
        $query = $this->db->get('osr_talbat_electric_device_files');
        return $query->result();
    }
    ...
    public function insert($main_id)
    {
        ...
        $this->db->insert("osr_talbat_electric_device", $data);
    ...
    public function update($id)
    {
        ...
        $data['mokadem_name'] = $this->input->post('mokadem_name');
        $this->db->where("main_id_fk", $id);
        $this->db->update("osr_talbat_electric_device", $data);
...
//new_funccc
    ...
    {
        $query = $this->db->where('suspend', null)->get('osr_talbat_electric_device')->result();
        $data = array();
        $x = 0;
...
// get_osra_in_last_date
    ...
    {
        $query = $this->db->where('file_num', $file_num)->get('osr_talbat_electric_device')->row();
