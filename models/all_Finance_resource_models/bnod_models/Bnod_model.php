...
//2-4-om
    ...
    {
        $sql = $this->db->where('code', $code)->where('type', $type)->get('fr_bnod_pills_setting')->row();
        if (!empty($sql)) {
            return true;
        } else {
            return false;
...
//2-4-om
    ...
    {
        $row = $this->db->where('id', $id)->get('fr_bnod_pills_setting')->row();
        $count_band = $this->count('fe2a', $row->fe2a, 4);
        if ($count_band > 1) {
            $this->db->where('id', $id)->delete('fr_bnod_pills_setting');
        } else {
            $this->db->where('id', $id)->delete('fr_bnod_pills_setting');
            $count_fe2a = $this->count('erad_tabro3', $row->erad_tabro3, 3);
            if ($count_fe2a > 1) {
                $this->db->where('erad_tabro3', $row->erad_tabro3)->where('from_id', $row->fe2a)->where('type', 3)->delete('fr_bnod_pills_setting');
            } else {
                $this->db->where('erad_tabro3', $row->erad_tabro3)->where('from_id', $row->fe2a)->where('type', 3)->delete('fr_bnod_pills_setting');
                $count_erad_tabro3 = $this->count('esal', $row->esal, 2);
                if ($count_erad_tabro3 > 1) {
                    $this->db->where('esal', $row->esal)->where('from_id', $row->erad_tabro3)->where('type', 2)->delete('fr_bnod_pills_setting');
                } else {
                    $this->db->where('esal', $row->esal)->where('from_id', $row->erad_tabro3)->where('type', 2)->delete('fr_bnod_pills_setting');
                    $count_esal = $this->count('from_id', $row->from_id, 1);
                    if ($count_esal > 1) {
                        $this->db->where('from_id', $row->esal)->where('type', 1)->delete('fr_bnod_pills_setting');
                    }else{
                        $this->db->where('from_id', $row->esal)->where('type', 1)->delete('fr_bnod_pills_setting');
...
*/
    ...
    public function change_status($valu, $id)
    {
        ...
        $status = 1 - $valu;
        $data['status'] = $status;
        $this->db->where('id', $id)->update('fr_bnod_pills_setting', $data);
