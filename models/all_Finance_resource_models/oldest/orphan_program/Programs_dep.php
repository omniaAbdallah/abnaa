...
public function get_mother($mother_id){
    ...
    return  $data =$h->row_array()["full_name"];
      //$total_name=$data["full_name"];
      //return $total_name;
}
...
public function get_father($father_id){
     ...
     $h = $this->db->get_where('father', array("mother_national_num_fk"=>$father_id));
         ...
         return  $data =$h->row_array()["full_name"];
     // $total_name=$data["f_first_name"]." ".$data["f_father_name"]." ".$data["f_grandfather_name"]." ".$data["f_family_name"];
     // return $total_name;
}
...
public function get_doner_name($donor_id){
     ...
     $h = $this->db->get_where('sponsors', array("id"=>$donor_id));
      $data =$h->row_array();
      $total_name=$data["name"];
...
public function get_member_name($member_id){
     ...
     $h = $this->db->get_where('f_members', array("id"=>$member_id));
      $data =$h->row_array();
      $total_name=$data["member_full_name"];
      return $total_name;
}
//-------------------------------------------------
...
/*****************/
    public function all_payments_on(){
        ...
        foreach($categories as $p_cat){
            $categories[$i]->orphan_name = $this->get_names("f_members",array("id"=>$p_cat->member_id),"member_full_name");
            $categories[$i]->program_name = $this->get_names("programs_depart",array("id"=>$p_cat->program_id_fk),"program_name");
            $categories[$i]->donar_name = $this->get_names("donors_t",array("id"=>$p_cat->donor_id),"user_name");
   ...
   /*********************************************/
     ...
     public function select_all_defin($table,$grouby,$limit,$order_by,$order_by_desc_asc){
        $this->db->select('*');
        $this->db->from($table);
