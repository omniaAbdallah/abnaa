...
class Esal_order_model extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from("fr_nmazg_pill_orders");
    ...
    public function select_last_pill_num($filed,$table)
    {
        $this->db->select('*');
        $this->db->from($table);
    ...
    public function get_emps($where)
    {
        $sql = $this->db->where($where)->get('employees')->result();
        if (!empty($sql)) {
            return $sql;
        }
    }

    public function edara_data($where)
    {
        $sql = $this->db->where($where)->get('department_jobs')->row();
        if (!empty($sql)) {
            return $sql;
        }
    }
    ...
    public function insert($id = false, $file = false)
    {
        ...
        $var = time() + 5;
        if ($var == true) {
            $last_pill_num = $this->select_last_pill_num('order_num','fr_nmazg_pill_orders');
        }

        $data['order_date'] = $this->chek_Null($this->input->post('pill_date'));
        $data['order_type'] = $this->chek_Null($this->input->post('pill_type'));
        ...
        if (empty($id)) {
            $data['order_num'] = $last_pill_num;
            $data['date'] = date('Y-m-d');
            $data['date_s'] = time();
            $data['date_ar'] = date('Y-m-d');
            $data['publisher'] = $_SESSION['user_id'];
            $data['publisher_name'] = $this->getUserName($_SESSION['user_id']);
            $this->db->insert('fr_nmazg_pill_orders', $data);
        } else {

            /********************تعديل قيد*******************/
            if (!empty($file)) {
                $data['morfaq'] = $file;
                $data['date_morfaq'] = date('Y-m-d');
                $data['date_s_morfaq'] = time();
                $data['date_ar_morfaq'] = date('Y-m-d');
                $data['publisher_morfaq'] = $_SESSION['user_id'];
                $data['publisher_name_morfaq'] = $this->getUserName($_SESSION['user_id']);
            }
            $this->db->where('id', $id);
            $this->db->update('fr_nmazg_pill_orders', $data);


            $data['order_num'] = $this->input->post('pill_num');
        }


    }

    public function insert_esal()
    {
        $data['moshro3_fk'] = $this->chek_Null($this->input->post('moshro3_fk'));
        ...
        $last_pill_num = $this->select_last_pill_num('pill_num','fr_all_pills');
        ...
        $data['esal_from_nmazg'] = 1;
        $data['order_num_fk'] = $this->input->post('pill_num');
        $this->update_esal_order($data['order_num_fk']);
        $this->db->insert('fr_all_pills', $data);

    }

    public function update_esal_order($order_num)
    {
        $data['made_esal'] = 1;
        $this->db->where('order_num', $order_num)->update('fr_nmazg_pill_orders', $data);
    }
    ...
    public function select_all_by_fr_all_pills_all()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$x] = $row;
                $data[$x]->pill_type_title = $this->GetTitleFromFr_bnod_pills_setting($row->order_type);
                ...
                $data[$x]->emp = $this->get_emps(array('emp_code' => $row->emp_code_fk))[0]->employee;
    ...
    public function DeletePill($id)
    {
        ...
        $this->db->delete('fr_nmazg_pill_orders');
        $this->db->where('order_num', $id);
    }
    ...
    {
        $this->db->select("fr_sponsor.k_name,fr_nmazg_pill_orders.person_id_fk,fr_sponsor.id");
        //$this->db->select('k_name');
        $this->db->from('fr_sponsor');
        //$this->db->where('id',$id);
        $this->db->join('fr_nmazg_pill_orders', 'fr_nmazg_pill_orders.person_id_fk=fr_sponsor.id');
        //	$this->db->group_by('fr_nmazg_pill_orders.person_id_fk');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $i = 0;
            foreach ($query->result() as $row) {
    ...
    public function get_all_pill($id)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$i] = $row;
                $data[$i]->pill_type_title = $this->GetTitleFromFr_bnod_pills_setting($row->order_type);
                $i++;
            }
            return $data;
        } else {
