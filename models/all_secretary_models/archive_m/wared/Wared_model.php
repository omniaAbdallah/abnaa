...
class Wared_model extends CI_Model
{
    ...
    {
//tarekt_estlam_name
        if ($this->input->post('wared_type') == 2) {
        ...
        } elseif ($this->input->post('wared_type') == 1) {

            $data['geha_ekhtsas_id_fk'] = $this->input->post('qsm_fk');
            $data['geha_ekhtsas_name'] = $this->input->post('qsm_n');
            $data['geha_morsla_id_fk'] = $this->input->post('edara_id_fk');
            $data['geha_morsla_name'] = $this->input->post('edara_id_fk');
            $data['morsel_name'] = $this->input->post('emp_name');
        }
 ...
 */
 ...
 return $this->db->order_by('id', 'desc')->get('arch_wared')->result();
 }
...
public function get_comments($waed_id_fk)
{
    $this->db->select('arch_wared_twgehat.*,employees.personal_photo');
    $this->db->from('arch_wared_twgehat');
    $this->db->join('employees','employees.id=arch_wared_twgehat.publisher');
    $this->db->where('wared_id_fk', $waed_id_fk);
...
//print_r($data);
   ...
   $this->add_history(5, $data['wared_id_fk']);
    ...
    public function add_relation()
{
    ...
    $this->add_history(4, $data['mo3mla_rkm']);
    return $this->db->insert_id();
...
public function update_seen_wared_comments()
{
    $data['seen'] = 1;
    $data['seen_date_ar'] = date('Y-m-d H:i:s');
    $this->db->where('seen',0)->where('to_user_id', $_SESSION['emp_code'])->update('arch_wared_twgehat', $data);
}
...
//yara4-5-2020
public function get_emp_wared_by_id($id)
{
    $this->db->where('wared_id_fk', $id);
...
{
    $operation = array('ÅÖÇÝå æÇÑÏ','ÊÚÏíá æÇÑÏ','ÍÐÝ æÇÑÏ','ÅÖÇÝå ãÑÝÞÇÊ æÇÑÏ','ÅÖÇÝå ÚáÇÞÇÊ æÇÑÏ','ÅÖÇÝå ÊÍæíáÇÊ æÇÑÏ','ÅÖÇÝå ÊæÌíåÇÊ æÇÑÏ',' ØÈÇÚå ÈÇÑßæÏ æÇÑÏ',' ØÈÇÚå ÊÑæíËå æÇÑÏ',' ØÈÇÚå ÙÑÝ ',' ÇäåÇÁ ãÚÇãáå æÇÑÏ');
...
public function get_count_comments($waed_id_fk)
{
    $this->db->select('COUNT(id) as count')->where('seen', 0)->where('to_user_id', $_SESSION['emp_code'])->where('wared_id_fk', $waed_id_fk)->where('seen', 0);
    return $this->db->get('arch_wared_twgehat')->row_array()['count'];
}
