  ...
  public function get_table_order($table,$arr){
   ...
   /* public function insert_sader($id=''){
    ...
    {   
       ...
       public function insert_sader($id = '')
    {
        if ($this->input->post('sader_type') == 2) {
            $data['geha_ekhtsas_fk'] = $this->input->post('geha_ekhtsas_fk');
            $data['geha_ekhtsas_n'] = $this->input->post('geha_ekhtsas_n');
            $data['geha_morsel_fk'] = $this->input->post('geha_morsel_fk');
            $data['geha_morsel_n'] = $this->input->post('geha_morsel_n');
            $data['mostlem_name'] = $this->input->post('mostlem_name');
        } elseif ($this->input->post('sader_type') == 1) {
            $data['geha_ekhtsas_fk'] = $this->input->post('qsm_fk');
            $data['geha_ekhtsas_n'] = $this->input->post('qsm_n');
            $data['geha_morsel_fk'] = $this->input->post('edara_id_fk');
          //  $data['geha_morsel_n'] = $this->input->post('edara_id_fk');
            $data['geha_morsel_n'] = $this->get_edara_n($this->input->post('edara_id_fk'));
          
            $data['mostlem_name'] = $this->input->post('emp_name');
        }
   ...
   public function get_edara_n($id){
        $this->db->where('id',$id);
   ...
   /* public function insert_comments($id){
    ...
    public function insert_comments($id){
        ...
        if (!empty($row_id)){
          ...
          $person=explode('-',$this->input->post('to_user_name'));
           
            $data['to_user_name'] 	       = $person[0];
            $data['to_user_id'] 	       = $person[1];


  
    $this->db->insert('arch_sader_comments',$data);
    $this->add_history(6, $id);
...
public function get_main_folders($from = '')
{
    ...
    $this->db->order_by('from_id_fk ASC');
...
public function update_seen_sader_comments()
{
    $data['seen'] = 1;
    $data['seen_date_ar'] = date('Y-m-d H:i:s');
    $this->db->where('seen',0)->where('to_user_id', $_SESSION['emp_code'])->update('arch_sader_comments', $data);
}

public function get_comments($sader_id)
{
    $this->db->select('arch_sader_comments.*,employees.personal_photo');
    $this->db->from('arch_sader_comments');
    $this->db->join('employees','employees.id=arch_sader_comments.publisher');
    $this->db->where('sader_id_fk', $sader_id);
    return $this->db->get()->result();
}
...
public function add_print_zarf()
{
    ...
    $insert_id = $this->db->insert_id();
    $this->add_history(9, $data['mo3mla_id_fk']);
    return $insert_id;
}
...
//yara4-5-2020
   ...
   public function get_cartsss()
    {
        $this->db->select('*,employees.administration as edara_id , department_jobs.dep_code as ddep_code');
        $this->db->from('employees');
        $this->db->join('department_jobs','employees.administration = department_jobs.id');
      ...
      public function get_edara_dep_code()
    {
        $this->db->select('*');
         $this->db->select('employees.*,department_jobs.dep_code as eddara ');
      $this->db->from('employees');
        $this->db->join('department_jobs','employees.administration = department_jobs.id','left');
        $this->db->where('employees.id', $_SESSION['emp_code']);
        $h = $this->db->get();
        $data = $h->row_array();
        return $data;
...
public function get_emp_sader_by_id($id)
{
    $this->db->where('sader_id_fk', $id);
    $this->db->group_by('to_user_id');
    $query = $this->db->get('arch_sader_history');
    if ($query->num_rows() > 0) {
        return $query->result();
    } else {
        return false;
    ...
    public function get_comment_byId($id){
        $this->db->where('id',$id);
        $query = $this->db->get('arch_sader_comments');
        if ($query->num_rows()>0){
    ...
    public function add_history($type,$rkm_mo3mla)
    {
        $operation = array('إضافه صادر','تعديل صادر','حذف صادر','إضافه مرفقات صادر','إضافه علاقات صادر','إضافه تحويلات صادر','إضافه توجيهات صادر',' طباعه باركود صادر',' طباعه ترويثه صادر',' طباعه ظرف ',' انهاء معامله صادر');
        $data['type']=$operation[$type];
        $data['rkm_mo3mla']= $rkm_mo3mla;
        $data['type_mo3mla']= 1;
        $data['date']= strtotime(date("Y-m-d"));
        $data['date_ar'] = date('Y-m-d H:i:s');
        if($_SESSION['role_id_fk']==1){
        ...
        $this->db->insert('arch_mo3mlat_history',$data);

    }
     
     
     public function get_count_comments($sader_id_fk)
{
    $this->db->select('COUNT(id) as count')->where('seen', 0)->where('to_user_id', $_SESSION['emp_code'])->where('sader_id_fk', $sader_id_fk)->where('seen', 0);
    return $this->db->get('arch_sader_comments')->row_array()['count'];
}
