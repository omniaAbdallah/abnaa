...
class Email extends CI_Model
{
    ...
    public function make_star($id, $star_value, $type)
    {
        switch ($type) {
            case 1:/*ward*/
                $star_value=$this->get_id('email_inbox','id',$id,'starred');
                $data['starred'] = 1 - $star_value;
                $this->db->where('id', $id)->update('email_inbox', $data);

                break;
            case 2:/*morsal*/
                $star_value=$this->get_id('email_inbox','email_rkm',$id,'star_from');
                $data['star_from'] = 1 - $star_value;
                $this->db->where('email_rkm', $id)->update('email_inbox', $data);
                break;
        }
    }
    ...
    public function make_action($emailes_ids, $action, $type)
    {
        ...
        if (!empty($emailes_ids)) {
            for ($i = 0; $i < count($emailes_ids); $i++) {
                $this->db->where($filed, $emailes_ids[$i])->update('email_inbox', $data);
            }
        }
    }

    public function select_all_star_email()
    {
        $this->db->select('*');
        ...
        $query_result = $this->db->where('starred', 1)->get()->result();
        if ($query_result > 0) {
            $i = 0;
    ...
    public function select_all__email_by($where_arr = false, $page)
    {
        $this->db->select('*');
        $this->db->from('email_inbox');
        $this->db->group_by('email_rkm');
        $this->db->order_by('id', 'desc');
        if ($_SESSION['role_id_fk'] == 1) {
            $x = $_SESSION['user_id'];
        } else {
        ...
        if (!empty($x)) {
            switch ($page) {
                case 4:
                    $this->db->group_start()->where('email_to_id', $x)->where('starred', 1)->group_end();
                    $this->db->or_group_start()->where('email_from_id', $x)->where('star_from', 1)->group_end();
                    break;
                case 3:
                    $this->db->group_start()->where('email_to_id', $x)->where('deleted', 1)->group_end();
                    $this->db->or_group_start()->where('email_from_id', $x)->where('delete_from', 1)->group_end();
                    break;
            }
        ...
        $query_result = $this->db->limit(10)->get()->result();
       /* echo "<pre>";
        ...
        if ($query_result > 0) {
            ...
            foreach ($query_result as $row) {
                $query_result[$i]->employee_photo = $this->get_employee_photo($row->email_to_id);
                $query_result[$i]->employee_photo_from = $this->get_employee_photo($row->email_from_id);
                $i++;
            }
            return $query_result;
        }
    ...
    public function select_all_my_email($where_arr = false)
    {
        ...
        $query_result = $this->db->where('sent', 1)->where('deleted', 0)->where('archived', 0)->limit(10)->get()->result();
    ...
    public function count_all_my_email()
    {
        ...
        return $this->db->where('sent', 1)->where('deleted', 0)->where('archived', 0)->get()->row()->count;
    }
    ...
    /*5-5-om*/
    public function make_deleted($id, $type)
    {
...
//                $query->emp_name = '';
    ...
    ////////////////////////////////////////yara///////////////////////////////////////////
    public function ClientsListCs($email_rkm)
    {
        $this->db->select('email_to_id,email_to_n');
        $this->db->from('email_inbox');
        //$this->db->order_by('emp_code','asc');
        $this->db->where("email_rkm", $email_rkm);
        if ($_SESSION['role_id_fk'] == 3) {
            $this->db->where("email_from_id", $_SESSION['emp_code']);
        } else {
            $this->db->where("email_from_id", $_SESSION['user_id']);
        }
    ...
    public function Clients($email_rkm)
    {
        $this->db->select('email_from_id,email_from_n,email_to_id');
        $this->db->from('email_inbox');
        $this->db->where("email_rkm", $email_rkm);
        if ($_SESSION['role_id_fk'] == 3) {
            $this->db->where("email_to_id", $_SESSION['emp_code']);
        } else {
            $this->db->where("email_to_id", $_SESSION['user_id']);
        }
        // $this->db->where("user_id !=","19");
        $query = $this->db->get();
        $r = $query->result_array();
    ...
    /*    public function PictureUrlById($id)
        {
            ...
            if (!empty($res['role_id_fk']) && ($res['role_id_fk'] == 3)) {
                return base_url('uploads/human_resources/emp_photo/thumbs/' . $res['user_photo']);
    ...
    public function get_employee_photoo($f_id)
    {
        $this->db->select('personal_photo,employee');
        $this->db->from('employees');
        $this->db->where("id", $f_id);
    ...
    public function GetName($user_id)
    {
        $this->db->select('email_to_id,email_to_n');
        $this->db->from('email_inbox');
        $this->db->where("email_to_id", $user_id);
        $this->db->limit(1);
        $query = $this->db->get();
        $res = $query->row_array();
        return $res['email_to_n'];
    }

    public function get_emps($email_rkm)
    {
        $ids = array();
        $q = $this->db->select('email_to_id,email_from_id')->where('email_rkm', $email_rkm)->get('email_inbox')->result();
        if (!empty($q)) {
            foreach ($q as $row) {
                array_push($ids, $row->email_to_id);
                array_push($ids, $row->email_from_id);
            }
            return $this->get_all_emp($ids);
        } else {
            return false;
        }
    }
    ...
    /*    public function add_emps_email()
        {
            ...
            $query = $this->db->from('email_inbox')->where('email_rkm', $email_rkm)->group_by('email_rkm')->get()->row_array();
            if (!empty($emp_ids)) {
                foreach ($emp_ids as $emp) {
                    unset($query['email_to_id']);
                    unset($query['email_to_n']);
                    unset($query['id']);
                    $query['email_to_id'] = $emp;
                    $query['email_to_n'] = $this->get_id("employees", 'id', $emp, "employee");
                    $this->db->insert('email_inbox', $query);
                }
            }
        }*/
    ...
    public function add_emps_email()
    {
        ...
        if (!empty($emp_ids)) {
            foreach ($emp_ids as $emp) {
                unset($query['email_to_id']);
                unset($query['email_to_n']);
                unset($query['email_from_id']);
                unset($query['email_from_n']);
                unset($query['id']);
                $query['email_to_id'] = $emp;
                $query['email_to_n'] = $this->get_id("employees", 'id', $emp, "employee");

                $query['email_from_id'] = $this->get_id("employees", 'id', $_SESSION['emp_code'], "id");
                $query['email_from_n'] = $this->get_id("employees", 'id', $_SESSION['emp_code'], "employee");
    ...
    public function get_email_by_id($id)
    {
        ...
        $query_result = $this->db->where('id', $id)->get()->row();

        $query_result->employee_photo = $this->get_employee_photo($query_result->email_from_id);
    ...
    public function PictureUrlById($id)
    {
        $this->db->select('user_id,emp_photo,role_id_fk,emp_code');
        $this->db->from('users');
        ...
        return 'https://abnaa-sa.org/uploads/human_resources/emp_photo/thumbs/' . $res['emp_photo'];
