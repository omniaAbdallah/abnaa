...
class Email extends CI_Model
{
    ...
    public function make_star($id,$star_value)
    {
        $data['starred'] = 1-$star_value;
        $this->db->where('id', $id)->update('email_inbox', $data);
    }

    public function make_action($emailes_ids, $action)
    {
        switch ($action) {
            case 1:
                $data['readed'] = 1;
        ...
        if (!empty($emailes_ids)){
            for ($i=0;$i < count($emailes_ids);$i++){
                $this->db->where('id', $emailes_ids[$i])->update('email_inbox', $data);
            }
        }
    }
    ...
    public function select_all__email_by($where_arr = false)
    {
        $this->db->select('*');
        $this->db->from('email_inbox');
        $this->db->order_by('id', 'desc');
