...
class Gawda_plans_model extends CI_Model {
    ...
    public function add_hadaf()
    {
        ...
        $data['ftra_qyas']	 	  = $this->input->post('ftra_qyas');
        $data['parent_code']  = 0;
   ...
   public function insert()
	...
	public function update($id)
	{
        ...
        ($this->input->post('ftra_qyas'))?$data['ftra_qyas']=$this->input->post('ftra_qyas'):'';
...
//        ($this->input->post('markz_tklfa')) ? $data['markz_tklfa'] = $this->input->post('markz_tklfa') : '';
    ...
    public function last_hdf_rkm($plan_rkm){
        $this->db->select('*');
        $this->db->from("gwd_make_plans");
        $this->db->order_by("id","DESC");
        $this->db->where('plan_rkm', $plan_rkm);
    ...
    //yara7-4-2020
    public function check_code($hdf_code,$level,$plan_rkm)
    {
        $this->db->select('code');
        $this->db->from("gwd_make_plans");
        $this->db->where("code", $hdf_code);
