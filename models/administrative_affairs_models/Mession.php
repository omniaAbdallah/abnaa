...
class Mession extends CI_Model
{
    ...
    {
        $this->db->select('mession.*, employees.id AS e_id, employees.employee, employees.department, prog_main_sitting.title');
        $this->db->from('mession');
        $this->db->join('employees','employees.id = mession.emp_id');
        $this->db->join('prog_main_sitting','prog_main_sitting.id = mession.type_id_fk');
     ...
     /*   if($_SESSION['role_id_fk'] == 3 && $_SESSION['head_dep_id_fk'] == 2   )
        ...
        if($_SESSION['role_id_fk'] == 3 && $_SESSION['head_dep_id_fk'] == 1   )
           ...
           //    $this->db->where('mession.emp_id',115);
