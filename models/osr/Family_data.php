...
//new
    public function get_sarf_months($mother_num)
    {
        $this->db->select('SUM(finance_sarf_order_details.value) AS sum_value , MONTH(finance_sarf_order.sarf_date_ar) AS m_sarf_date')
            ->from('finance_sarf_order_details')
            ->join('finance_sarf_order', 'finance_sarf_order.sarf_num = finance_sarf_order_details.sarf_num_fk')
            ->where('mother_national_num_fk', $mother_num)
            ->group_by("m_sarf_date")
            ->order_by('m_sarf_date', 'ASC');
        ...
        if ($query->num_rows() > 0) {
            ...
            $backgroundColor=[];
            foreach ($query->result() as $row) {
                $values[$i]=$row->sum_value;
                $labels[$i]= $months[$row->m_sarf_date];
                $backgroundColor[$i]="rgb(135, 177, 201)";

                $i++;
            }
            $data = array('data'=>json_encode($values),'labels'=>json_encode($labels),'backgroundColor'=>json_encode($backgroundColor));
            return $data;
        } else {
            return false;
        }
...
//new
    public function get_sarf_bands($mother_num)
    {

        $this->db->select('SUM(finance_sarf_order_details.value) AS sum_value , finance_sarf_order.bnod_help_fk AS bnod_help_fk')
            ->from('finance_sarf_order_details')
            ->join('finance_sarf_order', 'finance_sarf_order.sarf_num = finance_sarf_order_details.sarf_num_fk')
            ->where('mother_national_num_fk', $mother_num)
            ->group_by("bnod_help_fk")
            ->order_by('bnod_help_fk', 'ASC');
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $values[$i]=$row->sum_value;
                $labels[$i]=$this->get_by('bnod_help', array('id' => $row->bnod_help_fk),'title');
                $backgroundColor[$i]="rgb(250, 172, 181)";
