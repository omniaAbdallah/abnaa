...
//---------------------------------------------
      ...
      public function check_user_data(){
          ...
          $q = $this->db->select('id,user_id,user_name,file_num ,mother_national_num,final_suspend ,hidden_action,account_img ')->where('user_name', $user)->where('user_password', $pass)->where('account_status', 1)->get('basic')->row_array();
          if (!empty($q)) {
              return $q;
          } else {
              return 0;
