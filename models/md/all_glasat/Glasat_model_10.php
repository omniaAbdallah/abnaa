...
class Glasat_model extends CI_Model
{
    ...
    public function get_all_details_attend_new($rkm, $attend)
    {
        ...
        if ($query->num_rows() > 0) {
            $query = $query->result();
            foreach ($query as $key => $row) {
                $query[$key]->member_magles_data = $this->get_by('md_all_magls_edara_members', array('mem_id_fk' => $row->mem_id_fk));
                $query[$key]->member_data = $this->get_by_mem('md_all_gam3ia_omomia_members', array('id' => $row->mem_id_fk));
                $query[$key]->odwiat_data = $this->get_by_mem('md_all_gam3ia_omomia_odwiat', array('member_id_fk' => $row->mem_id_fk));
            }
            return $query;
...
//        }
    ...
    function get_member_hdoor($rkm)
    {
        ...
        $this->db->select('md_all_glasat_hdoor.*, md_all_gam3ia_omomia_odwiat.rkm_odwia, md_all_gam3ia_omomia_odwiat.subs_to_date_m, md_all_gam3ia_omomia_odwiat.rkm_odwia_full');
        $this->db->from('md_all_glasat_hdoor');
        $this->db->join('md_all_gam3ia_omomia_odwiat', 'md_all_gam3ia_omomia_odwiat.member_id_fk = md_all_glasat_hdoor.mem_id_fk', "LEFT");
