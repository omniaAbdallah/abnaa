...
class Osr_crm_zyraat_m extends CI_Model
{
    ...
    public function select_all($mother_num = null)
    {
...
// date('h:i A', strtotime($DateTime)),
        ...
        $this->add_history_osr(603, $this->input->post('mother_national_num'));
    }
    /*  public function insert(){
           $data['mother_national_num'] = $this->input->post('mother_national_num');
           $data['rkm_visit'] = $this->select_last_talab_rkm();
    ...
    {
        $mother_national_num = $this->db->where('id', $id)->get('osr_crm_zyraat')->row()->mother_national_num;
        $this->add_history_osr(605, $mother_national_num);
    ...
    public function update($id)
    {
        ...
        $this->add_history_osr(604, $this->input->post('mother_national_num'));
    }
    ...
    public function select_family_table()
    {
        ...
        $this->db->join('father', 'father.mother_national_num_fk = basic.mother_national_num', "left");
        $this->db->join('mother', 'mother.mother_national_num_fk = basic.mother_national_num', "left");
        $this->db->where('basic.final_suspend', 0);
        $this->db->where('basic.deleted', 0);
        $this->db->order_by("basic.id", "Asc");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
    ...
    public function select_family_table_file()
    {
    ...
    public function get_count_zeyarat($visit_ended = null)
    {
        $this->db->select('osr_crm_zyraat.*');
        $this->db->from('osr_crm_zyraat');
        if ($visit_ended != null) {
            $this->db->where('visit_ended', $visit_ended);
        }
        $this->db->order_by('id', 'desc');
        $query = $this->db->get();
        return $query;
...
// start_time_zyraa
    public function start_time_zyraa($id)
    {
        $data['visit_start_time'] = date('H:i a');
        $data['visit_start'] = 'yes';
        $this->add_history('start', $id);
        $mother_national_num = $this->db->where('id', $id)->get('osr_crm_zyraat')->row()->mother_national_num;
        $this->add_history_osr(607, $mother_national_num);
        $this->db->where('id', $id)->update('osr_crm_zyraat', $data);
    }
    public function end_zyraa($id)
    {
        $data['tawsyat_bahth'] = $this->input->post('tawsyat_bahth');
        $data['visit_ended'] = 'yes';
        $data['visit_ended_emp_name'] = $_SESSION['emp_name'];
        $data['visit_ended_date'] = date('Y-m-d');
        $data['visit_ended_time'] = date('H:i a');
        $this->db->where('id', $id)->update('osr_crm_zyraat', $data);
        $this->add_history('end', $id);
        $mother_national_num = $this->db->where('id', $id)->get('osr_crm_zyraat')->row()->mother_national_num;
        $this->add_history_osr(608, $mother_national_num);
//add_to _tabel_bahth
        $dataa['zyraa_id_fk'] = $id;
        $dataa['is_mother_present'] = $this->input->post('is_mother_present');
        $dataa['mother_impression_visit'] = $this->input->post('mother_impression_visit');
        $dataa['home_cleaning'] = $this->input->post('home_cleaning');
        $dataa['child_cleanliness'] = $this->input->post('child_cleanliness');
        $dataa['family_type'] = $this->input->post('family_type');
        $dataa['tawsyat_bahth'] = $this->input->post('tawsyat_bahth');
        $dataa['family_news'] = $this->input->post('family_news');
        $dataa['rai_baheth'] = $this->input->post('rai_baheth');
        if ($dataa['tawsyat_bahth'] == 'chnage_to_f2a') {
            $dataa['new_f2a'] = $this->input->post('new_f2a');
        } else if ($dataa['tawsyat_bahth'] == 'fired_afrard') {
            $dataa['fired_afrard'] = serialize($this->input->post('fired_afrard'));
        }
        $this->db->insert('osr_crm_zyraat_bahth', $dataa);
    }
...
//==========================================
    ...
    public function getByArray_zyraa_data($id)
    {
        $this->db->select('*');
        $this->db->from('osr_crm_zyraat_bahth');
        $this->db->where(array("zyraa_id_fk" => $id));
        $query = $this->db->get();
        return $query->row_array();
    }
    public function complete_zyraa_data($id)
    {
        $dataa['is_mother_present'] = $this->input->post('is_mother_present');
        $dataa['mother_impression_visit'] = $this->input->post('mother_impression_visit');
        $dataa['home_cleaning'] = $this->input->post('home_cleaning');
        ...
        if ($dataa['tawsyat_bahth'] == 'chnage_to_f2a') {
            $dataa['new_f2a'] = $this->input->post('new_f2a');
            $dataa['fired_afrard'] = null;
        } else if ($dataa['tawsyat_bahth'] == 'fired_afrard') {
            $dataa['new_f2a'] = null;
            $dataa['fired_afrard'] = serialize($this->input->post('fired_afrard'));
        }
        $this->db->where('zyraa_id_fk', $id)->update('osr_crm_zyraat_bahth', $dataa);
        $mother_national_num = $this->db->where('id', $id)->get('osr_crm_zyraat')->row()->mother_national_num;
        $this->add_history_osr(609, $mother_national_num);
    }
    /*******************************************************/
    ...
    function close_crm_zyraat($id)
    {
        ...
        $mother_national_num = $this->db->where('id', $id)->get('osr_crm_zyraat')->row()->mother_national_num;
        $this->add_history_osr(611, $mother_national_num);
    }
    ///zyraaa_yara
////yara16-2-2021
    public function insert_money($id, $num)
    {
        if (($this->input->post('income_max')) != '') {
            for ($a = 0; $a < $this->input->post('income_max'); $a++) {
                $data['mother_national_num_fk'] = $this->chek_Null($num);
                $data['zyraa_id_fk'] = $this->chek_Null($id);
                ...
                $data['type'] = 1;
                $this->db->insert('osr_crm_bahth_maly', $data);
            }
        }
        ...
        $this->add_history_osr(610, $num);
    }
// update_money
    public function update_money($id, $num)
    {
        $this->db->where('mother_national_num_fk', $num);
        $this->db->where('zyraa_id_fk', $id);
        $this->db->delete("osr_crm_bahth_maly");
    ...
    public function getArray($id, $num)
    {
        $this->db->select('*');
        $this->db->from('osr_crm_bahth_maly');
        $this->db->where("mother_national_num_fk", $num);
        $this->db->where('zyraa_id_fk', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
