...
public function get_letters_by_type($type)
    ...
    public function insert_letter($letter_type)
    {
        ...
        $this->add_history($letter_type,$this->uri->segment(4));
...
if(!empty($img_file)) {
    ...
    public function get_one_w_member($member_id){
     ...
     //add_history
function add_history($type,$mother_id)
{
    if($type==1)
    {
        $data['action_code'] = 306;
 $action_name= $this->db->where('code',$data['action_code'])->get('osr_action_process')->row()->process_name;
  
    $data['action_name'] = $action_name;
    }
    else if($type==6)
    {
        $data['action_code'] = 307;
 $action_name= $this->db->where('code',$data['action_code'])->get('osr_action_process')->row()->process_name;
