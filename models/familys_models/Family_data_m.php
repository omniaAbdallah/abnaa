...
class Family_data_m extends CI_Model
{
    ...
    public function family_data($mother_national_num)
    {
        ...
        $data['basic'] = $this->basic_data($mother_national_num);
        ...
        $data['history_files'] = $this->get_by('osr_all_history', array('mother_national_num' => $mother_national_num));
    ...
    public function get_devices($mother_national_num_fk)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$a] = $row;
                $data[$a]->d_house_device_status_name = $this->get_by('family_setting', array('id_setting' => $row->d_house_device_status_id_fk), 'title_setting');
                $a++;
            }
            return $data;
        }
    ...
    public function wakels_data($mother_national_num)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if (isset($data->relationship_id_fk) && $data->relationship_id_fk != 0 && $data->relationship_id_fk != '') {
                $data->relationship_id_fk_n = $this->get_from_family_settings($data->relationship_id_fk);
            } else {
                $data->relationship_id_fk_n = 'غير محدد';
            }
            if (isset($data->w_marital_status_id_fk) && $data->w_marital_status_id_fk != 0 && $data->w_marital_status_id_fk != '') {
                $data->w_marital_status_n = $this->get_from_family_settings($data->w_marital_status_id_fk);
            } else {
                $data->w_marital_status_n = 'غير محدد';
            }
            //
    ...
    {
        $current_year=current_hjri_year();
        $this->db->select('*');
        $this->db->from('f_members');
        $this->db->where('mother_national_num_fk', $mother_national_num);
        $query = $this->db->get();
    ...
    public function house_data($mother_national_num)
    {
        ...
        if (!empty($query)) {
            ...
            if ($query->h_house_owner_id_fk == 'rent') {
                $query->h_house_owner_tilte = 'ايجار';
            } else {
