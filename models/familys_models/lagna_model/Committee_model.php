...
/******************************************************************************************/
  ...
  /*public function get_session_procedure($session_num)
    ...
    public function get_session_procedure($session_num){
        ...
        if($query->num_rows()>0)
        {
            ...
            foreach ($query->result() as $row) {
                $data[$x] = $row;
                $data[$x]->decision = $this->get_decision($row->procedure_id_fk, $row->session_num_fk);
                // $data[$x]->members_details = $this->get_members($row->file_num, $row->session_num_fk);
                $x++;
    ...
    public function get_mahader_lagna_data($session_num){
        $this->db->select('all_mahader_lagna_arch_ended.session_num_fk,all_mahader_lagna_arch_ended.mehwar_id_fk,
        all_mahader_lagna_arch_ended.mehwar_title,all_mahader_lagna_arch_ended.mother_national_num,
        all_mahader_lagna_arch_ended.file_num,

        ');
        $this->db->from("all_mahader_lagna_arch_ended");
       //   $this->db->join('basic', 'basic.mother_national_num = all_mahader_lagna_arch_ended.mother_national_num',"left");
        $this->db->where('all_mahader_lagna_arch_ended.session_num_fk',$session_num);
    // $this->db->where('all_mahader_lagna_arch_ended.file_num',940);
        $this->db->order_by('all_mahader_lagna_arch_ended.mehwar_id_fk','asc');
        $this->db->group_by('all_mahader_lagna_arch_ended.mehwar_id_fk');
        $query=$this->db->get();
        if($query->num_rows()>0)
        {
            $data = array();
            ...
            foreach ($query->result() as $row) {
                ...
                $data[$x]->mahder_details =
                    $this->get_mahder_details(array('session_num_fk'=>$session_num,'mehwar_id_fk'=>$row->mehwar_id_fk));
                $x++;
            }
            return $data;
        }else{
    ...
    public function get_mahder_details($arr){
           ...
           $this->db->select('all_mahader_lagna_arch_ended.*,
basic.family_cat as fe2a_id , basic.family_cat_name as fe2a_name 
        ');
         $this->db->from("all_mahader_lagna_arch_ended");
        $this->db->join('basic', 'basic.mother_national_num = all_mahader_lagna_arch_ended.mother_national_num',"left");
        $this->db->where($arr);
    ...
    public  function get_decision($procedure,$session){
        ...
        if($query->num_rows()>0)
        {
            ...
            foreach($query->result() as $row)
            {
                ...
                //$data[$x]->all_members_details=$this->get_all_members_details($data[$x]->file_num);
