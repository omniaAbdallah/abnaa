...
class Model_lagna_setting extends CI_Model{
    ...
    public function get_session_num_transformation_lagna($session_number){
        $this->db->select('*');
        $this->db->from("transformation_lagna");
        $this->db->where(array("session_num_fk"=>$session_number));
  ...
  public function getByid_lagna(){
    ...
    public function get_last_session(){
       ...
       public function get_last_session_2(){
        $this->db->order_by('id','desc');
        $query=$this->db->get('selected_lagna_members');
        if($query->num_rows()>0){
            return  $query->row()->glsa_rkm;
        }else{
            return 0;
        }
    }
   ...
   /* public function get_member_session($num){
    ...
    public function get_member_session($num)
{
    $this->db->select('selected_lagna_members.*,lagna_members.member_job')->from('selected_lagna_members')
        ->join('lagna_members', 'selected_lagna_members.member_id=lagna_members.member_id')->where('selected_lagna_members.session_number', $num);
    ...
    public function insert_selected_lagna(){
        foreach ($this->input->post('member_id') as $key=>$value) {
            ...
            $data['suspend']=0;
              ...
              $data['glsa_rkm']=$this->input->post('session_number_2');
            $data['glsa_rkm_full']=$this->input->post('year').'/'.$this->input->post('session_number_2');
            $this->db->insert('selected_lagna_members',$data);
        }
    }
    ...
    public function update_approved_lagna(){
       ...
       // $ttt =$this->get_title_by_id('procedures_option_lagna',array('id'=>1),'title');
       /* echo "<pre>";
        print_r($ttt);
        echo "</pre>";
        die;*/
        ...
        // if($process ==5){
                ...
                if(!empty($_POST['ids'])){
                    ...
                    for ($a=0;$a<sizeof($_POST['ids']);$a++){
                        ...
                        if(!empty($procedure_arr[0])){
                            ...
                            $data2['reason_lagna_id_fk'] =$procedure_arr[0];
                        }
                          if(!empty($procedure_arr[1])){
                              $data2['reason_lagna_title'] =$procedure_arr[1];
                          }
                        ...
                        $this->db->where('mother_national_num',$_POST['mother_national_num']);
                        $this->db->where('procedure_id_fk',$_POST['procedure_id_fk']);
                        $this->db->update('transformation_lagna_members',$data2);
                        //===================================
                       // $data_lagna['finished']=0;
                        $data_lagna['approved_lagna']=1;
                        $this->db->where('session_num_fk',$_POST['session_num_fk']);
                        $this->db->where('procedure_id_fk',$_POST['procedure_id_fk']);
                        $this->db->where('mother_national_num',$_POST['mother_national_num']);
                        $this->db->update('transformation_lagna',$data_lagna);
            ...
            if(!empty($_POST['reason'])){
                ...
                $data2['reason_lagna_id_fk'] =$_POST['reason'];



               $data2['reason_lagna_title'] = $this->get_title_by_id('family_reasons_settings',array('id'=>$_POST['reason']),'title');

            }

            if(!empty($_POST['condition'])){
                $data2['halet_file_id_fk'] =$_POST['condition'];

                $data2['halet_file_title'] =$this->get_title_by_id('files_status_setting',array('id'=>$_POST['condition']),'title');
            }




             //procedures_option_lagna  reason
             //family_reasons_settings  reason
            //files_status_setting  condition
    ...
    public function check_opening_session($arr){
        $this->db->select('*');
        $this->db->from("transformation_lagna");
        $this->db->where($arr);
        $query= $this->db->get();
...
/***********************************************************************/
    ...
    public function get_last_session_closed(){
        $this->db->order_by('id','desc');
        $query=$this->db->get('selected_lagna_members');
        if($query->num_rows()>0){
            return  $query->row();
    ...
    public function get_all_files_status_title($id)
    {
        $this->db->where('id',$id);
    ...
    public function get_father_members($arr){
        $this->db->select('*');
        $this->db->where($arr);
        $query=$this->db->get('all_mahader_lagna_arch_ended');
        if($query->num_rows()>0)
        {
    ...
    public function get_mahader_lagna_data($session_num){
        $this->db->select('session_num_fk,mehwar_id_fk,mehwar_title');
        ...
        if($query->num_rows()>0)
        {
            ...
            foreach ($query->result() as $row) {
                $data[$x] = $row;
                $data[$x]->mahder_details =
                    $this->get_mahder_details(array(
                        'action_in_table'=>0,
                        'session_num_fk'=>$session_num,'mehwar_id_fk'=>$row->mehwar_id_fk));
                $x++;
    ...
    public function get_mahder_details($arr){
        $this->db->where($arr);
        $this->db->group_by('mother_national_num');
        ...
        if($query->num_rows()>0)
        {
            ...
            foreach ($query->result() as $row) {
                ...
                $data[$x]->members =$this->get_father_members(
                    array('file_num'=>$row->file_num ,
                        'mehwar_id_fk'=>$row->mehwar_id_fk,
                        'mother_national_num'=>$row->mother_national_num,
                        'session_num_fk'=>$row->session_num_fk)
                );
                $x++;
    ...
    public function get_mahder_details_by_id($id)
    {
        $this->db->select('mother_national_num,lagna_reason_title,id');
        $this->db->where('id',$id);
        $query=$this->db->get('all_mahader_lagna_arch_ended');
        if($query->num_rows()>0){
            return $query->row();
    ...
    public function change_file_setting($table)
    {
        $updated_id =$this->input->post('mahder_updated_id');
        if(!empty($this->input->post('halet_file'))){
            foreach ($this->input->post('halet_file') as $key =>$value){

                if($table ==='basic'){
                    $data['file_status'] = $value;
                    $data['process_title'] = $this->get_all_files_status_title($value);
                    $this->db->where('mother_national_num', $key);

                }elseif ($table ==='f_members'){

                    $data['persons_status'] = $value;
                    $data['persons_process_title'] = $this->get_all_files_status_title($value);
                ...
                $this->db->where('id',$updated_id[$key]);
                $this->db->update('all_mahader_lagna_arch_ended', array('action_in_table'=>1));

                /*******************************/
                $data_2['mother_national_num_fk'] = $this->get_mahder_details_by_id($updated_id[$key])->mother_national_num;
                $data_2['process_id'] = $value;
                $data_2['title'] =  $this->get_all_files_status_title($value);
                $data_2['reason'] = $this->get_mahder_details_by_id($updated_id[$key])->lagna_reason_title;
...
{
    return $this->db->where('member_id', $_SESSION['emp_code'])->order_by('id','DESC')->get('selected_lagna_members')->result_array();
}
function all_current_invation()
{
    return $this->db->where(array('member_id'=>$_SESSION['emp_code'], 'replay_invitation'=> 0))->get('selected_lagna_members')->result_array();
}
function update_seen_glasat_invation()
{
    $this->db->where(array('replay_invitation_seen' => 'no', 'member_id' => $_SESSION['emp_code'], 'replay_invitation' => 0))
        ->update('selected_lagna_members', array('replay_invitation_seen' => 'yes'));
}
function do_replay($row_id, $replay,$note)
{
    $this->db->where(array('id' => $row_id))
        ->update('selected_lagna_members', array('replay_invitation' => $replay,'replay_invitation_time'=>date('h:i a'),
            'replay_invitation_date_s'=>date('Y-m-d'),'notes_invitation'=>$note));
}


public function get_last_session_open()
{
    $this->db->order_by('id', 'desc');
    $query = $this->db->where('finished', 0)->where('suspend', 1)->get('selected_lagna_members');
    if ($query->num_rows() > 0) {
        return $query->row_array();
    } else {
        return 0;
...
public function GetTransformationLagnaDesicion()
{
    $this->db->select('transformation_lagna.* ,lagna.lagna_name as lagna_title_name,father.full_name,father.f_national_id');
    $this->db->from('transformation_lagna');
    ...
    $this->db->join('father', 'father.mother_national_num_fk = transformation_lagna.mother_national_num', "left");
    $this->db->where("transformation_lagna.finished", 0);
    $this->db->where('transformation_lagna.approved', 0);
    $this->db->where('transformation_lagna.process', 5);
    $this->db->where("transformation_lagna.type", 1);
...
public function update_deport_subject_ward()
{
    ...
    if (!empty($ids) && (is_array($ids))) {
        ...
        $data['session_num_fk'] = $this->input->post('session_num_id');
        $this->db->where_in('id', $ids);
        $this->db->update('transformation_lagna', $data);
        $this->insert_history($data['approved'],$ids);
        return 1;
    } else {
        return 0;
    }
...
public function insert_attach($id,$images)
{
    if(isset($images)&& !empty($images))
    {
        ...
        for($x=0; $x<$count;$x++)
        {
            ...
            $this->db->insert('lagna_files',$data);
        }
    }
}
public function delete_morfq($id)
{
    $this->db->where('id',$id)->delete('lagna_files');
}
public function get_morfq_by_id($id){
    $this->db->where('glsa_rkm_fk',$id);
    $query = $this->db->get('lagna_files');
    return $query->result();
}
