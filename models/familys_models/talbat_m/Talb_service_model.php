...
class Talb_service_model extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from("osr_talbat_service");
    ...
    {
        $this->db->select('*');
        $this->db->from("family_serv_khdmat_settings");
        $this->db->order_by("id", "DESC");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result();
    ...
    public function insert()
    {
     ...
     $data['wasf'] =$this->input->post('wasf');
        ...
        $this->db->insert("osr_talbat_service", $data);
    ...
    public function update($id)
    {
        ...
        $data['mokadem_name'] = $this->input->post('mokadem_name');
        $this->db->where("id", $id);
        $this->db->update("osr_talbat_service", $data);
    ...
    public function Delete($rkm)
    {
        ...
        $this->db->delete("osr_talbat_service");
...
public function insert_attach($id,$images)
{
    if(isset($images)&& !empty($images))
    {
        ...
        for($x=0; $x<$count;$x++)
        {
            ...
            $this->db->insert('osr_talbat_service_files',$data);
        }
    }
}
public function delete_morfq($id)
{
$this->db->where('id',$id)->delete('osr_talbat_service_files');
}
public function get_morfq_by_id($id){
    $this->db->where('talab_rkm_fk',$id);
    $query = $this->db->get('osr_talbat_service_files');
        return $query->result();
}
...
public function get_id($table,$where,$id,$select){
    ...
    // public function get_all_talb_history($talb_id_fk=null){

	// 	$this->db->select('osr_talbat_service_history.*');

	// 	$this->db->from('osr_talbat_service_history');
    ...
    //        $this->db->where('talb_id_fk', $talb_id_fk);   
...
/*        $parmas['talab_id_fk'] = $post['talab_id'];
        ...
        $this->db->insert('osr_talbat_service_history', $parmas);*/
        ...
        $this->db->update('osr_talbat_service', $update);
    }
    ...
    {
        $this->db->select('osr_talbat_service.*');
        $this->db->from('osr_talbat_service');
    ...
    {
        $this->db->where('current_to_user_id', $_SESSION['user_id']);
        $this->db->update('osr_talbat_service', array('seen' => 1));
    }
