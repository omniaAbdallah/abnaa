...
/************************************************/
    ...
    public function open_sesssion_id($lagna_id_fk){
     ...
     public function open_sesssion_id_new($lagna_id_fk){
        $this->db->select('*');
        $this->db->from("selected_lagna_members");
        $this->db->where("lagna_id_fk",$lagna_id_fk);
       // $this->db->where("suspend",1);
...
public function all_procedures($type){
    ...
    $last_galsa =$this->get_last_galsa();
    ...
    if ($query->num_rows() > 0) {
        ...
        foreach ($query->result() as $row) {
            $data[$a] = $row;
            if($type ==1){
                $data[$a]->transformation_lagna = $this->GetTransformationLagna(array('transformation_lagna.procedure_id_fk'=>$row->id,'transformation_lagna.session_num_fk'=>$last_galsa->session_number,'transformation_lagna.approved'=>$type),0);
            }else{
                $data[$a]->transformation_lagna = $this->GetTransformationLagna(array('transformation_lagna.procedure_id_fk'=>$row->id'transformation_lagna.approved'=>$type),0);
    ...
    public function GetTransformationLagna($arr,$conditions){
      ...
      //  $this->db->where("finished",0);
        ...
        foreach($query as $row)
        {
            ...
            $data[$x]->father=$this->get_data_father($row->mother_national_num);
            $data[$x]->file_num=$this->GetFileNum($row->mother_national_num);
            $data[$x]->open_sesssion_id=$this->open_sesssion_id_new($row->add_to_lagna_id_fk);
            $x++;
        }
        return $data;
    }
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                ...
                if($row->person_type ==2){
                    ...
                    $data[$a]->halet_file_id_after = $this->get_details_by_id("mother",array('mother_national_num_fk'=>$row->mother_national_num))->halt_elmostafid_m;
                    $data[$a]->halet_file_title_after =
                        $this->get_title_by_id('files_status_setting',array('id'=>$data[$a]->halet_file_id_after),'title');
                }
            $a++;}
            return $data;
        }
...
//20-2-2019
    ...
    public function finish_session($session_num){
        ...
        $tabe3t_tahwel_arr=array(1=>'ÇáÃÓÑ',2=>'ÇáÃÝÑÇÏ');
        $types = array(0 => "ÚÖæ ãÊØæÚ", 1 => "ÚÖæ ãÌáÓ ÇáÇÏÇÑå",
            2 => "ÚÖæ ÇáÌãÚíå ÇáÚãæãíå", 3 => "ÃÚÖÇÁ ÇáãæÙÝíä");
...
//
       ...
       $person_type_arr =array('1'=>'ÇáÃã','2'=>'ÇáÃÝÑÇÏ');
...
/*********************************************************/
    ...
    public function GetTransformationLagnaDesicion_test($arr,$conditions){
        $this->db->select('transformation_lagna.* ,lagna.lagna_name as lagna_title_name');
        $this->db->from('transformation_lagna');
        $this->db->join('lagna', 'lagna.id_lagna = transformation_lagna.add_to_lagna_id_fk',"left");
        $this->db->where("type",2);
   ...
   /* 30-6 
	 public function all_proceduresDesicion2($type,$session_id){
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$a] = $row;
               $data[$a]->transformation_lagna = $this->GetTransformationLagnaDesicion(
                array('transformation_lagna.session_num'=>$session_id,
                'transformation_lagna.procedure_id_fk'=>$row->id ,
                'transformation_lagna.approved_lagna'=>0,
                'transformation_lagna.approved'=>$type),0);
    ...
    */
     ...
     public function all_proceduresDesicion2($type,$session_id){
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$a] = $row;
                $data[$a]->transformation_lagna = $this->GetTransformationLagnaDesicion(array('transformation_lagna.session_num_fk'=>$session_id,'transformation_lagna.procedure_id_fk'=>$row->id ,'transformation_lagna.approved_lagna'=>0,'transformation_lagna.approved'=>$type),0);
                $a++;}
            return $data;
        }else{
            return 0;
    ...
    public function all_procedures_bnod($conditions){
        $this->db->select('id,title');
        $this->db->from("procedures_option_lagna");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$a] = $row;
                $data[$a]->transformation_lagna_band = $this->GetTransformationLagnaDesicion(array('transformation_lagna.procedure_id_fk'=>$row->id,'transformation_lagna.surf_num !='=>0),$conditions);
                $a++;}
            return $data;
        }else{
            return 0;
    ...
    public function get_transformation_lagna($arr)
    {
        $this->db->where($arr);
        $query=$this->db->get('transformation_lagna');
        if($query->num_rows()>0){
