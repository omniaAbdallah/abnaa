	...
	//  yaraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
	    public function total_rows($q = NULL)
        {
            $this->db->like('id', $q);
				...
				$this->db->where('receiver_id', $_SESSION['mother_national_num']);
			
            $this->db->where('seen', 0);
            $this->db->from('family_chat');
        ...
        public function update_seen_chat($id)
        {
			...
			$this->db->where('sender_id',$id)->where('receiver_id',$_SESSION['mother_national_num'])->update('family_chat', $data);
