   ...
   /* public function insert_attach($images)
    {
        if(isset($images)&& !empty($images))
        {
            ...
            for($x=0; $x<$count;$x++)
            {
                $data['t_rkm_id_fk']=$this->input->post('row_id');
                $data['title']=$this->input->post('title')[$x];
                $data['file']=$images[$x];
                $this->db->insert('finance_sadad_fatora_attach',$data);
            }
        }
       ...
       public function insert_attach($images)
    {
        if(isset($images)&& !empty($images))
        {
            ...
            for($x=0; $x<$count;$x++)
            {
                $gg=$this->input->post('row_id');
                $data['title']=$this->input->post('title')[$x];
                $data['file']=$images[$x];

                $this->db->where('id',$gg)->update('finance_sadad_fatora_details',$data);
            }
        }
    ...
    /*public function get_images($id)
    {
        ...
        $query=$this->db->get('finance_sadad_fatora_attach');
        if($query->num_rows()>0)
        {
            return $query->result();
        }else{
    ...
    {
        $this->db->where('id',$id);
        $this->db->delete('finance_sadad_fatora_attach');
    }*/
