...
class Taswia_model extends  CI_Model{
    ...
    public function get_sheek_sarf($bank_id,$rkm_hesab,$from_date,$to_date){

        $total = 0;
        $this->db->select('finance_sanad_sarf_sheek.*,finance_sanad_sarf.*');
        $this->db->from("finance_sanad_sarf_sheek");
        $this->db->join('finance_sanad_sarf','finance_sanad_sarf.rqm_sanad = finance_sanad_sarf_sheek.rqm_sanad_id_fk');
        $this->db->where("pay_method",3);
        $this->db->where("sheek_status",0);
        $this->db->where("taslem_sheek",1);
        $this->db->where("finance_sanad_sarf.sheek_bank_id",$bank_id);
        $this->db->where("finance_sanad_sarf.sheek_rqm_hesab",$rkm_hesab);
        $this->db->where("finance_sanad_sarf.date_sanad >=",$from_date);
        $this->db->where("finance_sanad_sarf.date_sanad <=",$to_date);
        $query = $this->db->get();
        if ($query->num_rows()>0){
           // return $query->result();
            $i =0 ;
