...
/************************************************************/
    ...
    $data['direct_manager_name']  = $this->get_emp($this->input->post('direct_manager_id'),'employee');
    	...
    	public function get_all_ozonat_gari(){
		...
		if ($query->num_rows() > 0) {
			...
			return false;
           	...
           	public function get_all_ozonat_canceled($suspend=null,$canceled=null){
		$this->db->select('finance_ezn_sarf.*');
		$this->db->from('finance_ezn_sarf');
        $this->db->order_by('id','desc');
        /*if($_SESSION['role_id_fk']==1){
    	...
    	public function get_all_ozonat($suspend=null){
		$this->db->select('finance_ezn_sarf.*');
		$this->db->from('finance_ezn_sarf');
        $this->db->order_by('id','desc');
        if($_SESSION['role_id_fk']==1){
		...
		if ($query->num_rows() > 0) {
        	...
        	public function get_all_ozonat_transformed(){
		$this->db->select('finance_ezn_sarf.*');
		$this->db->from('finance_ezn_sarf');
        
       $this->db->where('current_to_id', $_SESSION['user_id']);
        $this->db->order_by('id','desc');
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
				$i=0;
        	...
        	public function get_all_ozonat_history($ezn_id_fk=null){
		$this->db->select('finance_ezn_sarf_history.*');
		$this->db->from('finance_ezn_sarf_history');
        $this->db->order_by('id','ASC');

        if($ezn_id_fk !=null){
           $this->db->where('ezn_id_fk', $ezn_id_fk);   
        }
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
				$i=0;
    ...
    {
       $current_from_name = $post['current_from_name'];
        $parmas['ezn_id_fk'] =$post['ezn_id'];
      
      $parmas['ezn_rkm_fk'] =$post['ezn_rkm'];
  ...
  /*   if($post['option_choosed'] == 'accept'){
     ...
     $update['action_moder_mobasher'] = $post['option_choosed']; */
      ...
      $this->db->where('id', $post['ezn_id']);
     $this->db->update('finance_ezn_sarf', $update);
     $message = "لديك إشعار خاص بإذن الصرف مقدم من $current_from_name ";
  ...
  /*   if($post['option_choosed'] == 'accept'){
    ...
    public function add_action_mohaseb($post)
    {
      $parmas['ezn_id_fk'] =$post['ezn_id'];
      ...
      $parmas['talab_in_title'] = 'المحاسب';
      $parmas['date'] =strtotime(date('Y-m-d'));
      $parmas['date_ar'] =date('Y-m-d');
       $parmas['ttime'] = date('h:i A');
       
       if($post['option_choosed'] == 'accept'){
        $parmas['action_title']='الرصيد يسمح';
       }elseif($post['option_choosed'] == 'refuse'){
         $parmas['action_title']='الرصيد لا يسمح';
       }
        $parmas['process_title'] = 'التوصية'; 
      $this->db->insert('finance_ezn_sarf_history',$parmas);
     ...
     if($post['option_choosed'] == 'accept'){
      ...
      $update['seen'] = 0; 
     $update['action_mohaseb'] = $post['option_choosed']; 
     $update['action_mohaseb_date'] = date('Y-m-d'); 
      $update['current_to_id'] =$this->get_emp_user_id($post['to_person_id'],'user_id');;
      $update['current_to_name'] =$this->get_emp($post['to_person_id'],'employee');  
      $this->db->where('id', $post['ezn_id']);
     $this->db->update('finance_ezn_sarf', $update);   
      ...
      public function add_action_modermali($post)
    {
             $parmas['ezn_id_fk'] =$post['ezn_id'];
      ...
      $parmas['to_user'] =$this->get_emp_user_id($post['to_person_id'],'user_id');;
      $parmas['to_user_n'] =$this->get_emp($post['to_person_id'],'employee');
      $parmas['talab_in_title'] = 'المدير المالي ';
      $parmas['date'] =strtotime(date('Y-m-d'));
      $parmas['date_ar'] =date('Y-m-d');
       $parmas['ttime'] = date('h:i A');
         if($post['option_choosed'] == 'accept'){
     ...
     if($post['option_choosed'] == 'accept'){
      ...
      $update['sarf_method'] = $post['sarf_method']; 
      $update['seen'] = 0; 
     $update['action_moder_mali'] = $post['option_choosed'];
     
      $update['action_moder_mali_date'] = date('Y-m-d');  
      $update['current_to_id'] =$this->get_emp_user_id($post['to_person_id'],'user_id');;
      $update['current_to_name'] =$this->get_emp($post['to_person_id'],'employee');  
      $this->db->where('id', $post['ezn_id']);
     $this->db->update('finance_ezn_sarf', $update);   
        ...
        public function add_action_moderaam($post)
    {
      ...
      $parmas['to_user'] =$this->get_emp_user_id($post['to_person_id'],'user_id');;
      $parmas['to_user_n'] =$this->get_emp($post['to_person_id'],'employee');
      $parmas['talab_in_title'] = 'المدير العام ';
      $parmas['date'] =strtotime(date('Y-m-d'));
      $parmas['date_ar'] =date('Y-m-d');
       $parmas['ttime'] = date('h:i A');
            if($post['option_choosed'] == 'accept'){
        $parmas['action_title']='يعتمد';
         ...
         $parmas['process_title'] = 'تسليم المعاملة للمالية'; 
      $this->db->insert('finance_ezn_sarf_history',$parmas);
     ...
     if($post['option_choosed'] == 'accept'){
       ...
       $this->load->model('Model_family_cashing');
        $update['manager_name'] = $this->Model_family_cashing->get_magles_edara(1);
        //$update['naeb_name'] = $this->Model_family_cashing->get_magles_edara(2);
        $update['amin_name'] = $this->Model_family_cashing->get_magles_edara(3);
    ...
    /*    public function add_action_moderaam($post)
    {
      ...
      $parmas['date_ar'] =date('Y-m-d');
       $parmas['ttime'] = date('h:i A');
        ...
        $parmas['action_title']='يعتمد';
         ...
         $parmas['process_title'] = 'إختيار المفوضين'; 
      $this->db->insert('finance_ezn_sarf_history',$parmas);
     ...
     $this->db->update('finance_ezn_sarf', $update);   
                  ...
                  public function add_action_to_mohaseb($post)
    {
      ...
      $parmas['date_ar'] =date('Y-m-d');
       $parmas['ttime'] = date('h:i A');
            if($post['option_choosed'] == 'accept'){
        $parmas['action_title']='تم تسليم للمالية ';
       }elseif($post['option_choosed'] == 'refuse'){
         $parmas['action_title']='لم يتم التسليم للمالية';
       }
   ...
   public function add_action_to_sandok($post)
    {
      ...
      $parmas['date_ar'] =date('Y-m-d');
       $parmas['ttime'] = date('h:i A');
            if($post['option_choosed'] == 'accept'){
        $parmas['action_title']='تم إستلام المعاملة ';
       }elseif($post['option_choosed'] == 'refuse'){
         $parmas['action_title']='لم يتم إستلام المعاملة';
       }
       
         $parmas['process_title'] = 'إستلام المعاملة للمالية'; 
      $this->db->insert('finance_ezn_sarf_history',$parmas);
   ...
   //  $update['action_moder_3am'] = $post['option_choosed']; 
    ...
    function update_seen_ezn_sarf(){
    $this->db->where('current_to_id', $_SESSION['user_id']);
    $this->db->update('finance_ezn_sarf', array('seen' => 1));
         } 
     ...
     public function add_mofawden()

    {

      //  $data['emp_tawfed'] = $this->input->post('toggle');



       $emp_id =  $this->get_emp_for_ezn($this->input->post('process_rkm') ,'emp_id');
            $data['current_to_id'] =$this->get_emp_user_id($emp_id,'user_id');;
      $data['current_to_name'] =$this->get_emp($emp_id,'employee');  
       $data['seen'] =0; 
        $data['level'] = 6; 
      ...
      $parmas['from_user'] = $this->get_emp_for_ezn($this->input->post('process_rkm') ,'current_to_id');
      $parmas['from_user_n'] = $this->get_emp_for_ezn($this->input->post('process_rkm') ,'current_to_name');

     $parmas['to_user'] =$this->get_emp_user_id($emp_id,'user_id');;
     $parmas['to_user_n'] =$this->get_emp($emp_id,'employee'); 




        $this->db->where('id', $this->input->post('process_rkm'));
  ...
  //    $parmas['option_choosed'] =$post['option_choosed'];
    //  $parmas['reason_action'] =$post['notes'];
         $parmas['ezn_id_fk'] =$this->input->post('process_rkm'); 
         $parmas['ezn_rkm_fk'] =$this->get_emp_for_ezn($this->input->post('process_rkm') ,'ezn_rkm');
 ...
 $parmas['reason_action'] = 'تم اختيار المفوضين';
 $parmas['process_title'] = 'لطباعة الإذن وتسليم المعاملة';
...
public function get_emp_for_ezn($ezn_id,$data_need){

    $this->db->select('emp_id,emp_name,current_to_id,current_to_name,ezn_rkm');
    $this->db->where('id', $ezn_id);
    $query=$this->db->get('finance_ezn_sarf');
    if($query->num_rows()>0)
...
{
    $query = $this->db->where('ezn_sarf_id_fk', $id)->get('finance_ezn_sarf_attaches');
    if ($query->num_rows() > 0) {
        return $query->result();
    }
}
...
public function insert_attach($all_img)
{
    if (!empty($all_img)) {
        ...
        $files['ezn_sarf_id_fk'] = $this->input->post('row');
        ...
        $this->db->insert('finance_ezn_sarf_attaches', $files);
    }
}

public function delete_upload($id)
{
    $img = $query = $this->db->where('ezn_sarf_id_fk', $id)->get('finance_ezn_sarf_attaches')->row();
    if (file_exists("uploads/human_resources/ta3mem/" . $img->file)) {
        unlink(FCPATH . "uploads/human_resources/ta3mem/" . $img->file);
    }
}

public function delete_attach_ezn_sarf($id)
{
    $this->delete_upload($id);
    $this->db->where('id', $id);
    $this->db->delete('finance_ezn_sarf_attaches');
}

/*
public function get_ezn_details($id){
    $this->db->select('finance_ezn_sarf.*');
                           
    $this->db->from("finance_ezn_sarf");
    $this->db->where('finance_ezn_sarf.id',$id);    
    $query = $this->db->get();
    if ($query->num_rows() > 0) {
...
*/
    public function get_ezn_details($id = null)
    {
        $this->db->select('finance_ezn_sarf.*');
        $this->db->from('finance_ezn_sarf');
    ...
    public function edite_ezn_sarf($id)
{
    $data['ezn_date'] = $this->chek_Null(strtotime($this->input->post('ezn_date')));
    $data['ezn_date_ar'] = $this->chek_Null($this->input->post('ezn_date'));
    $data['edara_fk'] = $this->chek_Null($this->input->post('edara_fk'));
...
function update_ezn_sarf($insert_id, $id)
{

    $sand_data = $this->get_by('finance_sanad_sarf', array('id' => $insert_id), 1);
    if (!empty($sand_data)) {

        $qed_num = $sand_data->rqm_quod;
        $rqm_sanad = $sand_data->rqm_sanad;


        $this->db->where('id', $id);
        $this->db->update('finance_ezn_sarf', array('rqm_sanad' => $rqm_sanad, 'qed_num' => $qed_num));
 ...
 function cancel_ezn(){
    ...
    $ezn_id = $this->input->post('var_ezn_id');
    $cancel = 'yes';
    $cancel_reason = $this->input->post('cancel_reason');
$person_cancel = $this->getUserName($_SESSION['user_id']);
 ...
 */ 
    $this->db->where('id', $ezn_id)->update('finance_ezn_sarf', array('canceled' => 'yes' ,'suspend' => 2 ,
              ...
              public function send_mail($message){ //Email_send/new_send
 ...
 //   $this->load->helper(array('form'));
  ...
  //  $user_email = "mosad.elsayed@gmail.com ";
   $subject = 'السلام عليكم ';
            ...
            $this->email->from('it.abna463@gmail.com', 'الجمعية الخيرية لرعاية الأيتام ببريدة');
          /*  $data = array(
