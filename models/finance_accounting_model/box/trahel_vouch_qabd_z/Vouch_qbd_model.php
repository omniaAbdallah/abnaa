...
class Vouch_qbd_model extends CI_Model {
	...
	public function insert_update($id)
	{
		...
		$data_qued['qued_date'] = $this->input->post('date_sanad');
		$data_qued['totlal_value'] = $this->input->post('total_value');
    ...
    public function insert_update_datails($id)
	{
		if($this->input->post('rqm_hesab')){
			...
			for ($i=0 ;$i < $count; $i++){
				...
				$data_qued['maden'] = "حساب الصندوق";
				$data_qued['dayen'] = $this->input->post('name_hesab')[$i];
				$data_qued['value'] = $this->input->post('value')[$i];
