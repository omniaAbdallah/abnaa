...
class Sheek_tracks_model extends CI_Model
{
    ...
    public function select_sheeks_sarf()
    {
        $this->db->select('*');
    ...
    public function select_sheeks_qabd()
    {
        $this->db->select('*');
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $data[$x] = $row;
                $data[$x]->details = $this->getdetailsByRqm_sanad("finance_sanad_qabd", $row->rqm_sanad_id_fk);
    ...
    public function change_taslem_sheek()
    {
        ...
        if (!empty($action_id_fk)) {
            $update['taslem_sheek'] = $action_id_fk[0];
            $update['taslem_sheek_name'] = $action_id_fk[1];
        }
    ...
    public function change_sheek_status()
    {
        ...
        if (!empty($action_id_fk)) {
            $update['sheek_status'] = $action_id_fk[0];
            $update['sheek_status_name'] = $action_id_fk[1];
        }
        $this->db->where('id', $this->input->post('id'));
        $this->db->update('finance_sanad_sarf_sheek', $update);
    ...
    /***************************************************/
    public function getSand_Sarf($rkm)
    {
    ...
    public function getSand_Qabd($rkm)
    {
        $this->db->where("rqm_sanad",$rkm);
        ...
        if ($sql->num_rows() > 0) {
            ...
            foreach ($sql->result() as $row){
                $data[$i] = $row;
                $data[$i]->delails = $this->getdetailsByIdQabd($row->id);
                $i++;}
            return $data;
        }
        return false;
