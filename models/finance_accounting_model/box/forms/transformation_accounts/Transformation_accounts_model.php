...
class Transformation_accounts_model extends CI_Model
{
    ...
    public function chek_Null($post_value)
    {
        ...
        public function select_last_process()
    {
        $this->db->select('*');
        ...
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result()[0]->process_rkm + 1;
        } else {
            return 1;
        }
    }
    ...
    public function select_all($arr = false)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                ...
                $data[$x]->sanad_rkm = $this->Getsanad_rkm($row->rqm_sanad_id)['rqm_sanad'];
                $data[$x]->real_rkm_qued = $this->Getsanad_rkm($row->rqm_sanad_id)['rqm_quod'];
                $x++;
            }
            return $data;
        }
      ...
      public function Getsanad_rkm($rqm_sanad_id)
    {
        $h = $this->db->get_where("finance_sanad_sarf", array('id' => $rqm_sanad_id));
        if ($h->num_rows() > 0) {
            return $h->row_array();
        } else {
            return 0;
      ...
      public function Get_deport_account_transform_status($process_rkm)
    {
        $h = $this->db->get_where("finance_account_transformations", array('process_rkm' => $process_rkm));
        if ($h->num_rows() > 0) {
            return $h->row_array()['deport'];
        } else {
            return 0;
        }
    }
    ...
    public function get_result($arr)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $query->row()->start_laqb_name = $this->Get_laqb_name($query->row()->start_laqb,8);
            $query->row()->end_laqb_name = $this->Get_laqb_name($query->row()->end_laqb,9);
      ...
      public function Get_laqb_name($id_setting,$type)
    {
        $this->db->select('*');
    ...
    public function insert_attach($rkm, $all_img)
    {
      ...
      public function get_sum_result_details($arr)
    {
   ...
   /* public function get_geha_by_id($id)
    ...
    {
        $this->db->where('id', $id);
        $query = $this->db->get('finance_account_transformations_gehat');
        if ($query->num_rows() > 0) {
            return $query->row();
        }
        return false;
     ...
     public function update_geah($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('finance_account_transformations_gehat', $data);
    }
    public function delete_geha($id)
    {
        $this->db->where('id', $id);
        $this->db->delete('finance_account_transformations_gehat');
    }
  /*  public function update_geah($id)
    {
        $data['name'] = $this->input->post('geha_title');
  ...
  /*  public function get_transformation_data($arr)
    ...
    ,all_defined_setting.defined_id,all_defined_setting.defined_title as job_title,users.emp_code as users_emp_code,  users.user_id ');
     ...
     public function get_transformation_data($arr)
    {
        $this->db->select('finance_account_transformations.*, employees.id as  employees_id ,employees.mosma_wazefy_n, employees.emp_code as emp_codes,employees.employee,employees.degree_id,employees.personal_photo
    ,all_defined_setting.defined_id,all_defined_setting.defined_title as job_title,users.emp_code as users_emp_code,  users.user_id ');
        $this->db->from('finance_account_transformations');
        $this->db->where($arr);
        $this->db->join('employees', 'employees.emp_code=finance_account_transformations.emp_code', 'left');
    ...
    {
        $this->db->select('hr_egraat_emp_setting.*,employees.id as emp_id,employees.emp_code as emp_codes, employees.personal_photo,employees.mosma_wazefy_n,employees.employee,employees.degree_id,
    ...
    {
        $this->db->select('users.*, employees.emp_code as employees_code ,employees.id as emp_id,employees.employee,
        employees.personal_photo,employees.mosma_wazefy_n,employees.degree_id,
    all_defined_setting.defined_id,all_defined_setting.defined_title as job_title');
        $this->db->from('users');
        $this->db->where($arr);
   ...
   /* public function get_user_data($arr)
    ...
    all_defined_setting.defined_id,all_defined_setting.defined_title as job_title,employees.mosma_wazefy_n');
        $this->db->from('users');
        $this->db->where($arr);
        $this->db->join('employees', 'employees.id=users.emp_code', 'left');
        $this->db->join('all_defined_setting', 'all_defined_setting.defined_id = employees.degree_id', 'left');
  ...
  */
   ...
   /* public function select_geha()
       ...
       public function insert_geha()
    {
        ...
        $data['address'] = $this->input->post('address');
        $data['date'] = date('Y-m-d');
        $this->db->insert('finance_account_transformations_gehat', $data);
    }
    public function select_geha()
    {
        $this->db->select('*');
        $this->db->from('finance_account_transformations_gehat');
    ...
    function save_attachment($images, $process_rkm_fk)
    {
        if (!empty($images)) {
            $data['process_rkm_fk'] = $process_rkm_fk;
            ...
            $data['publisher'] = ($_SESSION['user_id']);
            $data['publisher_name'] = $this->getUserName($_SESSION['user_id']);
            $this->db->insert('finance_transform_account_attechment', $data);
        }
    }
    function get_attachment($where = false)
    {
        ...
        return $this->db->get('finance_transform_account_attechment')->result();
    }
    function delete_morfaq($row_id, $process_rkm_fk, $folder_path)
    {
        $this->delete_upload($row_id, $process_rkm_fk,$folder_path);
        $this->db->where(array('process_rkm_fk' => $process_rkm_fk, "id" => $row_id))->delete('finance_transform_account_attechment');
    }
    public function delete_upload($row_id, $process_rkm_fk,$folder_path)
    {
        $img = $this->get_by('finance_transform_account_attechment', array('process_rkm_fk' => $process_rkm_fk, "id" => $row_id), 1);
    ...
    public function update_amin_manager()
    {
