...
// for report detalies
   public function get_all_data_hdoor()
    ...
    {
       $q=$this->db->where(array('emp_national_num'=>$emp_card,'date'=>$currentDate,'type'=>$type))
         ->get('hr_hdoor_ensraf')->row();  
         return $q;
    }*/
...
public function check_dwam_emp($hdoor_date,$day_hdoor,$emp_id)
{
    ...
    $q=$this->db->select('end_enter,end_out')->where('from_date <=',$hdoor_date)->where('to_date >=',$hdoor_date)
        ->where('emp_id',$emp_id)->where($days_en[$day_hdoor],1)->get('emp_always_times');
    if ($q->num_rows() > 0) {
        return $q->row();
    }
