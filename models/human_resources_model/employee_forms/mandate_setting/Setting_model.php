...
class Setting_model extends CI_Model{
    ...
    public function get_code($from_code)
    {
        ...
        $query = $this->db->get('hr_mandate_setting');
        if ($query->num_rows() > 0) {
    ...
    public function update_setting($id)
    {
        ...
        $data['color'] = $this->input->post('color');
        $this->db->where('id', $id);
        $this->db->update('hr_mandate_setting', $data);
    }

    public function getById($id)
    {
        return $this->db->get_where('hr_mandate_setting', array('id' => $id))->row();
    }

    public function delete_setting($id)
    {
        $this->db->where('id', $id)->delete('hr_mandate_setting');
    }

    public function get_by_id($id)
    {
        $this->db->where('id', $id);
        return $this->db->get('hr_mandate_setting')->row();
    }
