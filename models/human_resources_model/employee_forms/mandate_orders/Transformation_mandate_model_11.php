...
class Transformation_mandate_model extends CI_Model
{
    ...
    {
        $q = $this->get_by_id('hr_mandate_orders', $id);
...
//            $data[$x]->emp_name = $this->get_emp_name($query->row()->emp_user_id);
...
//            $data[$x]->emp_job = $this->Get_job($query->row()->emp_user_id);
            return $query->row();
        } else {
            return false;
        }
...
//                $data['current_to_id'] = $q->emp_user_id;
        ...
        if ($this->input->post('level') == 2 && $data['suspend'] == 1) {
            $data['suspend'] = 4;
            $data['level'] = 0;
    ...
    public function insert_transformation()
    {
        ...
        //print_r($data);
        $data['seen'] = 0;
        $this->db->insert('hr_mandate_orders_history', $data);
    }
    ...
    public function get_moder_final_data($emp_id)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $data[$x]->job_name = $this->get_job_name($data[$x]->degree_id);
            return $data[$x];
