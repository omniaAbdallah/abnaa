...
class Temporary_employment_qrars_model extends CI_Model
{
    ...
    //yara
    public function all_after_interview($id){
        $this->db->select('*');
       ...
       $this->db->where('id',$id);
        ...
        if (!empty($q)) {
                ...
                $q->job_title = $this->get_job($q->job_request_id_fk);
                $q->nationality=$this->get_details($q->nationality_id_fk);
                $q->social_status_name=$this->get_details($q->social_status);
