...
class Overtime_hours_orders_model extends CI_Model
{
    ...
    public function my_over_time_emp()
    {
    ...
    }   public function my_over_time_manger()
    {
         if ((isset($_SESSION['user_id']))&&!empty($_SESSION['user_id'])){
            $this->db->where('current_from_user_id',$_SESSION['user_id']);
...
public function estlam_task($id,$value=false)

{
    ...
    $data['suspend']=1;
    $data['level']=1;
    $data['current_to_user_id']='';
    $data['current_to_user_name']='';
    }
    $this->db->where('id',$id)->update('hr_overtime_hours_orders',$data);
    return $data['suspend'];
}

     public function get_emp_uesr_data($arr)
    {
    ...
    public function insert()
    {
       ...
       // $data['emp_id_fk'] = $this->chek_Null($this->input->post('emp_id_fk'));
        ...
        $data['emp_name'] = $this->get_id("employees", 'emp_code',$data['emp_code_fk'], "employee");
      ...
      // $data['edara_id_fk'] = $this->Public_employee_main_data->get_edara_id_by_emp_id($data['emp_id_fk']);
        ...
        $emp_user_data_from=$this->get_emp_uesr_data(array('employees.id'=>$data['direct_manager_id_fk']));
        if (!empty($emp_user_data_from)) {
            $data['current_from_user_name']=$emp_user_data_from->employee;
            $data['current_from_user_id']=$emp_user_data_from->user_id;  
        } 
        $emp_user_data_to=$this->get_emp_uesr_data(array('employees.id'=>$data['emp_id_fk']));
        if (!empty($emp_user_data_to)) {
            $data['current_to_user_name']=$emp_user_data_to->employee;
            $data['current_to_user_id']=$emp_user_data_to->user_id;  
        }
    ...
    public  function  insert_overtime_details($id){
        ...
        //     for ($s = 0; $s < count($this->input->post('from_hour')); $s++) {
                ...
                $data2['order_num_fk'] =  $id;
                $data2['from_hour'] = $this->chek_Null($this->input->post('from_hour'));
                $data2['to_hour'] = $this->chek_Null($this->input->post('to_hour'));
                $data2['num_hours'] = $this->chek_Null($this->input->post('num_hours'));
                $data2['bdal_type_fk'] = $this->chek_Null($this->input->post('bdal_type'));
                $data2['work_assigned'] = $this->chek_Null($this->input->post('work_assigned'));
                if ($data2['bdal_type_fk'] ==1){
                    $data2['dbal_type_name'] = "بدل نقدي";
                }
                if ($data2['bdal_type_fk'] ==2){
                    $data2['dbal_type_name'] = "بدل أيام الراحة" ;
                }
                //  $data['date'] = strtotime(date('Y-m-d'));
                $data2['date_ar'] = $this->chek_Null($this->input->post('date'));
                $data2['date'] = strtotime($this->chek_Null($this->input->post('date')));
                if ($_SESSION['role_id_fk'] == 1) {
    ...
    public  function  update_overtime_details($id){
        ...
        //     for ($s = 0; $s < count($this->input->post('from_hour')); $s++) {
                ...
                $this->db->where('id',$id)->update('hr_overtime_hours_details', $data2);
        //     }
    ...
    public function update($id)
    {
       ...
       // $this->delete_all_items($id);
        ...
        $data['work_assigned_details'] = $this->chek_Null($this->input->post('work_assigned_details'));
        $data['total_hours'] = $this->chek_Null($this->input->post('total_hours'));
         $emp_user_data_from=$this->get_emp_uesr_data(array('employees.id'=>$data['emp_id_fk']));
        if (!empty($emp_user_data_from)) {
            $data['current_from_user_name']=$emp_user_data_from->employee;
            $data['current_from_user_id']=$emp_user_data_from->user_id;  
        } 
        $emp_user_data_to=$this->get_emp_uesr_data(array('employees.id'=>$data['direct_manager_id_fk']));
        if (!empty($emp_user_data_to)) {
            $data['current_to_user_name']=$emp_user_data_to->employee;
            $data['current_to_user_id']=$emp_user_data_to->user_id;  
        }
...
public function get_all_edite($id){
    if (!empty($id)){
        $this->db->where('id',$id);
    }
    $query = $this->db->get('hr_overtime_hours_orders');
...
public function get_all_details($id){
    ...
    $query = $this->db->get('hr_overtime_hours_details')->row();
    if (!empty($query)){
