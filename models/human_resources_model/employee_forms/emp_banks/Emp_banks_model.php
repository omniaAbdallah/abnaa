...
class Emp_banks_model extends CI_Model
{
	...
	public function change_bank_account_emp($img)
	{
		...
		$data['current_bank_image'] = $this->get_past_bank_image($this->input->post('emp_id'));
		$data['order_time']=date('H:i:s a');
		$data['publisher'] = $_SESSION['emp_code'];
        $data['publisher_name'] = $this->getUserName($_SESSION['user_id']);
        //yara
        $data['current_from_user_name'] = $this->input->post('emp_n');
        $data['current_from_user_id'] = $this->get_user_id($data['emp_id']);
        ///yara13-10-2020
        ...
        if (!empty($emp_to_data)){
            ...
            $data['level'] = 1;
            $data['level_title'] = 'مدير الشوؤن المالية والادارية';
        }
...
public function get_user_id($emp_code)
	...
	{
		$query= $this->db->where('emp_code', $id)->get('bank_employes_details')->row()->bank_id_fk_image;
            if(!empty($query))
            {
                return $query;
            }
...
$this->db->where('id',$id)->update('hr_emp_hesabat_banks_orders',$data);
    ...
    public function update_main_tabel($new_bank_id_fk,$new_bank_account_num,$new_bank_image,$bank_id_fk,$bank_account_num,$bank_id_fk_image,$empCode)
    {
        ...
        $data['bank_code'] = $this->get_bank_code($new_bank_id_fk);
    ...
    public function get_bank_code($bank_id)
    {
        $this->db->where('id', $bank_id);
        $query = $this->db->get('banks_settings');
        if ($query->num_rows() > 0) {
            return $query->row()->bank_code;
        } else {
            return "";
        }
    }
    ...
    function update_seen_hesabat_banks_orders()
{
    ...
    $this->db->update('hr_emp_hesabat_banks_orders', array('seen' => 1));
}
}
