...
class Transformation_ta3gel_model extends CI_Model
{
    ...
    {
        $q = $this->db->where('level', $level)->where('tbl', 'ta3gel')->get('hr_all_transformation_setting')->row();
        if (!empty($q)) {
            return $q;
        }
    }
    ...
    {
        $this->db->select('*');
        $this->db->from("hr_all_solaf_ta3gel_history");
        $this->db->where($arr);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $query->row();
    ...
    public function select_allhistory_by_ta3gel_rkm_fk($ta3gel_rkm_fk)
    {
        $this->db->select('*');
        $this->db->from("hr_all_solaf_ta3gel_history");
        $this->db->where("ta3gel_rkm_fk", $ta3gel_rkm_fk);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $c = 0;
   ...
   /* public function save_from_transfomation()
    {
        ...
        $insert['ta3gel_rkm_fk'] = $this->input->post('ta3gel_rkm_fk');
        $insert['ta3gel_id_fk'] = $this->input->post('ta3gel_id_fk');
        ...
        switch ($level) {
            ...
            case 3:
                ...
                $update['suspend'] = $update['action_moder_hr'];
                ...
                $action = $update['action_moder_hr'];
                if ($action == 1) {
                   // $this->make_quset($insert['ta3gel_id_fk']);
                }
                break;
            default:
                break;
        ...
        // die;
        $update['seen'] = 0;
        $this->db->insert("hr_all_solaf_ta3gel_history", $insert);
        $this->db->where('t_rkm', $this->input->post('ta3gel_rkm_fk'));
        $this->db->update('hr_solaf_ta3gel', $update);
    }
*/
...
public function save_from_transfomation()
{
    ...
    switch ($level) {
        case 2:
            ...
            break;
        case 3:
            $update['action_mohaseb'] = $this->input->post('action_mohaseb');
            if ($update['action_mohaseb'] == 1) {
                $action = 1;
            } elseif ($update['action_mohaseb'] == 2) {
                $level = 0;
                $action = 2;
            }
            break;
        case 4:
            ...
            $update['level'] = 0;
            $update['action_moder_fr'] = $this->input->post('action_moder_fr');
            $update['suspend'] = $update['action_moder_fr'];
            $to_user = $this->get_user_data2(array('employees.id' => $this->input->post('emp_id_fk')));
            if (!empty($to_user)) {
                $insert['to_user'] = $to_user->user_id;
                $insert['to_user_n'] = $to_user->employee;
    ...
    function make_quset($talab_id)
    {
        $talab_data = $this->db->where('id',$talab_id)->get('hr_solaf_ta3gel')->row_array();
        $solfa_rkm = $talab_data['solfa_rkm'];
        $for_month = $talab_data['for_month'];
        $fe2a_ta3gel = $talab_data['fe2a_ta3gel'];
        $qemt_qst = $talab_data['qemt_qst'];
        $data['paid'] = 'wait';
        $this->db->where('t_rkm_fk', $solfa_rkm)->where('month', $for_month)->update('hr_solaf_quest', $data);
        switch ($fe2a_ta3gel) {
            case 1:
                if ($for_month < 12) {
                    $for_month = $for_month + 1;
                } else if ($for_month == 12) {
                    $for_month = 1;
                }
                $this->db->set('value_of_qst', 'value_of_qst+' . $qemt_qst, FALSE)->where('t_rkm_fk', $solfa_rkm)->where('paid', 'no')->where('month', $for_month)->update('hr_solaf_quest');
                break;
            case 2:
                $id = $this->db->select_max('id')->where('paid', 'no')->where('t_rkm_fk', $solfa_rkm)->get('hr_solaf_quest')->row_array()['id'];
                $this->db->set('value_of_qst', 'value_of_qst+' . $qemt_qst, FALSE)->where('id', $id)->where('paid', 'no')->update('hr_solaf_quest');
                break;
            case 3:
                $count = $this->db->where('t_rkm_fk', $solfa_rkm)->where('paid', 'no')->get('hr_solaf_quest')->num_rows();
                $qust_value = ($qemt_qst / $count);
                $qust_remain = (($qust_value - round($qust_value)) * $count);
                $this->db->set('value_of_qst', 'value_of_qst+' . $qust_value, FALSE)->where('t_rkm_fk', $solfa_rkm)->where('paid', 'no')->update('hr_solaf_quest');
                $id = $this->db->select_max('id')->where('paid', 'no')->where('t_rkm_fk', $solfa_rkm)->get('hr_solaf_quest')->row_array()['id'];
                $this->db->set('value_of_qst', 'value_of_qst+' . $qust_remain, FALSE)->where('id', $id)->where('paid', 'no')->update('hr_solaf_quest');
                break;
            case 4:
                $last_qust_data = $this->db->select_max('id')->where('paid', 'no')->where('t_rkm_fk', $solfa_rkm)->get('hr_solaf_quest')->row_array();
                $last_qust_data['value_of_qst']=$qemt_qst;
                $for_month=$last_qust_data['month'];
                if ($for_month < 12) {
                    $for_month = $for_month + 1;
                } else if ($for_month == 12) {
                    $for_month = 1;
                    $last_qust_data['year']=$last_qust_data['year']+1;
                }
                $last_qust_data['month']=$for_month;
                $datas['qst_date_ar'] = date("Y-m-d", strtotime("+" . 1 . " month", ($last_qust_data['qst_date'])));
                $datas['qst_date'] = strtotime(date("Y-m-d", strtotime("+" . 1 . " month", $last_qust_data['qst_date'])));
                break;
            default:
                break;
        }
    ...
    public function select_from_hr_solaf_ta3gel($arr)
    {
        $this->db->select('hr_solaf_ta3gel.*,employees.id as emp_id,employees.employee');
        $this->db->from("hr_solaf_ta3gel");
        $this->db->where($arr);
        $this->db->join('employees', 'employees.id=hr_solaf_ta3gel.emp_id_fk', 'left');
        $this->db->order_by("hr_solaf_ta3gel.id", "DESC");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $x = 0;
    ...
    public function get_solaf_ta3gel_data($arr)
    {
        $this->db->select('hr_solaf_ta3gel.*,employees.id as emp_id,employees.employee,employees.degree_id,employees.personal_photo
        ,all_defined_setting.defined_id,all_defined_setting.defined_title as job_title
         ');
        $this->db->from(' hr_solaf_ta3gel');
        $this->db->where($arr);
