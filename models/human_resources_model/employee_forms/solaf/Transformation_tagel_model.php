...
class Transformation_tagel_model extends CI_Model
{
...
// omnia
    ...
    {
        $q = $this->db->where('level', $level)->where('tbl', 'tagel')->get('hr_all_transformation_setting')->row();
        if (!empty($q)) {
            return $q;
        }
    }
    ...
    {
        $this->db->select('*');
        $this->db->from("hr_all_solaf_tagel_history");
        $this->db->where($arr);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $query->row();
    ...
    public function select_allhistory_by_tagel_rkm_fk($tagel_rkm_fk)
    {
        $this->db->select('*');
        $this->db->from("hr_all_solaf_tagel_history");
        $this->db->where("tagel_rkm_fk", $tagel_rkm_fk);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $c = 0;
...
// Get_photo
    ...
    public function save_from_transfomation()
    {
        ...
        $insert['tagel_rkm_fk'] = $this->input->post('tagel_rkm_fk');
        $insert['tagel_id_fk'] = $this->input->post('tagel_id_fk');
        ...
        switch ($level) {
            ...
            case 3:
                ...
                if ($action == 1) {
                    $this->make_quset($insert['tagel_id_fk']);
                }
                break;
            default:
                break;
        ...
        // die;
        $update['seen'] = 0;
        $this->db->insert("hr_all_solaf_tagel_history", $insert);
        $this->db->where('t_rkm', $this->input->post('tagel_rkm_fk'));
        $this->db->update('hr_solaf_tagel', $update);
    }

    function make_quset($talab_id)
    {
        $talab_data = $this->db->where('id',$talab_id)->get('hr_solaf_tagel')->row_array();
        $solfa_rkm = $talab_data['solfa_rkm'];
        $for_month = $talab_data['for_month'];
        $fe2a_tagel = $talab_data['fe2a_tagel'];
    ...
    public function select_from_hr_solaf_tagel($arr)
    {
        $this->db->select('hr_solaf_tagel.*,employees.id as emp_id,employees.employee');
        $this->db->from("hr_solaf_tagel");
        $this->db->where($arr);
        $this->db->join('employees', 'employees.id=hr_solaf_tagel.emp_id_fk', 'left');
        $this->db->order_by("hr_solaf_tagel.id", "DESC");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $x = 0;
    ...
    public function get_solaf_tagel_data($arr)
    {
        $this->db->select('hr_solaf_tagel.*,employees.id as emp_id,employees.employee,employees.degree_id,employees.personal_photo
        ,all_defined_setting.defined_id,all_defined_setting.defined_title as job_title
         ');
        $this->db->from(' hr_solaf_tagel');
        $this->db->where($arr);
