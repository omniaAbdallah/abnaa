...
//    die;
    ...
    public function get_agaza_data($arr)
    {
        ...
        employees.edara_n ,  employees.qsm_n, employees.mosma_wazefy_n, 
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($vac_id == 2) {
                $result = $this->get_days_vacation_year($emp_code, $vac_id, $query->row()->agaza_to_date);
            } elseif ($vac_id == 1) {
                $result = $this->get_days_vacation_cousal_by_vid($emp_code, $vac_id, $query->row()->agaza_to_date);
            } else {
                $result = $this->get_days_vacation_by_vid($emp_code, $vac_id, $query->row()->agaza_to_date);
            }
            if (!empty($result)) {
    ...
    public function get_days_vacation_year($emp_id, $vac_id, $agaza_to_date)
    {
    ...
    public function get_days_vacation_cousal_by_vid($emp_id, $vac_id, $agaza_to_date)
    {

        $q = $this->db->select('casual_vacation_num, emp_code')
            ->where('emp_code', $emp_id)
            ->get('contract_employe')->row();
        $q->vDays = $this->get_days_pervious($q->emp_code, $vac_id, $agaza_to_date);
    ...
    public function get_days_pervious($emp_code, $vac_id, $agaza_to_date, $current_year = 0)
    {
        $this->db->select(' SUM(hr_all_agzat_orders.num_days) as vDays')
            ->where('emp_code_fk', $emp_code)
            ->where('no3_agaza', $vac_id)
            ->where('suspend', 4)
            ->where('agaza_from_date <=', $agaza_to_date);
    ...
    public function get_days_vacation_by_vid($emp_code, $vac_id, $agaza_to_date)
    {
        $q = $this->db->select('max_days')
            ->where('id', $vac_id)
            ->get('holiday_setting')->row();
        $q->vDays = $this->get_days_pervious($emp_code, $vac_id, $agaza_to_date);
