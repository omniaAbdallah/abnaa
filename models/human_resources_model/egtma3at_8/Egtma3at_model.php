        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query as $key => $row) {
                /* $query[$key]->member_data = $this->get_by_mem('md_all_gam3ia_omomia_members', array('id' => $row->emp_id_fk));
                 $query[$key]->odwiat_data = $this->get_by_mem('md_all_gam3ia_omomia_odwiat', array('member_id_fk' => $row->emp_id_fk));*/
            }
            return $query;
        } else {
            return false;
...
//        }
    ...
    {
        $this->db->select('hr_all_glasat_emp_hdoor.*, md_all_gam3ia_omomia_odwiat.rkm_odwia, md_all_gam3ia_omomia_odwiat.subs_to_date_m, md_all_gam3ia_omomia_odwiat.rkm_odwia_full');
        $this->db->from('hr_all_glasat_emp_hdoor');
