...
//25-4-om
    public function select_all()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                ...
                $data[$x]->gadwal_a3mal = $this->get_gadwal_a3mal($row->glsa_rkm, $row->mgls_code);
                $x++;
            }
            return $data;
        } else {
    ...
    public function get_gadwal_a3mal($glsa_rkm, $mgls_code)
    {
        return $this->db->select('COUNT(id) as count ')->where('mgls_code', $mgls_code)->where('glsa_rkm', $glsa_rkm)
            ->get('md_gadwal_a3mal')->row()->count;
    }

    public function delete_galsa($rkm, $code)
    {
        $this->db->where('mgls_code', $code)->where('glsa_rkm', $rkm)->delete('md_all_glasat');
        $this->db->where('mgls_code', $code)->where('glsa_rkm', $rkm)->delete('md_all_glasat_hdoor');
