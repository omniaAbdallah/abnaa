...
class Donor_web extends CI_Model
{
...
//---------------------------------------------------
    ...
    //=======================================================
    public function select_donors($where,$approve){
        $this->db->select('*');
    ...
    //===========================
    public  function  insert_guaranty($national_id_file,$bank_card_file,$bank_deduction_voucher_file,$other_img_file){
        ...
        $data['num_payments']=$this->chek_Null( $this->input->post('num_payments'));
        $data['num_of_aytam']=$this->chek_Null( $this->input->post('num_of_aytam'));
        $data['need_paid']=$this->chek_Null( $this->input->post('need_paid'));
    ...
    public function approve($id){
        $data['approve']=1;
        $this->db->where('id', $id);
        if($this->db->update('donors_t',$data)) {
            return true;
        }else{
            return false;
        }
    ...
    public function refuse($id){
        $update['approve']=2;
        $this->db->where('id', $id);
        if($this->db->update('donors_t',$update)) {
            return true;
        }else{
            return false;
        }
