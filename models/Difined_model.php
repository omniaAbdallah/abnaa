 ...
 *  */
   ...
   public function select_search_mosama(){
        $this->db->select('*');
        $this->db->from('hr_egraat_setting');
        $this->db->where('code',801); 
         $this->db->or_where('code',802); 
          $this->db->or_where('code',803); 
           $this->db->or_where('code',804);    
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $data[] = $row;
  ...
  public function select_search_key($table,$search_key,$search_key_value){
   ...
   public function select_search_key_files($table,$search_key,$search_key_value){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($search_key,$search_key_value); 
         $this->db->where('id_setting !=',2106);
    ...
    public function funcname()
{
    ...
    $this->db->join('penalty b', 'b.emp_id=a.id', 'left');
    $this->db->join('mon_rewards c', 'c.emp_id=a.id', 'left');
 ...
 public function select_donors($where){
        $this->db->select('*');
     ...
     public function select_all_users($table,$grouby,$limit,$order_by,$order_by_desc_asc){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->group_by($grouby);
        $this->db->order_by($order_by,$order_by_desc_asc);
