   	...
   	$sql = $this->db->select('*')
		...
		if ($sql->num_rows() > 0) {
			...
			foreach ($sql->result() as $row) {
				...
				$data[$i]->device_name = $this->get_device_id_fk($row->restore_id_fk);
				$i++;
			}
			return $data;
		}
...
public function getById_Restore_repair($id)
	{
		return $this->db->select('restore_repairs_order.*, mother.m_first_name, mother.m_father_name, mother.m_family_name, mother.m_birth_date_hijri, mother.m_mob')
