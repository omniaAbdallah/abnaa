...
class Pages_m extends CI_Model
{
    ...
    public function get_my_page_permession()
    {
        ...
        $this->db->from("pages_gam3ia_omomia");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $i = 0;
            $data = $query->result();
    ...
    public function get_sub($id)
    {
        ...
        $query= $this->db->get('pages_gam3ia_omomia');
        $data=array();
        $x=0;
        if($query->num_rows()>0)
        {
...
//        $this->db->where('user_id',$_SESSION['user_id']);
        ...
        $parent = $this->db->get("pages_gam3ia_omomia");
    ...
    public  function getgroupbyid($id){
        $query= $this->db->get_where('pages_gam3ia_omomia',array('page_id'=>$id));
        return $query->row_array();
    }
    public function get_group_title($id){
        $h = $this->db->get_where("pages_gam3ia_omomia", array('page_id'=>$id));
        $data=$h->row_array();
