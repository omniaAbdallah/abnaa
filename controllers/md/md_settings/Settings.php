...
$data['mgls_reasons_settings'] =   $this->Settings_model->mgls_get_reasons_settings();
    ...
    public function AddSitting($type){  //  md/md_settings/Settings/AddSitting
        ...
        elseif ($this->input->post($type)=='w2f_3doya_reasons'){
            $this->Settings_model->add_reasons();
            $this->message('success','إضافة ');
            redirect('md/md_settings/Settings/settings/'.$type ,'refresh');

        }
        elseif ($this->input->post($type)=='w2f_mgls_reasons'){
            $this->Settings_model->mgls_add_reasons();
            $this->message('success','إضافة ');
            redirect('md/md_settings/Settings/settings/'.$type ,'refresh');
    ...
    public function UpdateSitting($id,$type){
        ...
        $data['reasons_settings_result'] = $this->Settings_model->getById_reasons_settings($id);
        $data['mgls_reasons_settings_result'] = $this->Settings_model->mgls_getById_reasons_settings($id);
        if($this->input->post('add')){
    ...
    public function Deletew2f_3doya_reasons($id){
        $type="w2f_3doya_reasons";
        $this->Settings_model->delete_reasons($id);
        $this->message('success','حذف ');
    redirect('md/md_settings/Settings/settings/'.$type ,'refresh');
    }
    public function Deletew2f_mgls_reasons($id){
        $type="w2f_mgls_reasons";
