...
return  $this->session->set_flashdata('message', '<div class="alert alert-danger alert-dismissable">
  ...
  /**
     ...
     */      
        ...
        public function add_main_data(){   // main_data/Main_data/add_main_data
        $this->load->model('main_data/Model_main_data');
        if ($this->input->post('add')){
            ...
            redirect('main_data/Main_data/add_main_data');
        }
        ...
        if($this->input->post('num_m'))
        {
            if($this->input->post('num_m') != 0)
            {
                ...
                $this->load->view('admin/main_data/main_data', $data);
            }
        }
        else{
     ...
     public function update_main_data($id){  // main_data/Main_data/update_main_data
        ...
        if($this->input->post('add')){
          ...
          $this->Model_main_data->delete($id);
            ...
            redirect('main_data/Main_data/add_main_data','refresh');
        }
        $data['table'] = '';
...
//         $this->test($data['result']);
     ...
     /*public function add_main_data(){   // main_data/Main_data/add_main_data
        ...
        if ($this->input->post('add')){
            ...
            $file= $this->upload_image($file_name);
            $row = $this->Model_main_data->insert($file);
            $this->Model_main_data->insert_phone($row);
            $this->Model_main_data->insert_fax($row);
            $this->Model_main_data->insert_email($row);
            $this->Model_main_data->insert_banks($row);
    ...
    public function get_banks()
    {
        $this->load->model('main_data/Model_main_data');
        $data['banks'] = $this->Model_main_data->select_bank();
        $this->load->view('admin/main_data/get_banks', $data);
  }
    ...
    public function update_bank_status($id,$bank_account,$gmaya_id)
    {
        ...
        redirect("main_data/Main_data/update_main_data/$gmaya_id" ,'refresh');
    }
