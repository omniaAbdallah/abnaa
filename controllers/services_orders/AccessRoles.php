...
class AccessRoles extends MY_Controller
{
     ...
     */
     ...
     public  function EmployeeRoles(){   //  services_orders/AccessRoles/EmployeeRoles
          
         $data["all_employees"]=$this->Model_rules_service->select_employee();
         $data['title'] = 'صلاحيات الموظفين ';
         $data['metakeyword'] = 'صلاحيات الموظفين ';
         $data['metadiscription'] = 'صلاحيات الموظفين ';
         $data['subview'] = 'admin/services_orders/services_approved_new/access_roles/add_employee_roles';
         $this->load->view('admin_index', $data);
     }
     //=================================================================================
    public function AddEmployeeRoles($emp_code){  //  services_orders/AccessRoles/AddEmployeeRoles/
       
        if($this->input->post("operation")  =="UPDATE"){
            $this->Model_rules_service->delete_emp_role($emp_code);
            if(sizeof($this->input->post("roles")) > 0){
                $this->Model_rules_service->insert_emp_role($emp_code);
            }
            redirect('services_orders/AccessRoles/EmployeeRoles', 'refresh');
        }
