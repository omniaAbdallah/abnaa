...
class Strategy extends MY_Controller {
    ...
    public function e3dad_strategy()//gwd/strategy/Strategy/e3dad_strategy
    {
        //$data['customer_js'] = $this->load->view('admin/gwd_v/strategy_v/app_js', '', TRUE);
        $data['subview'] = 'admin/gwd_v/strategy_v/e3dad_strategy';
        $this->load->view('admin_index', $data);
    }
    public function data_e3dada()
    {
        ...
        if(!empty($all_strategy)){
            ...
            foreach($all_strategy as $row){
                ...
                $arr['data'][] = array(
                    ...
                    ' <a target="_blank" class="btn btn-sm btn-add"  href="#"> 
                    <i class="hvr-buzz-out fa fa-recycle" aria-hidden="true">
                          </i>
                           إرسال الخطه</a>  '
                    ,
                    '
                         <a target="_blank" class="btn btn-sm btn-primary"  href="'. base_url() .'gwd/Gawda_plans/gawda_plans_tree/'.$row->id.'"> 
                ...
                $arr['data'][] = array(
                    ...
                    '
                      ...
                      <ul class="dropdown-menu">
                       <li><a target="_blank"
                         href="'. base_url() .'gwd/Gawda_plans/gawda_plans_tree/'.$row->id.'"> 
                         إعداد الخطة</a></li>
                          <li><a target="_blank"
                         href="'. base_url() .'gwd/strategy/Strategy/strategy_files/'.$row->id.'"> 
                          ...
                          <li><a target="_blank"
                         href="'. base_url() .'gwd/strategy/Strategy/add_wants_values/'.$row->id.'"> 
                          إضافه القيم المستهدفة  </a></li>


                          <li><a target="_blank"
                         href="'. base_url() .'gwd/strategy/Strategy/add_achived_values/'.$row->id.'"> 
                          إضافه القيم المتحققة   </a></li>
                          <li><a target="_blank"
                          href="'. base_url() .'gwd/strategy/Strategy/all_strategy/'.$row->id.'"> 
                          المؤشرات الاستراتيجية   </a></li>
   ...
   '
    ...
    {
        $data['all_Emps'] = $this->Strategy_model->get_by('employees',array('employee_type'=>1,'status'=>96));
    ...
    {
        $data['record'] = $this->Strategy_model->get_by('gwd_strategy_programs',array('strategy_id_fk'=>$prog_id))[0];
     if(!empty($data['record'])){
       // $data['tshgly_strtegy_name'] = $this->Strategy_model->get_by('gwd_strategy_plans',array('id'=>$data['record']->strategy_id_fk))[0]->pln_name;
        //$data['tshgly_strtegy_name']->pln_name;
        $data['strtegy_name'] = $this->Strategy_model->get_by('gwd_strategy_plans',array('id'=>$data['record']->strategy_id_fk))[0]->pln_name;
     }
       $data['get_all'] = $this->Strategy_model->get_by('gwd_strategy_plans',array('id'=>$prog_id),1);
       // $this->test($data['tshgly_strtegy_name']);
        $data['title'] = 'أضافه   القيم المستهدفة';
        $data['subview'] = 'admin/gwd_v/strategy_v/prog_wants_values';
        $this->load->view('admin_index', $data);
    }
  public function add_wants()
  {
      $prog_id=$this->input->post('id');
      $strategy_id_fk=$this->input->post('strategy_id_fk');
    if ($this->input->post('update')) {     
        $this->Strategy_model->update_wants_values_program($prog_id);
        echo $strategy_id_fk;
    }
    if ($this->input->post('add')=='add') {     
        $this->Strategy_model->insert_wants_values_program($strategy_id_fk);
        echo $strategy_id_fk;
    }
  }
  //////////////////yara28-9-2020
  public function add_achived_values($prog_id)
  {
     ...
     // $this->test($data['tshgly_strtegy_name']);
      ...
      $data['subview'] = 'admin/gwd_v/strategy_v/prog_achived_values';
      $this->load->view('admin_index', $data);
  }
public function add_achived()
{
    ...
    $strategy_id_fk=$this->input->post('strategy_id_fk');
  if ($this->input->post('update')) {     
      $this->Strategy_model->insert_achived_values_program($prog_id);
      echo $strategy_id_fk;
  }
}

public function all_strategy($id)//gwd/strategy/Strategy/all_strategy
    {
        $data['all_data']=$this->Strategy_model->all_pln_data($id);
       // $this->test($data['all_data']);
        $data['title'] = "المؤشرات الاستراتيجية";
        $data['subview'] = 'admin/gwd_v/strategy_v/all_strategy';
        $this->load->view('admin_index', $data);
    }
