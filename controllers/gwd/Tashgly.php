        ...
        if(!empty($all_strategy)){
            ...
            foreach($all_strategy as $row){
                ...
                $arr['data'][] = array(
                    ...
                    $row->program_wasf,
                    ' 
                    <a target="_blank" class="btn btn-sm btn-add" href="'. base_url() .'gwd/Tashgly/add_mo24er_prog/'.$row->id.'" >
                    <i class="hvr-buzz-out fa fa-recycle" aria-hidden="true">
                          </i>
                            اضافه مؤشر تشغيلي</a>
...
//old
  ...
  {
    $data['record'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id))[0];
     if(!empty($data['record'])){
      $data['tshgly_strtegy_name'] = $this->Tashgly_model->get_by('gwd_tashgly_plans',array('id'=>$data['record']->tashgly_id_fk))[0]->pln_name;
      $data['tshgly_prog_name'] = $this->Tashgly_model->get_by('gwd_tashgly_programs',array('id'=>$data['record']->program_id_fk))[0]->program_name;
      $data['tshgly_mo24er_prog'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('tashgly_id_fk'=>$data['record']->tashgly_id_fk,'program_id_fk'=>$data['record']->id,'all_wanted_values!='=>0,'total_result'=>NULL));
    }
    $data['result'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id));
     // $this->test($data['tshgly_strtegy_name']);
      $data['title'] = 'أضافه   القيم المتحققه';
      $data['subview'] = 'admin/gwd_v/tashgly_v/prog_achived_values';
      $this->load->view('admin_index', $data);
  }
  public function update_achived_values()
  {
      $prog_id=$this->input->post('prog_id');
      $id=$this->input->post('id');
      $data['record'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id))[0];
     // $data['record'] = $this->Tashgly_model->get_by('gwd_tashgly_programs',array('id'=>$prog_id))[0];
       $data['recordd'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('id'=>$id))[0];
      if(!empty($data['record'])){
       $data['tshgly_strtegy_name'] = $this->Tashgly_model->get_by('gwd_tashgly_plans',array('id'=>$data['record']->tashgly_id_fk))[0]->pln_name;
       $data['tshgly_prog_name'] = $this->Tashgly_model->get_by('gwd_tashgly_programs',array('id'=>$data['record']->program_id_fk))[0]->program_name;
       $data['tshgly_mo24er_prog'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('tashgly_id_fk'=>$data['record']->tashgly_id_fk,'program_id_fk'=>$data['record']->id));
     
      }
     // $this->test($data['tshgly_strtegy_name']);
      $data['title'] = 'تعديل القيم المتحققه';
     // $data['subview'] = 'admin/gwd_v/tashgly_v/add_mo24er_prog';
      $this->load->view('admin/gwd_v/tashgly_v/prog_achived_values', $data);
  }
  public function get_wants_data()
  {
    $id=$this->input->post('id');
    $data=$this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('id'=>$id))[0];
    echo json_encode($data);
  }
public function add_achived()
{
    $prog_id=$this->input->post('program_id_fk');
    $tashgly_id_fk=$this->input->post('tashgly_id_fk');
  if ($this->input->post('update')) {
    $id=$this->input->post('mo24er_id_fk');
      $this->Tashgly_model->update_achived_values_program($id);
     // echo $tashgly_id_fk;
  }
...
public function add_wants_values($prog_id)
    {
       ...
       if(!empty($data['record'])){
        $data['tshgly_strtegy_name'] = $this->Tashgly_model->get_by('gwd_tashgly_plans',array('id'=>$data['record']->tashgly_id_fk))[0]->pln_name;
        $data['tshgly_prog_name'] = $this->Tashgly_model->get_by('gwd_tashgly_programs',array('id'=>$data['record']->program_id_fk))[0]->program_name;
        $data['tshgly_mo24er_prog'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('tashgly_id_fk'=>$data['record']->tashgly_id_fk,'program_id_fk'=>$data['record']->id,'all_wanted_values'=>0));
       }
       $data['result'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id));
       // $this->test($data['tshgly_strtegy_name']);
    ...
    public function update_wants_values()
    {
        $prog_id=$this->input->post('prog_id');
        $id=$this->input->post('id');
        $data['record'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id))[0];
       ...
       // $this->test($data['tshgly_strtegy_name']);
        $data['title'] = 'تعديل القيم المستهدفة';
       // $data['subview'] = 'admin/gwd_v/tashgly_v/add_mo24er_prog';
        $this->load->view('admin/gwd_v/tashgly_v/prog_wants_values', $data);
    }
    // form_wants_values
    public function form_wants_values()
    {
        $prog_id=$this->input->post('prog_id');;
        $data['record'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id))[0];
        if(!empty($data['record'])){
  ...
  //  $prog_id=$this->input->post('program_id_fk');
   // $tashgly_id_fk=$this->input->post('tashgly_id_fk');
    if ($this->input->post('update')) {     
        $id=$this->input->post('mo24er_id_fk');
        $this->Tashgly_model->update_wants_values_program($id);
        echo $tashgly_id_fk;
    }
    else if($this->input->post('add'))
    {
  ...
  //////////////////////new__funccc//
  public function add_mo24er_prog($prog_id)
  {
      $data['record'] = $this->Tashgly_model->get_by('gwd_tashgly_programs',array('id'=>$prog_id))[0];
     if(!empty($data['record'])){
      $data['tshgly_strtegy_name'] = $this->Tashgly_model->get_by('gwd_tashgly_plans',array('id'=>$data['record']->tashgly_id_fk))[0]->pln_name;
     ...
     // $this->test($data['tshgly_strtegy_name']);
     $data['result'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('program_id_fk'=>$prog_id));
      $data['title'] = 'أضافة الموشرات التشغلية';
      $data['subview'] = 'admin/gwd_v/tashgly_v/add_mo24er_prog';
      $this->load->view('admin_index', $data);
  }
  public function update_mo24er_prog()
  {
      $prog_id=$this->input->post('prog_id');
      $id=$this->input->post('id');
      $data['recordd'] = $this->Tashgly_model->get_by('gwd_tashgly_prog_mo24er',array('id'=>$id))[0];
     ...
     // $this->test($data['tshgly_strtegy_name']);
      $data['title'] = 'تعديل الموشرات التشغلية';
     // $data['subview'] = 'admin/gwd_v/tashgly_v/add_mo24er_prog';
      $this->load->view('admin/gwd_v/tashgly_v/add_mo24er_prog', $data);
  }
public function add_add_mo24er()
{
   
  //  $tashgly_id_fk=$this->input->post('tashgly_id_fk');
  if ($this->input->post('add')) {     
      $this->Tashgly_model->insert_mo24er_program();
    //  echo $tashgly_id_fk;
  }
 else if ($this->input->post('update')) {     
    $id=$this->input->post('id');
    $this->Tashgly_model->update_mo24er_program($id);
 //   echo $tashgly_id_fk;
}

}
public function data_mo24er()
    {
        $prog_id=$this->input->post('prog_id');
        $all_strategy =  $this->Tashgly_model->select_all_program_mo24er($prog_id);
        $arr = array();
        $arr['data'] = array();
        if(!empty($all_strategy)){
            $x = 0;
            foreach($all_strategy as $row){
                ...
                $arr['data'][] = array(
                  ...
                  <a href="#" onclick=\'swal({
                                ...
                                function(){
                                    get_mo24er_program('.$row->id.','.$row->program_id_fk.');
                            });\'>
                            <i class="fa fa-pencil-square-o"> </i></a>
                  <a href="#" onclick=\'swal({
                                title: "هل انت متأكد من الحذف ؟",
                                ...
                                function(){
                                    delete_mo24er_program('.$row->id.','.$row->program_id_fk.');
                            });\'>
                            <i class="fa fa-trash"> </i></a>
   '
                );
    ...
    public function delete_mo24er_program()
    {
        $id = $this->input->post('id');
        $this->Tashgly_model->delete_mo24er_program($id);
        echo 1;
        return ;
    }
    ...
    // data_wants_value
    public function data_wants_value()
    {
        $prog_id=$this->input->post('prog_id');
        $all_strategy =  $this->Tashgly_model->select_all_wants_mo24er($prog_id);
        $arr = array();
        $arr['data'] = array();
        if(!empty($all_strategy)){
            $x = 0;
   ...
   '
    ...
    public function delete_wants_values()
    {
        $id = $this->input->post('id');
        $this->Tashgly_model->delete_wants_values($id);
        echo 1;
        return ;
    }

    public function data_achived_value()
    {
        $prog_id=$this->input->post('prog_id');
        $all_strategy =  $this->Tashgly_model->select_all_achived_mo24er($prog_id);
        $arr = array();
        $arr['data'] = array();
        if(!empty($all_strategy)){
            $x = 0;
