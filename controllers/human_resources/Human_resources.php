...
class Human_resources extends MY_Controller
{
    ...
    */
    ...
    public function index()
    {
        ...
        $data["last_emp_code"] = $this->Employee_model->select_last_code_3();
        $data['deyana'] = $this->Difined_model->select_search_key('employees_settings', 'type', '3');
        $data['type_card'] = $this->Difined_model->select_search_key('employees_settings', 'type', '5');
    ...
    public function emp_card()
    {

        $data["personal_data"] = $this->Employee_model->get_emp_card(5);

        $data['subview'] = 'admin/Human_resources/add_employee/emp_card';
        $this->load->view('admin_index', $data);
    }
    ...
    { //manager_category
        ...
        /*  $data["direct_all_emps"] = $this->Employee_model->get_direct_manger($data["emp"]->edara_id);
       if (!empty($data['emp'])) {
         ...
         */
        $data["direct_all_emps"] = $this->Employee_model->get_new_direct_mangerby_degree_id($data["emp"]->degree_id);/*3-9-20-om*/
        /* if (!empty($data['emp'])) {
             $data['job_data'] = $this->Employee_model->get_new_job_data(array('qsm' => $data['emp']->qsm_id));
         } else {
             $data['job_data'] = $this->Employee_model->get_new_job_data();
        ...
        if (!empty($data['emp'])) {
            ...
            if (!empty($data['emp']->qsm_id)) {
                $cond_arr = array('qsm' => $data['emp']->qsm_id);
            } else {
                $cond_arr = array('edara' => $data['emp']->edara_id);
            }
            $data['job_data'] = $this->Employee_model->get_new_job_data($cond_arr);


            // $data['job_data'] = $this->Employee_model->get_new_job_data(array('qsm' => $data['emp']->qsm_id));
            $data['departs'] = $this->Difined_model->select_search_key('hr_edarat_aqsam', 'from_id_fk', $data['emp']->edara_id);

        } else {
            $data['job_data'] = $this->Employee_model->get_new_job_data();
            $data['departs'] = $this->Difined_model->select_all('hr_edarat_aqsam', '', '', 'trteeb', 'ASC');

        }


        $data['edarads'] = $this->Difined_model->select_search_key('hr_edarat_aqsam', 'from_id_fk', '0');


        $data['mosayer_category'] = $this->Employee_model->get_mosayer_cat();
        //$data['departs'] = $this->Employee_model->select_depart();
    ...
    /*
     function get_new_job_data()
 {
     $qsm = $this->input->post('depart_id');
     $where_arr = array('qsm' => $qsm);
     $data = $this->Employee_model->get_new_job_data($where_arr);
    ...
    function get_new_job_data()
    {
        ...
        if (!empty($qsm) && ($qsm != 'undefined') && ($qsm != ' ')) {
            $where_arr = array('qsm' => $qsm);
        } elseif ((!empty($edara_id)) && ($edara_id != 'undefined') && ($edara_id != ' ')) {
            $where_arr = array('edara' => $edara_id);
        }
        $data = $this->Employee_model->get_new_job_data($where_arr);
        echo json_encode($data);
    ...
    function get_new_depatment_data()
    {
        $edara_id = $this->input->post('edara_id');

        $data = $this->Difined_model->select_search_key('hr_edarat_aqsam', 'from_id_fk', $edara_id);
        echo json_encode($data);
    }

    public function get_new_direct_manger()
    {
        $id = $this->input->post('marg3_mobasher');
        $data["direct_mangers"] = $this->Employee_model->get_new_direct_manger($id);
    ...
    {
        $administrative_structure_id = $this->input->post('depart_id');
        $where_arr = array('administrative_structure_id_fk' => $administrative_structure_id);
        $data = $this->Employee_model->get_job_data($where_arr);
        echo json_encode($data);
    }
...
//======================= ahmed zedan ==============================================================  
    ...
    /*    function load_mrakz()
  {
    ...
    public function add_financeEmployee()
    {
        if ($this->input->post('add')) {
            $emp_id = $this->input->post('emp_id');
            $emp_code = $this->input->post('emp_code');
            $this->Finance_employee_model->emp_badlat_discount_details($emp_id, $emp_code);
        }
    }

    public function addfinanceEmployee()
    {
        $emp_code = $this->input->post('emp_code');
        $this->Finance_employee_model->financeEmployee($emp_code);
    }


    /*  public function add_banks_employee(){
        $emp_id=$this->input->post('emp_id');
       // $this->bank_employes_details($emp_code);
        $this->Finance_employee_model->bank_employes_details($emp_id);
   }*/
    //-----------------------------------------
    ...
    public function load_details_banks()
    {
        ...
        $data['allData'] = $this->Finance_employee_model->getAllData($empCode)[0];
        $data['banks'] = $this->Finance_employee_model->getBanks();
        $this->load->view('admin/Human_resources/badlat_setting/load_banks_tabel', $data);
    }

    public function delete_emp_finance_data()
    {
    ...
    public function load_details()
    {
        ...
        $data['bdalat_id'] = $this->Finance_employee_model->getBadalat_id(1);
        $data['badalat'] = $this->Finance_employee_model->getBadalat(1);
        $this->load->view('admin/Human_resources/badlat_setting/load_tabel_esthkak', $data);
    }

    public function load_details_estktat3()
    {
        $empCode = $this->input->post('emp_id');
        $data['allData'] = $this->Finance_employee_model->getAllData($empCode)[0];
        $data['cuts_id'] = $this->Finance_employee_model->getBadalat_id(2);
        $data['discounts'] = $this->Finance_employee_model->getBadalat(2);
        $this->load->view('admin/Human_resources/badlat_setting/load_tabel_estkta3', $data);
    }
    ...
    public function add_record()
    {
        ...
        $this->load->view('admin/Human_resources/badlat_setting/add_row', $data);
    }
    //-----------------------------------------
    ...
    public function getBanks()
    {
        ...
        $data['number'] = $this->input->post('count');
        $data['banks'] = $this->Finance_employee_model->getBanks();
        $this->load->view('admin/Human_resources/badlat_setting/getBanks', $data);
    }
    ...
    public function add_option_select()
    {
        ...
        $data['type'] = $this->input->post('type');
        $data['records'] = $this->Finance_employee_model->get_all_badalat($data['type'], $ids);
        $this->load->view('admin/Human_resources/badlat_setting/get_option_select', $data);
    }
    //-----------------------------------------
    /* public function add_option_select(){
          $ids=$this->input->post('valu');
...
//======================= ahmed zedan ==============================================================
    ...
    public function contractEmployee($empCode) // human_resources/Human_resources/contractEmployee/7
    {
        ...
        $data['check_finance_data'] = $this->Contract_employee_model->check_finance_data_new($empCode);
    ...
    public function employee_files($emp_id)
    { // human_resources/Human_resources/employee_files/7
        $this->load->model('Difined_model');
        $this->load->model('human_resources_model/Files_employee_model');
        /*if ($this->input->post('add')) {
        ...
        if ($this->input->post('add')) {
            ...
            $this->Files_employee_model->insert($this->uri->segment(4), $files);
            $this->messages('success', 'تسجيل بيانات المستندات والبطاقات والمهارات');
            redirect("human_resources/Human_resources/employee_files/" . $emp_id . "");
        }
        $data["personal_data"] = $this->Employee_model->get_one_employee($emp_id);
        $data['files_names'] = $this->Difined_model->select_search_key("employees_settings", 'type', 16);
    ...
    public function custody($empCode){ // human_resources/Human_resources/custody/7
     ...
     public function get_custody()
        ...
        */
    public function custody($empCode)
    {     // human_resources/Human_resources/custody/7
        $this->load->model('human_resources_model/Custody_employee_model');
    ...
    { //human_resources/Human_resources/print_employee/987654321000
    ...
    /****************************************************************************************/
    /********************************ahmed*****************************/
    public function add_custody_devices()
    {     //human_resources/Human_resources/add_custody_devices
        $this->load->model('human_resources_model/Custody_devices_model');
        $this->load->model('Difined_model');
 ...
 public function update_human_always_times(){ //human_resources/Human_resources/add_human_always_times
    ...
    */
    ...
    public function add_human_always_times()
    { //human_resources/Human_resources/add_human_always_times
        $this->load->model('human_resources_model/Human_always_times_model');
        $this->load->model('Difined_model');
        $data['settings'] = $this->Difined_model->select_all('always_setting', '', '', "", "");
        $data['records'] = $this->Human_always_times_model->select_all();
    ...
    public function update_human_always_times()
    { //human_resources/Human_resources/add_human_always_times
        $this->load->model('human_resources_model/Human_always_times_model');
        $this->load->model('Difined_model');
        $data['settings'] = $this->Difined_model->select_all('always_setting', '', '', "", "");
    ...
    /*public function edit_having_employee()
         {
             ...
             $emp_code=$this->input->post('emp_code');
             $type=$this->input->post('type');
             $this->Finance_employee_model->update_discut_having_employee($emp_id,$emp_code,$type);
           //  redirect('human_resources/Human_resources/financeEmployee/'.$emp_code,'refresh');
         }*/
    ...
    public function getConnection()
    {
        ...
        if (!empty($all_Donors)) {
            ...
            foreach ($all_Donors as $row_donors) {
                ...
                $arr_donors['data'][] = array(
                    '<span onclick="GetMemberName(this)"   id="member' . $row_donors->id . '" data-city_name="' . $row_donors->city_name . '" data-hai_name="' . $row_donors->name . '"
...
//yara_mo2hl
    ...
    public function employee_files_table($emp_id)
    {
        $this->load->model('human_resources_model/Files_employee_model');
        $data['allData'] = $this->Files_employee_model->getAllData($emp_id);
        $this->load->view('admin/Human_resources/emp_files/employee_files_table', $data);
    }
...
//        redirect("human_resources/Human_resources/employee_files/" . $empCode . "");
    ...
    public function data()
    {
        ...
        $customer = $this->Employee_model->select_allEmployee();
        ...
        if (!empty($customer)) {
            ...
            foreach ($customer as $row) {
                ...
                $main_link = base_url();
                if ($row->personal_photo != null or $row->personal_photo != 0) {
                    $personal_photo = $row->personal_photo;
                    $emp_image = " <img src='$main_link/uploads/human_resources/emp_photo/thumbs/$personal_photo'
    class='img-circle' width='45' height='45' alt='user'> ";
                } else {
                    $emp_image = '';
                }

                if ($row->mosma_wazefy_n != null or $row->mosma_wazefy_n != 0) {
                    $mosma_wazefy_n = $row->mosma_wazefy_n;

                } else {
                    $mosma_wazefy_n = 'غير محدد';
                }
                ...
                $arr['data'][] = array(
                    ...
                    '<span class="badge badge-' . $class . ' m-r-15"
                      style="font-size: 13px; width: 60px;">' . $ok_non_ok . $emp_type[$row->employee_type] . ' </span> ',
                    '<span class="badge badge-primary" style="font-size: 13px;
width: 183px;">' . $mosma_wazefy_n . ' </span> ',
                    ...
                    '
                      ...
                      <ul class="dropdown-menu">
                       ...
                       <li><a target="_blank"
                                 href="' . base_url() . 'human_resources/Human_resources/edit_emp/' . $row->emp_code . '">البيانات
                                  الأساسية </a></li>
                          <li><a target="_blank"
                                 href="' . base_url() . 'human_resources/Human_resources/add_job_data/' . $row->id . '">البيانات
                                  الوظيفيه </a></li>
 ...
 التعاقد </a></li>
 <li><a href="' . base_url() . 'human_resources/Dwam_settings/add_dwam/' . $row->id . '">بيانات
 الدوام </a></li>
 ...
 <li><a href="' . base_url() . 'human_resources/human_resources/emp_card/' . $row->id . ' ">
تجريبي كارت </a></li> 
                  ...
                  <a href="' . base_url() . 'human_resources/tamouh_emp/Tamouh/edit_emp/' . $row->emp_code . '?>"><i
                  class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>  
                  
                  
                  <a href="' . base_url() . 'human_resources/tamouh_emp/Tamouh/DeleteEmpAll/' . $row->id . '?>"><i
   ...
   '
    ...
    //yara
    public function employees()//human_resources/human_resources/employees
    {
        $data['customer_js'] = $this->load->view('admin/Human_resources/add_employee/app_js', '', TRUE);
        $data['subview'] = 'admin/Human_resources/add_employee/employees';
        $this->load->view('admin_index', $data);
    ...
    /*  public function main_egraat()//human_resources/Human_resources/main_egraat
      {
          $data['pdfname'] = "pdf.pdf";
          $data['title'] = "   لائحة  إجراءات الموارد البشرية ";
          $data['subview'] = 'admin/Human_resources/main_egraat_folder/showpdf';
          $this->load->view('admin_index', $data);
      }*/
    ...
    public function main_egraat()//human_resources/Human_resources/main_egraat
    {
        //  $data['system']= $this->Employee_model->select_all_by('pr_system',array('type' =>1 ,'dep_code'=>104 ));
        // $data['sysa']= $this->Employee_model->select_all_by('pr_system',array('type' =>2));

        $data['system'] = $this->Employee_model->get_pr_systems(1, 104);
        $data['sysa'] = $this->Employee_model->get_pr_systems(2, 104);


        /*

        */
        $data['title'] = "   لوائح وسياسات الموارد البشرية ";
        $data['subview'] = 'admin/Human_resources/main_egraat_folder/system_allaw27';
        $this->load->view('admin_index', $data);
    }


    public function sysat_setting()//human_resources/Human_resources/sysat_setting
    {

        $data['title'] = "   إعداد السياسات  ";
        $data['subview'] = 'admin/Human_resources/sysat_setting';
        $this->load->view('admin_index', $data);
    }

    public function add_record_1()
    {
        ...
        $data['type'] = $this->input->post('type');
        if ($type == 1) {
            $ids = $this->Finance_employee_model->getBadalat_id_by_emp($type, $empCode);
        } else {
            $ids = $this->Finance_employee_model->getBadalat_id_by_emp($type, $empCode);
        }
        $data['records'] = $this->Finance_employee_model->get_all_badalat($type, $ids);
...
//new
    public function load_edite_esthkak()
    {
        $id = $this->input->post('id');
        $data['badalatt'] = $this->Finance_employee_model->getBadalat(1);
...
//old
    ...
    {
        $img = 'userfile';
        $img_file = $this->upload_image_bank($img, 'human_resources/emp_banks');
        $emp_id = $this->input->post('emp_id');
        $this->Finance_employee_model->bank_employes_details($emp_id, $img_file);
    }
...
//    new
    public function edite_bank()
    {
        ...
        $id = $this->input->post('id');
        $data['banks'] = $this->Finance_employee_model->getBanks();
        $data['allData'] = $this->Finance_employee_model->getEmpBankby_id($id);
        $this->load->view('admin/Human_resources/badlat_setting/edite_banks', $data);
    }
...
// old
    public function edit_account()
    {
        $img = 'bank_image';
        $img_file = $this->upload_image_bank($img, 'human_resources/emp_banks');
        $this->Finance_employee_model->edit_bank_account($img_file);
    }
