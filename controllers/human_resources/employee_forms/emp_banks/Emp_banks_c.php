...
if($_SESSION['role_id_fk']==1)
{
   ...
   // $this->test($data['allData_added']);
    ...
    $data['allData_suspened']=$this->Emp_banks_model->get_by_options('hr_emp_hesabat_banks_orders',array('emp_id' =>$empCode,'suspend' =>4));
...
public function change_bank_profile()
{
    $this->load->model('human_resources_model/employee_forms/emp_banks/Emp_banks_model');
$empCode=$_SESSION['emp_code'];
$data['employee'] = $this->Emp_banks_model->getEmpData($empCode);
...
public function change_bank_form()//human_resources/employee_forms/emp_banks/Emp_banks_c/change_bank_form
{
...
public function edite_banks_employee()
{
    ...
    $data['bank_id'] = $this->Emp_banks_model->get_by_options_by_id('bank_employes_details', array('emp_code' => $emp_id));
    $data['last_record'] = $this->Emp_banks_model->get_last_record($emp_id);
    if (!empty($data['bank_id'])) {
        $bank_id = $this->Emp_banks_model->get_by_options_by_id('bank_employes_details', array('emp_code' => $emp_id))->id;
        $data['allData'] = $this->Emp_banks_model->getEmpBankby_id($bank_id);
    }
...
public function edite_bank()
{
    $id = $this->input->post('id');
    $data['banks'] = $this->Emp_banks_model->getBanks();
    $data['oneData'] = $this->Emp_banks_model->get_by_options_by_id('hr_emp_hesabat_banks_orders', array('id' => $id));
    if ($this->input->post('from_suspend')) {
        $data['from_suspend'] = 1;
    }
    $this->load->view('admin/Human_resources/employee_forms/emp_banks/edite_banks', $data);
}
public function suspend_bank()
{
    
$id=$this->input->post('id');
$data['banks'] = $this->Emp_banks_model->getBanks();
$data['allData'] =$this->Emp_banks_model->get_by_options_by_id('hr_emp_hesabat_banks_orders',array('id' =>$id));
$this->load->view('admin/Human_resources/employee_forms/emp_banks/suspend_bank',$data);
}
...
$allData = $this->Emp_banks_model->getEmpBankby_id($bank_id);
    ...
    public function print_eqrar(){

        $id=$this->input->post('id');
        $data['banks'] = $this->Emp_banks_model->getBanks();
        $data['allData'] =$this->Emp_banks_model->get_by_options_by_id('hr_emp_hesabat_banks_orders',array('id' =>$id));
    
        $this->load->view('admin/Human_resources/employee_forms/emp_banks/print_eqrar', $data);
    }
    
    
    
    
    public function make_change_bank()//human_resources/employee_forms/emp_banks/Emp_banks_c/make_change_bank
{
    $empCode = $_SESSION['emp_code'];
    $data['last_record'] = $this->Emp_banks_model->get_last_record($empCode);
    $data['employee'] = $this->Emp_banks_model->getEmpData($empCode);
    ...
    if ($this->input->post('from_profile')) {
        $data['loaded'] = 1;
        $data['subview'] = 'admin/Human_resources/employee_forms/emp_banks/main_page';
...
{
    $this->Emp_banks_model->update_seen_hesabat_banks_orders();
