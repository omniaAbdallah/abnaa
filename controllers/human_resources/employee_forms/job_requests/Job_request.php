...
class Job_request extends MY_Controller{
   ...
   /* public function index(){ // human_resources/employee_forms/job_requests/Job_request
        ...
        //yara
        $data['jobtitles']= $this->Job_request_model->get_all_jobs();
        //$this->test($data['jobtitles']);
...
{ // human_resources/employee_forms/job_requests/Job_request
    $data['title'] = 'طلب احتياج وظيفة';
    $data['deparments'] = $this->Job_request_model->get_departments();
    //yara
    $data['jobtitles'] = $this->Job_request_model->get_all_jobs();
    ...
    $data['work_nature'] = $this->Job_request_model->select_forms_settings(2);
    $data['all_requsts'] = $this->Job_request_model->dispaly_requests();
    $data["last_rkm_talab"] = $this->Job_request_model->select_last_rkm_talab();
        ...
        if(!empty($records)) {
            ...
            foreach ($records as $row) {
                ...
                $arr['data'][] = array(
                    ...
                    $row->type_name,
                    $row->nature_name,
                    '<button data-toggle="modal" data-target="#detailsModal" class="btn btn-sm btn-info" style="padding:1px 6px"  
                    onclick="load_page('.$row->id.');">
                           <i class="fa fa-list "></i>
                    </button>
                    <div class="btn-group">
                      <button type="button" class="btn btn-danger">إضافه</button>
                      ...
                      <ul class="dropdown-menu">
                       ...
                       <a href="#" onclick=\'swal({
                          ...
                          <li><a target="_blank"
                         href="'. base_url() .'human_resources/employee_forms/job_requests/Job_request/details_reason_request/'.$row->id.'"> 
                          <i class="fa fa-plus-circle" aria-hidden="true"> </i>   أسباب ومبررات الإحتياج</a>
                           </li>
                           <li><a target="_blank"
                         href="'. base_url() .'human_resources/employee_forms/job_requests/Job_request/details_need_request/'.$row->id.'"> 
                          <i class="fa fa-plus-circle" aria-hidden="true"> </i>   متطلبات العمل بالوظيفة</a>
                           </li>
                           <li><a onclick="print_request('.$row->id .')" >
                                            <i class="fa fa-print"> </i> طباعة</a></li>
...
// ******************************************************************************** //
    ...
    public function add_application(){// human_resources/employee_forms/job_requests/Job_request/add_application
        if($this->input->post('add')=== 'save_main_data'){
            ...
            $data['jobs'] = $this->Job_request_model->select_jobs_settings();
           // $this->test( $data['jobs']);
    ...
    public function read_file($file_name)
    {
       ...
       public function details_reason_request($id){

        $data['get_requset']= $this->Job_request_model->get_by_id($id)[0];

        $data['title'] = ' إضافة أسباب ومبررات الإحتياج';
        $data['title_t'] = ' أسباب ومبررات الإحتياج';
        $data['subview'] = 'admin/Human_resources/employee_forms/job_requests/details_reason';
        $this->load->view('admin_index', $data);

    }
       public function details_need_request($id){

        $data['get_requset']= $this->Job_request_model->get_by_id($id)[0];

        $data['title'] = ' إضافة متطلبات العمل بالوظيفة';
        $data['title_t'] = ' متطلبات العمل بالوظيفة';
        $data['subview'] = 'admin/Human_resources/employee_forms/job_requests/details_need';
        $this->load->view('admin_index', $data);

    }
        public function load_details_result(){

        $row_id = $this->input->post('row_id');
        $type = $this->input->post('type');
        $data['row_id'] = $row_id;
        $data['type'] = $type;
        $data['get_items']= $this->Job_request_model->get_items($row_id,$type);
        $this->load->view('admin/Human_resources/employee_forms/job_requests/load_details_result',$data);
    }
        public function add_update_details(){

        if ($this->input->post('add')){
            $row_id=$this->input->post('row_id');
            $this->Job_request_model->add_request_details($row_id);
            echo 1;
            return ;
        }else if($this->input->post('update')){
            $id=$this->input->post('id');
            $this->Job_request_model->update_request_details($id);
            echo 1;
            return ;
        }
        ...
        $id =$this->input->post('id');
        $this->Job_request_model->delete_item($id);
        echo 1;
        return ;
    }
    ...
    public function applicationsDetails(){//human_resources/employee_forms/Job_requests/Job_request/applicationsDetails
      //  $data['jobs'] = $this->Job_request_model->select_forms_settings_titles(3);
      $data['jobs'] = $this->Job_request_model->select_jobs_settings();
       $data['records'] = $this->Job_request_model->all_before_interview();
      // $this->test($data['records']);
       $data['title'] = 'المقابلات الشخصية';
       $data['subview'] = 'admin/Human_resources/employee_forms/job_requests/applicationsDetails';
       $this->load->view('admin_index', $data);
 
}  
public function work_offers(){//human_resources/employee_forms/Job_requests/Job_request/work_offers
  //  $data['jobs'] = $this->Job_request_model->select_forms_settings_titles(3);
  $data['jobs'] = $this->Job_request_model->select_jobs_settings();
   $data['records'] = $this->Job_request_model->all_after_interview();
  // $this->test($data['records']);
   $data['title'] = ' اضافه عرض العمل';
   $data['subview'] = 'admin/Human_resources/employee_forms/job_requests/work_offers';
   $this->load->view('admin_index', $data);
