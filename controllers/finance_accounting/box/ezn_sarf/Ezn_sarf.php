...
public function print_namozag_ezn($id)
{
      $this->load->model('Model_family_cashing');
    $data['talab_emp_data'] ='';
    $data['ezn_data'] = $this->Ezn_sarf_model->get_ezn_details($id);
       $number = number_format((float)$data['ezn_data']->value, 2, '.', '');
    if (strpos($number,'.') !== false) {
        $val =explode('.',$number);
        $integer =$this->convert_number($val[0]);
        $float =$this->convert_number(round($val[1]));
    ...
    $this->load->view('admin/finance_accounting/box/ezn_sarf/print_namozg_ezn', $data); 
...
//$data['employees'] = $this->Difined_model->select_all("employees","","","","");
 ...
 //https://abnaa-sa.org/finance_accounting/box/ezn_sarf/Ezn_sarf/editeezns/19   
 ...
 public function all_ozonat_sarf(){
    ...
    $data['records'] = $this->Ezn_sarf_model->get_all_ozonat_gari();
    $data['ozonat_suspended'] = $this->Ezn_sarf_model->get_all_ozonat_suspended(4);
    $data['ozonat_refused'] = $this->Ezn_sarf_model->get_all_ozonat_suspended(2);
    $data['title'] = 'إضافة إذن صرف ';
    $data['subview'] = 'admin/finance_accounting/box/ezn_sarf/all_ozonat_sarf';
    $this->load->view('admin_index', $data);  
 }
 
  public function all_ozonat_sarf_transformed(){
    
    $this->load->model('Model_family_cashing');
    $data['records'] = $this->Ezn_sarf_model->get_all_ozonat_transformed();
  ...
  // $data['moder_malis'] = $this->Ezn_sarf_model->get_all_emps_egraat(501); 
     ...
     $data['title'] = 'أذونات الصرف المحولة ';
    
    $data['subview'] = 'admin/finance_accounting/box/ezn_sarf/all_ozonat_sarf_transformed';
    $this->load->view('admin_index', $data);  
 }
...
*/
  ...
  function update_amin_manager()
    ...
    public function morfqat_ezn($id = false)
    {    //finance_accounting/box/ezn_sarf/Ezn_sarf/morfqat_ezn

        $data['ezn_sarf_id_fk'] = $id;
        $data['path'] = "uploads/files/sarf_attaches";
        $data['title'] = 'إضافة إذن صرف ';
        $data['subview'] = 'admin/finance_accounting/box/ezn_sarf/morfqat_ezn';
        $this->load->view('admin_index', $data);
...
//     $this->test($data['result_details']);
    ...
    $data['all_qabd_sheek'] = $this->Ezn_sarf_model->get_all_qabds_sheek();
    $data['all_qabd_naqdi'] = $this->Ezn_sarf_model->get_all_qabds_naqdi();

    $data['all_sarf_naqdi'] = $this->Ezn_sarf_model->get_all_sarf_naqdi();
    $data['all_sarf_sheek'] = $this->Ezn_sarf_model->get_all_sarf_sheek();

    $data['last_id'] = $this->Vouch_sarf_model->getLastId(array('id!='=>0))+1;
    $data['geha_table'] =$this->Vouch_sarf_model->select_gehat();

    $data['title'] = ' سند صرف';
    $data['subview'] = 'admin/finance_accounting/box/ezn_sarf/vouch_sarf';
    $this->load->view('admin_index', $data);

}

public function do_sand_sarf($id)
{
    ...
    $this->Ezn_sarf_model->update_ezn_sarf($inserted_id,$id);
...
print_r($_POST);
   ...
   $this->Ezn_sarf_model->cancel_ezn();
}
