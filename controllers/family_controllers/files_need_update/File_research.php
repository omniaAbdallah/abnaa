        ...
        if (!empty($file)) {
            ...
            foreach ($file as $row) {
                ...
                $arr['data'][] = array(
                    ...
                    $row['contact_mob'],
                    '
      <a onclick="printRequiredFiles_new(' . $row['mother_national_num'] . ');">
                                    xyz
                                </a>           
                ...
                <ul class="dropdown-menu">
                    <li>
                        <ul class="mega-col">
                            ...
                            <li><a target="_blank"
                                   href="' . base_url() . 'family_controllers/Family_letter/Civil_Status/' . $row['mother_national_num'] . '/' . $row['file_num'] . '">خطاب
                                    الاحوال المدنيه </a></li>
                            <li><a target="_blank"
                                   href="' . base_url() . 'family_controllers/Family_letter/Passports/' . $row['mother_national_num'] . '/' . $row['file_num'] . '">خطاب
                                    الجوازات </a></li>
                            <li><a target="_blank"
                                   href="' . base_url() . 'family_controllers/Family_letter/Insurance_letter_father/' . $row['mother_national_num'] . '/' . $row['file_num'] . '">خطاب
                                    تأمينات ( الأب ) </a></li>
                            <li><a target="_blank"
                                   href="' . base_url() . 'family_controllers/Family_letter/Insurance_letter/' . $row['mother_national_num'] . '/' . $row['file_num'] . '">خطاب
                                    تأمينات ( الأم ) </a></li>
                            <li><a target="_blank"
                                   href="' . base_url() . 'family_controllers/Family_letter/Retirement_letter/' . $row['mother_national_num'] . '/' . $row['file_num'] . '">خطاب
                                    التقاعد </a></li>
                            <li><a target="_blank"
                                   href="' . base_url() . 'family_controllers/Family_letter/daman_letter/' . $row['mother_national_num'] . '/' . $row['file_num'] . '">خطاب
                                    الضمان</a></li>
                        </ul>
                        <ul class="mega-col">
							...
							<a target="_blank" href="'.base_url().'family_controllers/Family/talb_mostandat/'.$row['mother_national_num'].'/'. $row['file_num'].'" 
>  اضافه المستندات </a>	
...
////////////////////////////////////////////////////////////////////////////////yara_start///////////////////
    ...
    public function add_file_research($mother_num,$fil_num){ //family_controllers/files_need_update/File_research/add_file_research/1042645620
       

        $this->load->model("familys_models/files_need_update/File_research_attaches");
        $this->load->model("Difined_model");
        $data['mother']=$this->File_research_attaches->get_mother_data()[0];
        $data['mother_num']=$mother_num;
        $data['last_id']=$this->File_research_attaches->select_last_id();
        $data['father']=$this->File_research_attaches->get_father_data()[0];
        $data['f_members']=$this->File_research_attaches->get_f_member();
        $data['wakel']=$this->File_research_attaches->get_wakel_data();
        $data["basic_family"]=$this->File_research_attaches->get_basic_data($mother_num);
        $data['letters']=$this->File_research_attaches->get_letters_by_type();
        $data["main_attach_files"]=$this->Difined_model->select_search_key('family_setting','type',47);
        if($this->input->post('submit'))
        {
         //  $this->test($_POST);
            $this->File_research_attaches->insert_files();
            redirect('family_controllers/files_need_update/File_research/add_file_research/'.$mother_num."/".$fil_num,'refresh');
        }
      
        
        $data['title']="المستندات المطلوبة لاعاده البحث";
     
        $data['subview'] = 'admin/familys_views/files_need_update/add_files_research';
        $this->load->view('admin_index', $data);


    }
    public function update_file_research($mother_num,$num,$file_num){
      //  $this->load->model('familys_models/family_letters_models/family_letter_model');
        ...
        $data['wakel']=$this->File_research_attaches->get_wakel_data();
        $data["main_attach_files"]=$this->Difined_model->select_search_key('family_setting','type',47);
       $data["path_method"]=$mother_num."/".$num."/".$file_num;
        $data["record"]=$this->File_research_attaches->get_RequiredFiles($num);
       // $this->test($data['result']);
        if($this->input->post('submit'))
        {
            $this->File_research_attaches->update_files($num);
            redirect('family_controllers/files_need_update/File_research/add_file_research/'.$mother_num."/".$file_num,'refresh');
        }
    ...
    public function delete_file_research($id,$mother_num,$file_num){
        $this->load->model("familys_models/files_need_update/File_research_attaches");
        $this->File_research_attaches->delete_letter($id);
