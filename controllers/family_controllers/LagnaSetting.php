...
//--------------------------------------------------
    ...
    */
    ...
    //=========================================================
    /// ادارة الإجراءات
    public function procedures_option_lagna()
    { // family_controllers/LagnaSetting/procedures_option_lagna
        $this->load->model('familys_models/Model_lagna_setting');
...
//echo strtotime(date('Y-m-d'));
        ...
        $data['session_num'] = $this->Model_lagna_setting->get_last_session();
        $data['session_num_2'] = $this->Model_lagna_setting->get_last_session_2();
        $data['session_members'] = $this->Model_lagna_setting->get_all_session();
        $data['lagna_member'] = $this->Model_lagna_setting->get_member_lagna(1);
        //$this->test($data['lagna_member']);
        $data['subview'] = 'admin/familys_views/all_lagna_setting/galsa_member';
        $this->load->view('admin_index', $data);
    }
    public function add_selected_member()
    ...
    public function AddLagnaAttach($id)
    {   // family_controllers/LagnaSetting/AddLagnaAttach/
        $this->load->model('familys_models/Model_lagna_setting');
    ...
    public function DeleteLagnaAttach($id)
    {  //  family_controllers/LagnaSetting/DeleteLagnaAttach/
        $this->load->model('familys_models/Model_lagna_setting');
    ...
    public function my_read_file_sarf($file_name)
    {
        $this->load->helper('file');
        $path_iamge = './uploads/family_sarf/';
        $path_files = './uploads/family_sarf/';
        if (file_exists($path_iamge . $file_name)) {
            $main_path = $path_iamge;
        } elseif (file_exists($path_files . $file_name)) {
            $main_path = $path_files;
    ...
    public function transformation_lagna_permissions()
    {   //family_controllers/LagnaSetting/transformation_lagna_permissions
        /********************************************************************/
    ...
    public function transformation_lagna_approved()
    {
        ...
        $id = $_POST['id'];
        $this->Transformation_lagna_model->transformation_lagna_approved($process, $id);
        $this->Model_lagna_setting->insert_history($process, $id);
        echo json_encode($_POST);
    }
    ////=========================================================
    //===========================================================
    //===========================================================
    public function LagnaDesicion()
    {   // family_controllers/LagnaSetting/LagnaDesicion
    ...
    {
        //$this->test($session_id);//44
        $session_id;
        $this->load->model('familys_models/Transformation_lagna_model');
        $this->load->model('familys_models/lagna_model/Lagna_session_model');
        $this->load->model('familys_models/Model_lagna_setting');
        $this->load->model('Difined_model');
        $conditions = array('transformation_lagna.approved' => 1, 'transformation_lagna.approved_lagna' => 0, 'transformation_lagna.session_num_fk' => $session_id, "finished" => 0);
        $data['accepted'] = $this->Transformation_lagna_model->all_procedures2($conditions);
        $data['accepted_bnod'] = $this->Transformation_lagna_model->all_procedures_bnod($conditions);
        //$this->test($data['accepted']);
        $Conditions_arr = array("session_num_fk" => $session_id, "approved_lagna !=" => 0, "finished" => 0);
        $data['all_orders_end'] = $this->Transformation_lagna_model->all_procedures2($Conditions_arr);
        $data['all_orders_end_bnod'] = $this->Transformation_lagna_model->all_procedures_bnod($Conditions_arr);
        //$this->test($data['accepted']);
        ...
        $data['session_data2'] = $this->Model_lagna_setting->get_session_num_transformation_lagna($session_id);
        //$this->test($data['session_data2']);
        $data['title'] = 'جلسات اللجان المفتوحة  ';
    ...
    {
        if ($this->input->post('mother_num')) { //all_procedures
            /************************************************************/
            $this->load->model('familys_models/Transformation_lagna_model');
 ...
 public function AllDetailsLoadpage()
    ...
    public function ShowMemberSession()
    { // family_controllers/LagnaSetting/ShowMemberSession
        $this->load->model('familys_models/Member_session');
        $this->load->model('familys_models/lagna_model/Committee_model');
    ...
    public function details_family_files___()
    {
        $mother_national_num = $this->input->post('mother_num');
        $this->load->model('familys_models/Father');
        $this->load->model('Nationality');
...
//2
    ...
    /*public function insert_db(){
        $this->load->model('familys_models/Model_lagna_setting');
        $data['all_data']=$this->Model_lagna_setting->get_from_trans();
    ...
    public function GetModalEdit()
    {
        ...
        //$this->test( $data["transformation_lagna"]);
        $data['all_lagna_to'] = $this->Difined_model->select_all("lagna", "", "", "", "");
        $data['reasons_arr'] = $this->Difined_model->select_search_key('family_reasons_settings', 'type', $data['transformation_lagna']->transfer_type_id_fk);
        ...
        $data['member_data'] = $this->Family_members->select_all($mother_national_num);
        $data["transformation_lagna_members"] = $this->Model_transformation_process->get_transformation_lagna_members_(array('mother_national_num' => $data['transformation_lagna']->mother_national_num, 'session_num_fk' => $data['transformation_lagna']->session_num_fk, 'procedure_id_fk' => $data['transformation_lagna']->procedure_id_fk));
        $data['members'] = $this->Model_transformation_process->get_session_member(array('mother_national_num' => $data['transformation_lagna']->mother_national_num, 'session_num_fk' => $data['transformation_lagna']->session_num_fk, 'procedure_id_fk' => $data['transformation_lagna']->procedure_id_fk));
        //$this->test($data['members']);
        $data['father'] = $this->Father->get_by_mother($mother_national_num, 'father');
        // $this->test(  $data["procedures_option"]);
        //$this->test(  $data['all_procedures']);
        $this->load->view('admin/familys_views/all_lagna_setting/get_edit_page', $data);
    }
//all_mahader_lagna_arch_ended
    /************************************************************************/
    public function print_mahader_lagna($session_num)
    {
        $this->load->model('familys_models/lagna_model/Committee_model');
        $data['row'] = $this->Committee_model->get_session_data($session_num);
        $data['member_galsa'] = $this->Committee_model->get_member_session($session_num);
        $data['records'] = $this->Committee_model->get_mahader_lagna_data($session_num);
        // $this->test($data['records']);
        $this->load->view('admin/familys_views/all_lagna_setting/print_mahader_lagna', $data);
    }
    public function check_opening_session()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $session_num = $this->Model_lagna_setting->get_last_session();
        $data = $this->Model_lagna_setting->check_opening_session(
            array('mother_national_num' => $_POST['mother_num'],
                'file_num' => $_POST['file_num'],
                'session_num_fk' => $session_num,
                'transfer_type_id_fk' => $_POST['tahweel_type'],
                'procedure_id_fk' => $_POST['twsia'])
        );
        // $data['vacation_end'] = date('m-d-Y', strtotime($date. " +$num days"));
        //tahweel_type
        //twsia
        //mother_num
        //file_num
    ...
    /****************************************************/
    public function Lagna_decisions_approved()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $this->load->model("familys_models/Register");
        $data['file_status'] = $this->Register->get_all_files_status();
        $data['session'] = $this->Model_lagna_setting->get_last_session_closed();
        //  echo '<pre>';
        // print_r( $data['session']);
        $data['records'] = $this->Model_lagna_setting->get_mahader_lagna_data($data['session']->session_number);
        $data['subview'] = 'admin/familys_views/all_lagna_setting/Lagna_decisions_approved';
        $this->load->view('admin_index', $data);
    }
    public function Lagna_decisions_approved_action($table)
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $this->Model_lagna_setting->change_file_setting($table);
        redirect('family_controllers/LagnaSetting/Lagna_decisions_approved', 'refresh');
    }
    /*************************ahmed****************/
    public function GetModalEditBand()
    {
        $this->load->model('Difined_model');
        $this->load->model('familys_models/Father');
        $this->load->model('Model_transformation_process');
        ...
        //$this->test($data["transformation_lagna"]);
        $data["last_lagna_desision"] = $this->Model_transformation_process->get_last_lagna_transformation(
            array('file_num' => $data["transformation_lagna"]->file_num, 'session_num_fk' => $data["transformation_lagna"]->session_num_fk, 'procedure_id_fk' => $data["transformation_lagna"]->procedure_id_fk))[0];
        if (!empty($data["last_lagna_desision"])) {
            $data["all_procedures"] = $this->Difined_model->select_search_key('procedures_option_lagna', 'type', $data["last_lagna_desision"]->transfer_type_id_fk);
        }
        $data['all_lagna_to'] = $this->Difined_model->select_all("lagna", "", "", "", "");
        ...
        $data['procedures_option'] = $this->Difined_model->select_search_key('procedures_option_lagna', 'type', $data['transformation_lagna']->transfer_type_id_fk);
        // $data["transformation_lagna_members"] =$this->Model_transformation_process->get_transformation_lagna_members_(array('mother_national_num'=>$data['transformation_lagna']->mother_national_num,'session_num_fk'=>$data['transformation_lagna']->session_num_fk,'procedure_id_fk'=>$data['transformation_lagna']->procedure_id_fk));
        // $data['members']= $this->Model_transformation_process->get_session_member(array('session_num_fk'=>$data['transformation_lagna']->session_num_fk,'procedure_id_fk'=>$data['transformation_lagna']->procedure_id_fk));
        $this->load->view('admin/familys_views/all_lagna_setting/get_edit_page_band', $data);
    }
    function all_glasat_invation()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $data['all_current_invation'] = $this->Model_lagna_setting->all_current_invation();
        $data['all_glasat_invation'] = $this->Model_lagna_setting->all_glasat_invation();
        $data['title'] = 'الدعوات لحضور جلسة لجنة الاسر';
        $data['subview'] = 'admin/familys_views/all_lagna_setting/all_glasat_invation';
        $this->load->view('admin_index', $data);
    }
    function get_member_session_load()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $session_number = $this->input->post('session_number');
        $data['member_session'] = $this->Model_lagna_setting->get_member_session($session_number);
        $data['subview'] = 'admin/familys_views/all_lagna_setting/member_session_load';
        $this->load->view($data['subview'], $data);
    }
    function update_seen_glasat_invation()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $this->Model_lagna_setting->update_seen_glasat_invation();
    }
    function do_replay()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        ...
        $this->Model_lagna_setting->do_replay($row_id, $replay, $note);
    }
    public function load_galsa_detailes()
    {
        $session_num = $this->input->post('session_num');
        $this->load->model('familys_models/lagna_model/Committee_model');
        $data['row'] = $this->Committee_model->get_session_data($session_num);
        ...
        $data['records'] = $this->Committee_model->get_mahader_lagna_data($session_num);
        // $this->test($data['records']);
        $this->load->view('admin/familys_views/all_lagna_setting/load_galsa_detailes', $data);
    }
    function update_seen_glasat_decision()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $this->Model_lagna_setting->update_seen_glasat_decision();
    }
    function subject_ward()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $data['session_num'] = $this->Model_lagna_setting->get_last_session_open();
        $data['all_orders'] = $this->Model_lagna_setting->GetTransformationLagnaDesicion(0);
//        $this->test($data);
        /**********************************************************/
        $data['title'] = "مواضيع الواردة";
        $data['subview'] = 'admin/familys_views/all_lagna_setting/subject_ward_view';
        $this->load->view('admin_index', $data);
    }
    public function update_deport_subject_ward()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $data = $this->Model_lagna_setting->update_deport_subject_ward();
        echo json_encode($data);
    }
    public function add_picture()
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $id = $this->input->post('session_num');
        $data['title'] = 'إضافة  مرفقات';
        $data['glsa_rkm_fk'] = $this->input->post('session_num');
        $data['folder_path'] = 'uploads/files';
        $data['morfqat'] = $this->Model_lagna_setting->get_table('lagna_files', array('glsa_rkm_fk' => $id));
        $this->load->view('admin/familys_views/all_lagna_setting/add_picture', $data);
    }
    public function load()
    {
        ...
        $data['folder_path'] = 'uploads/files';
        $data['morfqat'] = $this->Model_lagna_setting->get_table('lagna_files', array('glsa_rkm_fk' => $id));
        $this->load->view('admin/familys_views/all_lagna_setting/load', $data);
    }
    ...
    public function upload_img()
    {
        ...
        $this->Model_lagna_setting->insert_attach($id, $images);
    }
    public function delete_upload($id)
    {
        $this->load->model('familys_models/Model_lagna_setting');
        $img = $this->Model_lagna_setting->get_table_by_id('lagna_files', array('id' => $id));
        $path = 'uploads/files';
