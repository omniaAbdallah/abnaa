...
class FamilyCalender extends MY_Controller{
    ...
    public function get_events($where=false)
    {
        ...
        foreach($records as $record) {
            ...
            if($record->visit_from_date == strtotime(date("Y-m-d")) || $record->visit_to_date-(24*60*60) == strtotime(date("Y-m-d")) || 
            ($record->visit_to_date-(24*60*60) >=strtotime(date("Y-m-d")) && $record->visit_from_date <=strtotime(date("Y-m-d")) )) {
                $className = 'label-important';
            }
            $data_events[] = array(
                "id"                => $record->id,
                "title"             => $record->title_setting,
                "end"               => date("Y-m-d",$record->visit_to_date),
                "start"             => date("Y-m-d",$record->visit_from_date),
                "className"         => $className,
    ...
    {
        $id = $this->FamilyCalender_model->update_event();
        $this->get_events(array('family_visitors.id'=>$id));
    }
