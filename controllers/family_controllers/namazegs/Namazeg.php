...
//-----------------------------------------
    ...
    public function index(){     // family_controllers/namazegs/Namazeg
    ...
    public function insert(){
        ...
        redirect('family_controllers/namazegs/Namazeg','refresh');
   ...
   /* public function getNamozegDetails(){
        $data['result'] = $this->Namazeg_model->getAllDetails(array('family_namazeg_letters.letter_rkm'=>$_POST['letter_rkm']));
        $data['details']=$this->Difined_model->getById("family_nmazg_letter_setting",$_POST['id']);
        $data['file_num_data'] = $this->Namazeg_model->getFileNUmData($_POST['file_num']);
           $data['afrad_num'] =   $this->Namazeg_model->getFamilyNum($data['file_num_data']['mother_national_num']);
           $data['mother_num'] =   $this->Namazeg_model->get_mother_status($data['file_num_data']['mother_national_num']);
        $data['family_members_num'] = $data['afrad_num'] +  $data['mother_num'];
          $data['family_members'] = $this->Namazeg_model->family_member($_POST['file_num']);
      ...
      public function getNamozegDetails()
    {
        ...
        $data['family_members_num'] = $data['afrad_num'] + $data['mother_num'];
        $data['family_members'] = $this->Namazeg_model->family_member($_POST['file_num']);
//$this->test($data['family_members']);
        $this->load->view('admin/familys_views/namazeg/namozeg_details_load_page', $data);
    }
    public function getFamilyTable()
    {
        $customer = $this->Namazeg_model->select_family_table(' ');
        $arr = array();
        $arr['data'] = array();
        if (!empty($customer)) {
            $x = 0;
    ...
    public function family_member()
    {
        ...
        $family_mumber_data = $this->Namazeg_model->family_member($file_num);
...
*/
    ...
    public function getFamilyNum(){
       ...
       $data = $this->Namazeg_model->getFileNUmData($_POST['file_num']);
...
<script>
		   Swal.fire({
            ...
            if (result.value) {
                ...
                window.location.href = "'.base_url().'family_controllers/namazegs/Namazeg/printNamazeg/'.$valu.'"   ;
            }
        })
    ...
    public  function  printNamazeg($rkm){
         ...
         $data['result'] = $this->Namazeg_model->getAllDetails(array('family_namazeg_letters.letter_rkm'=>$rkm));
        $data['titles'] = $this->Namazeg_model->GetFromFr_settings(8);
        $data['greetings'] = $this->Namazeg_model->GetFromFr_settings(9);
         $data['family_members_num'] =$this->Namazeg_model->getFamilyNum($data['result']['mother_national_num']);

        //$this->test($data['titles'][$data['result']['start_laqb']]);
          // $data['qued_data'] = $this->Quods_model->data_qued($id);
         $data['title'] = 'طباعة نموذج خطاب';
         $this->load->view('admin/familys_views/namazeg/printNamazeg', $data);
         ...
         /*public function edit($rkm){
             ...
             $data['family_members'] = $this->Namazeg_model->family_member($data['result']['file_num']);
             $data['title']='النماذج';
             $data['subview'] = 'admin/familys_views/namazeg/namazeg';
             $this->load->view('admin_index', $data);
         }*/
    ...
    public function edit($rkm)
    {
        ...
        $data['result'] = $this->Namazeg_model->getAllDetails(array('family_namazeg_letters.letter_rkm' => $rkm));
//                           31-1-om start
        $data['member_letter']= $this->Namazeg_model->get_member_letter(array('leter_rkm' => $rkm));
//                           31-1-om end
        $data['titles'] = $this->AllPills_model->GetFromFr_settings(8);
        $data['greetings'] = $this->AllPills_model->GetFromFr_settings(9);
        $data['family_members_num'] = $this->Namazeg_model->getFamilyNum($data['result']['mother_national_num']);
    ...
    public function getDetailsDiv(){
        ...
        $data['result'] = $this->Namazeg_model->getAllDetails(array('family_namazeg_letters.letter_rkm'=>$_POST['rkm']));
        $data['titles'] = $this->Namazeg_model->GetFromFr_settings(8);
        $data['greetings'] = $this->Namazeg_model->GetFromFr_settings(9);
        $data['family_members_num'] =$this->Namazeg_model->getFamilyNum($data['result']['mother_national_num']);
 //$this->test($data['result']);
        $this->load->view('admin/familys_views/namazeg/getDetailsDiv',$data);
  ...
  /*  public function delete_geha($id){
    	    ...
    	    public function add_morfaq()
    {
       ...
       $this->Namazeg_model->insert_attach($id,$rkm,$images);
    ...
    public function get_attaches()
    {
      ...
      $data['one_data'] = $this->Namazeg_model->get_attach($rkm);
        ...
        $this->load->view('admin/familys_views/namazeg/get_table_attaches',$data);
    }
    ...
    {
        $records = $this->Namazeg_model->select_all();
        $arr = array();
        $arr['data'] = array();
        if (!empty($records)) {
            $x = 0;
            foreach ($records as $row) {
                ...
                $edit_link = '' . base_url() . 'family_controllers/namazegs/Namazeg/edit/' . $row['letter_rkm'] . '';
                $print_link = '' . base_url() . 'family_controllers/namazegs/Namazeg/printNamazeg/' . $row['letter_rkm'] . '';
    ...
    {  
        ...
        $data['letters_type_arr']=$this->Difined_model->select_all("family_nmazg_letter_setting","","","","");
        $data['title']='النماذج';
        $data['subview'] = 'admin/familys_views/namazeg/mos3dat/mosa3da';
        $this->load->view('admin_index', $data);
