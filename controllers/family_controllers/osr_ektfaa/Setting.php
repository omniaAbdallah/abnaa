...
class Setting extends  MY_Controller{
    public function __construct()
    {
        ...
        $this->load->model('familys_models/osr_ektfaa_m/Setting_model');
        $this->myarray = array (
            201=>'  نوع المشروع',
            301=>'  الحالة',
            401=>'    نوع التدريب',
            501=>'بنود  لجنة المقابلات'
    ...
    public function add_setting($type=0){ //family_controllers/osr_ektfaa/Setting/add_setting
        $data['typee']= $type;
        $data['all'] = $this->Setting_model->get_all_data($this->myarray);
       // $this->test($data['all']);
        $data['title'] = "اعدادات  ";
        $data['subview'] = 'admin/familys_views/osr_ektfaa_v/osr_ektfaa_setting/setting_view';
        ...
        if ($this->input->post('add')) {
         //   $this->test($type);
          //  die;
            ...
            redirect('family_controllers/osr_ektfaa/Setting/add_setting/'. $type, 'refresh');
        }
    }
