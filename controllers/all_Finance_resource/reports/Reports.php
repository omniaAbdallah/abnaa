...
class Reports extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('all_Finance_resource_models/reports/Report_kafalat_model');
    ...
    public function get_report_member()
    {
        ...
        if ($type_member == 0 && $fe2a == 0) {
          $data_all['all_armls'] = $this->Report_kafalat_model->get_all_arml();

            $data_all['all_aytam'] = $this->Report_kafalat_model->get_all_aytam();
            $this->load->view('admin/all_Finance_resource_views/reports/load_all_page', $data_all);

        } elseif ($type_member == 1 && $fe2a == 0) {
            $data_all['all_aytam'] = $this->Report_kafalat_model->get_f_member();
            $data_all['all_armls'] = $this->Report_kafalat_model->get_f_mother();
            $this->load->view('admin/all_Finance_resource_views/reports/load_all_page', $data_all);
           ...
           $data_all['all_aytam'] = $this->Report_kafalat_model->get_member_fr_khfalat();
           $data_all['all_armls'] = $this->Report_kafalat_model->get_mother_fr_khfalat();
...
//        $this->load->model('all_Finance_resource_models/reports/Reports_Model');
    ...
    die;*/
     ...
     public function sponsors_report(){ // all_Finance_resource/reports/Reports/sponsors_report

        $data['sponsors'] =   $this->Reports_Model->get_all_sponsors() ;
        $data['title'] = 'تقرير  الكفلاء الغير مربوطين';
        $data['subview'] = 'admin/all_Finance_resource_views/reports/sponsors_report_view';
        $this->load->view('admin_index', $data);
    }  
