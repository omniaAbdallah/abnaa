...
*/
    public function print_kafel_details(){
 ...
 public function getConnection($KafalaType)
  {
         ...
         $arr2 = array();
          ...
          if(!empty($armal)){
              foreach($armal as $row){
                  ...
                  if($KafalaType ==1 || $KafalaType ==4){
                      if($row['first_k_id'] != 0 && $row['first_to'] > strtotime(date('Y-m-d'))){
                          continue;
                  ...
                  $radio ='<input type="radio" name="choosed"  value="'.$row['id'].'" onclick="getMemberData($(this).val(),'.$KafalaType.')" style="width: 20px; height: 20px;" />';
      ...
      }elseif($KafalaType==1 || $KafalaType==3){
              ...
              foreach($members as $row){
                  ...
                  if($KafalaType ==2){
                    ...
                    if($row['first_kafala_type'] !=2){
                      ...
                      if (($row['first_k_id'] != 0   &&  $row['first_to'] > strtotime(date('Y-m-d')) &&
                              $row['second_k_id'] != 0   && $row['second_to'] > strtotime(date('Y-m-d')))
                          ){
...
/***********25/8*********************************************/
                if($row['first_kafala_type'] == 2  && $row['first_halet_kafala'] == 1 ||
                         $row['second_kafala_type']==2 && $row['second_halet_kafala'] == 1){
                ...
                foreach($members as $row){
                    ...
                    if($KafalaType ==2){
                        ...
                        if($row['first_kafala_type'] !=2){
                            ...
                            if($row['first_halet_kafala'] == 1  &&  $row['second_halet_kafala'] == 1){
                                continue;
...
/*
       ...
       public function getMemberData()
    {
      ...
      $data['halet_kafalaat_reasons']= $this->Sponsors_model->get_halet_kafalaat_reasons_settings();
...
*/
    ...
    if(!empty($customer)){
        ...
        foreach($customer as $row){
          ...
          //  $link ='onclick="modal_link('.$row->id.');"';
           $link ='onclick="modal_link_delete('.$row->id.');"';
...
*/
            ...
            $arr['data'][] = array(
                ...
                $row->fe2a_title['title']
            ,
                       '  <button style="    color: #fff;width: 70px;    font-size: 15px;  background-color:'. $row->kafel_status['color'].'  " 
...
<ul class="dropdown-menu">
  ...
  ', '<a type="button"  class="myModal" '.$link2.' data-toggle="modal" style="cursor:pointer"
        ...
        <a   onclick="print_ajax('.$row->id.',\'print_kafel_details\')"
   target="_blank" title="طباعه ملف الكافل" >
    <i class="fa fa-print" aria-hidden="true"></i> </a>
      <a  onclick="print_ajax('.$row->id.',\'print_card\')"
   target="_blank" title="طباعه كارت الكافل" >
    <i style="background-color: #0a568c" class="fa fa-print"
       aria-hidden="true"></i> </a>
       <a type="button" class="" data-toggle="modal"  ' . $link3 . '
    ...
    public function fill_select()
{
   ...
   //   echo 'kafala_type_fk='. $data['result_Kfala_data']['kafala_type_fk'].'<br/>';
  ...
  $data['armal'] = $this->Sponsors_model->getMembersArmal(array('mother.id'=>$data['result_Kfala_data']['person_id_fk']
                ,'mother.categoriey_m'=>1,'mother.person_type'=>62))[0];
               ...
               /* echo 'person_id_fk='. $data['result_Kfala_data']['person_id_fk'];
                echo '<br>';
                echo $type;
                */
...
/*****************************************************************/
 ...
 /*public function print_card_kafala(){
...
/*
      ...
      public function get_kafel_khafalat()
    {
        ...
        $data['records']=$this->Sponsors_model->Getkhafalat($kafel_id);
        $data['year'] =$this->current_hjri_year();
        $data['kafels'] = $this->Sponsors_model->getById($kafel_id)[0];//3-2-om
        $this->load->view('admin/all_Finance_resource_views/sponsors/getkhafel_Kafalat',$data);
    ...
    /*****************************/
     ...
     public  function  getReasons(){
        $data= $this->Sponsors_model->get_halet_kafalaat_reasons_settingsByArray(array('reason_type'=>$_POST['suspension_type']));
    ...
    /*
    public function  all_kafalat_num(){


        $this->load->model('all_Finance_resource_models/sponsors/Sponsors_model_load');

        $data["all_aytam"]= $this->Sponsors_model_load->all_aytam();
        ...
        $data['title'] = '';
        $data['subview'] = 'admin/all_Finance_resource_views/sponsors/all_kafalat_num';
        $this->load->view('admin_index', $data);
        ...
        /*echo "<pre>";
        print_r($data["all_armal_mkfol"]);
        echo "</pre>";
        die;*/
        $data['title'] = '';
        $data['subview'] = 'admin/all_Finance_resource_views/sponsors/kafalat_ehsaey/all_kafalat_num';
        $this->load->view('admin_index', $data);
    ...
    public function GetAllPage(){
        ...
        $this->load->view('admin/all_Finance_resource_views/sponsors/kafalat_ehsaey/GetAllPage', $data_load);
    ...
    public function GetSubPage(){
        ...
        $this->load->model('all_Finance_resource_models/sponsors/Sponsors_model_load');
        $data_load['all_data'] = $this->Sponsors_model_load->GetSubData2($this->input->post('type'),$this->input->post('personType'));

        if($this->input->post('type')==='aytam'){

            $this->load->view('admin/all_Finance_resource_views/sponsors/kafalat_ehsaey/GetSubPageAytam', $data_load);

        }else{

            $this->load->view('admin/all_Finance_resource_views/sponsors/kafalat_ehsaey/GetSubPage', $data_load);
 ...
 public function show_table()
        {
          ...
          $data['table_data']=$this->Sponsors_model->get_data_table($data['input_num'],  $data['type']);
            ...
            $this->load->view('admin/all_Finance_resource_views/sponsors/table_data_load', $data);
        }   
 
 
 
 
   public function load_print_details(){
        ...
        $data["all_kafalat"]= $this->Sponsors_model->get_sponsors_kafalat($kafel_id,$id);
        $data["result"]=$this->Sponsors_model->getById($kafel_id)[0];
       // $data['men_num'] = $this->Sponsors_model->get_main_data('men_num');
       // $data['woman_num'] = $this->Sponsors_model->get_main_data('woman_num');
        $data['kafel_id']= $kafel_id;
        $data['kafala_id']= $id;
        $this->load->view('admin/all_Finance_resource_views/sponsors/load_print_details',$data);
    }
    
    
    public function load_kafala_history(){

        $kafel_id = $this->input->post('kafel_id');
        $mother_num = $this->input->post('mother_num');
        $data['get_all']=$this->Sponsors_model->get_kafala_history($kafel_id,$mother_num);

        $this->load->view('admin/all_Finance_resource_views/sponsors/load_kafala_history',$data);
...
// new function instead of xyz
	public function xyz_datatable()
    {
        /*
        $this->load->model("familys_models/Register");
        $data["relationships"]=$this->Register->select_relashion(array('type'=>34));*/
        ...
        $data['title']="الكفلاء";
        $data['records']= $this->Sponsors_model->select_all();
        $data['customer_js'] = $this->load->view('admin/all_Finance_resource_views/sponsors/app_js_datatable', '', TRUE);

        $this->load->view('admin/all_Finance_resource_views/sponsors/app_datatable', $data);
    }


// new function replace of (data function )
    ...
    function getLists(){
        ...
        //->getRows($_POST);
        $customer= $this->Sponsors_model->getRows($_POST);//->select_all();
                        ...
                        <ul class="dropdown-menu">
                          ...
                          <li><a target="_blank"
                            ...
                            الكفالة </a></li>
                                 ...
                                 <li><a target="_blank"
                            href="'.base_url().'all_Finance_resource/sponsors/Sponsor/kafel_account/'.$row->id.'">
                            بيانات
                            الحساب </a></li>
        ...
        $countrow = $this->Sponsors_model->select_count();
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $countrow,//$this->Sponsors_model->countAll()
 ...
 public function edite_date_kafala()
    {
        $result_id = $_POST['id'];
        if (!empty($result_id)) {
            $data['result_Kfala_data'] = $this->Sponsors_model->getById_main_kafalat_detai($result_id);
            $this->load->view('admin/all_Finance_resource_views/sponsors/edite_date_kafala', $data);
        }
    }
    
    
    
     public function kafel_account($id)
{

    $data['result'] = $this->Sponsors_model->get_kafel_account(0);
   $data['table'] = $this->Sponsors_model->get_kafel_account($id);
   $data['id'] = $id;
    if ($this->input->post('add')) {

        $this->Sponsors_model->insert_account($id);
        $this->message('success', 'تم الاضافة بنجاح');
        redirect('all_Finance_resource/sponsors/Sponsor/kafel_account/' . $id);
    }
 

    $data['title'] = "بيانات حساب الكافل";
    $data['subview'] = 'admin/all_Finance_resource_views/sponsors/kafal_account';
    $this->load->view('admin_index', $data);


}
public function update_kafel_account($id)
{

    $data['table'] = '';
    $data['result'] = $this->Sponsors_model->get_kafel_account($id);
    //$this->test($data['result']);
    $data['id'] = $id;
    if ($this->input->post('add')) {
        $this->Sponsors_model->update_account($id);
        $this->message('success', 'تم التعديل بنجاح');
        redirect('all_Finance_resource/sponsors/Sponsor/kafel_account/' . $id);
    }
    $data['title'] = " تعديل بيانات حساب الكافل";
    $data['subview'] = 'admin/all_Finance_resource_views/sponsors/kafal_account';
    $this->load->view('admin_index', $data);
