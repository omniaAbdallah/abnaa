...
class  Donor extends MY_Controller
{
    ...
    /************************************************************/
    public function addDonor()	// all_Finance_resource/donors/Sponsor/addDonor
    { //Sponsors_model
         $this->load->model("familys_models/Register");
        if($this->input->post('add')){
            $person_img = 'person_img';
            $file= $this->upload_image($person_img);
            $this->Donor_model->insert_maindata('',$file);
            $this->message('success','إضافة  بيانات المتبرع');
            redirect('all_Finance_resource/donors/Donor/addDonor','refresh');
        }
        ...
        $data['last_id'] = $this->Donor_model->select_last_id();
        $data['gender_data'] =$this->Donor_model->GetFromEmployees_settings(1);
        $data['nationality'] =$this->Donor_model->GetFromEmployees_settings(2);
        $data['dest_card'] =$this->Donor_model->GetFromFr_settings(4);
        $data['job_role'] = $this->Donor_model->GetFromFr_settings(2);
        $data['banks'] = $this->Difined_model->select_all('banks_settings','','',"id","asc");
        $data["current_hijry_year"] = $this->current_hjri_year();
        $data['type_card'] =$this->Donor_model->GetFromFr_settings(3);
        $data['employer'] = $this->Donor_model->GetFromGeneral_assembly_settings(1);
        $data['activity']= $this->Donor_model->GetFromFr_settings(5);
        $data['specialize']= $this->Donor_model->GetFromFr_settings(7);
        $data['attach_arr']= json_encode($this->Donor_model->GetFromFr_settings2(10));
        $data['records']= $this->Donor_model->select_all();
         $data['halat_donor'] =  $this->Finance_resource_setting->all_frhk_settings('fr_kafalat_kafel_status',2);
          $data['f2a'] = $this->Difined_model->select_search_key('fr_sponser_donors_setting','type',2);
         
        $data['title'] = 'إضافة  البيانات الأساسية لمتبرع';
        $data['subview'] = 'admin/all_Finance_resource_views/donors/Add_donor';
...
/*
    public function updateDonor($id)
    {
        ...
        if($this->input->post('add')){
            $person_img = 'person_img';
            $file= $this->upload_image($person_img);
            $this->Donor_model->insert_maindata($id,$file);
            $this->message('success','تعديل بيانات المتبرع');
            redirect('all_Finance_resource/donors/Donor/addDonor','refresh');
        }
        $this->load->model('all_Finance_resource_models/settings/Finance_resource_setting');
         $data["relationships"]=$this->Register->select_relashion(array('type'=>34));
        ...
        $data['activity']= $this->Donor_model->GetFromFr_settings(5);
        $data['specialize']= $this->Donor_model->GetFromFr_settings(7);
        $data['result']= $this->Donor_model->getById($id)[0];
       $data['k_status_data'] =$this->Difined_model->select_search_key('fr_kafalat_kafel_status','id',$data['result']->halat_donor_id);
    ...
    */
    ...
    public function updateDonor($id)
    {
        ...
        $data['result']= $this->Donor_model->getById($id)[0];
       $data['k_status_data'] =$this->Difined_model->select_search_key('fr_kafalat_kafel_status','id',$data['result']->halat_donor_id);
      // $data['records']= $this->Donor_model->select_all()[0];
      // $data['emp'] =$this->Difined_model->select_search_key('employees', 'emp_code', $code)[0];
      if(!empty($data['result']->d_city)){
       $data['city_name']= $this->Donor_model->get_city_name($data['result']->d_city);
      }else{
        $data['city_name']= '';
      }
     //  $this->test( $data['city_name']);
     if(!empty(  $data['result']->d_hai)){
       $data['hai_name']= $this->Donor_model->get_city_name($data['result']->d_hai);}else
       {
        $data['hai_name']= '';
       }
          $data['halat_donor'] =  $this->Finance_resource_setting->all_frhk_settings('fr_kafalat_kafel_status',2);
    ...
    public function delete_donor($id)
    {
        ...
        $this->message('success',' تم حذف  متبرع بنجاح');
        redirect('all_Finance_resource/donors/Donor/addDonor','refresh');
    }
    ...
    public function print_card($id){
        ...
        $this->load->view('admin/all_Finance_resource_views/donors/print/print_card',$data);
    }

    public function print_donor_details($id){
        $data["current_hijry_year"] = $this->current_hjri_year();
        $data['gender_data'] =$this->Donor_model->GetFromEmployees_settings(1);
        $data['nationality'] =$this->Donor_model->GetFromEmployees_settings(2);
        $data['dest_card'] =$this->Donor_model->GetFromFr_settings(4);
        $data['cities']= $this->Donor_model->GetFromFr_settings(6);
        $data['job_role'] = $this->Donor_model->GetFromFr_settings(2);
        $data['banks'] = $this->Difined_model->select_all('banks_settings','','',"id","asc");
        $data["current_hijry_year"] = $this->current_hjri_year();
        $data['type_card'] =$this->Donor_model->GetFromFr_settings(3);
        ...
        $data['specialize']= $this->Donor_model->GetFromFr_settings(7);
        $data['result']= $this->Donor_model->getById($id)[0];
        $data['table_images']= $this->Donor_model->getImagesById($id)[0];
        $this->load->view('admin/all_Finance_resource_views/donors/print/print_donor_details', $data);
    }
    ...
    public function add_attach(){
               ...
               if($this->input->post('add')){
                   ...
                   $this->Donor_model->add_attach($all_img);
    ...
    {
        $this->Donor_model->delete_attach($_POST['id']);
        echo json_encode($_POST);

    }
/*********************************************************************************/

 public function complete_donner_data($id)
{
    $data['banks_accounts']= $this->Sponsors_model->select_banks_accounts();
    $data['mybanks']= $this->Sponsors_model->getBanks();
    $data['halat']= $this->Donor_model->GetFromFr_settings(1);

if($this->input->post('save'))
{
    $img=$this->upload_image('mostdem_img');

    $this->Donor_model->insert_tabro3($id,$img);
    redirect('all_Finance_resource/donors/Donor/complete_donner_data/'.$id,'refresh');
}
    $data['row']=$this->Donor_model->get_by_id($id);

    $data['title']="استكمال بيانات التبرع";
...
//    $data['cuttent_higry_year'] = $this->current_hjri_year();
    $data['result']= $this->Donor_model->getById($id);
//    echo  $data['result'][0]->d_name;
    $data['records']=$this->Donor_model->get_all($id);


    $data['subview'] = 'admin/all_Finance_resource_views/donors/complete_donner_data';
    $this->load->view('admin_index', $data);
}
public function update_donner_data($id)
{
    $data['banks_accounts']= $this->Sponsors_model->select_banks_accounts();
    $data['mybanks']= $this->Sponsors_model->getBanks();
    $data['halat']= $this->Donor_model->GetFromFr_settings(1);
    ...
    if($this->input->post('save'))
    {
        ...
        $this->Donor_model->update_tabro3($id,$img);
        redirect('all_Finance_resource/donors/Donor/complete_donner_data/'.$id,'refresh');
    }
    $data['row']=$this->Donor_model->get_by_id($id);
  ...
  //  $data['result'] = $this->Sponsors_model->getMembers(array('f_members.id'=>$this->input->post('id')))[0];
    $data['banks'] = $this->Difined_model->select_all('banks_settings','','',"id","asc");
    $data['result']= $this->Donor_model->getById($data['row']->donor_id_fk);
    $data['accounts']=$this->Donor_model->get_all_accounts();
...
public function delete_rec($row,$id)
{
    $this->Donor_model->delete_rec($row);
