...
class Esal_order extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('all_Finance_resource_models/nmazg/esal_orders/Esal_order_model');
    ...
    public function add_esal_orders()
    {    // all_Finance_resource/nmazg/esal_orders/Esal_order/add_esal_orders


        $data['projects'] = $this->Esal_order_model->get_projects();
        $data['edara'] = $this->Esal_order_model->select_by();

        if ($this->input->post('save') != '') {

            $file = $this->upload_file("morfaq", "files/fr/esal_order");
            ...
            $id = base64_decode($id);
            $last_id_print = $this->Esal_order_model->insert($id, $file);
            ...
            if ($this->input->post('save') === 'save') {
                redirect('all_Finance_resource/nmazg/esal_orders/Esal_order/add_esal_orders', 'refresh');
            } elseif ($this->input->post('save') === 'print_pill') {
                redirect('all_Finance_resource/nmazg/esal_orders/Esal_order/Print_Pill2/' . $IIID, 'refresh');
            } elseif ($this->input->post('save') === 'print_kafel') {
                redirect('all_Finance_resource/nmazg/esal_orders/Esal_order/Print_Pill2/' . $IIID . '?type=kafel', 'refresh');
            }
            ...
            if (!empty($id)) {
                ...
                $data['result'] = $this->Esal_order_model->select_all_by_fr_all_pills(array('id' => $id))[0];
                ...
                if (!empty($data['result'])) {
                    ...
                    $data['erad_tbro3_arr'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
                        array('fe2a' => 0, 'band' => 0, 'erad_tabro3' => 0, 'esal' => $data['result']->order_type));

                    $data['fe2a_type_arr'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
                        array('fe2a' => 0, 'band' => 0, 'erad_tabro3' => $data['result']->erad_type));

                    $data['bnd_type_arr'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
                        array('band' => 0, 'fe2a' => $data['result']->fe2a_type1));

                    $data['fe2a_type_arr2'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
                        array('fe2a' => 0, 'band' => 0, 'erad_tabro3' => $data['result']->erad_type));

                    $data['bnd_type_arr2'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
                        array('band' => 0, 'fe2a' => $data['result']->fe2a_type2));


                    $data['bank_accounts_arr'] = $this->Esal_order_model->select_all_by_condition(
                        array('society_main_banks_account.bank_id_fk' => $data['result']->bank_id_fk), '');

                    $data['eda3_data'] = $this->Esal_order_model->select_all_by_condition(
                    ...
                    $data['all_data'] = $this->Esal_order_model->select_all_by_DeviceData(array('fr_devices_points.device_id_fk' => $data['result']->device_num), 'bank_id_fk');
                    $data['bank_account_code_shabka_arr'] = $this->Esal_order_model->select_all_by_condition(
                    ...
                    $data['emp_data']=$this->Esal_order_model->get_emps(array( 'administration'=>$data['result']->edara_id_fk));

                }

                if ($_SESSION['role_id_fk'] == 3) {
                    $data['gathering_emp_id'] = $this->Esal_order_model->slect_where('fr_gathering_place', array('emp_id_fk' => $_SESSION['emp_code']))['gathering_place_id_fk'];
                    $data['raqm_deveice_emp'] = $this->Esal_order_model->slect_where('fr_devices_points_emp', array('emp_id' => $_SESSION['emp_code']));
                    if (!empty($data['raqm_deveice_emp']['device_id_fk'])) {

                        $data['shabka_banks_data'] = $this->Esal_order_model->select_all_by_DeviceData(
            ...
            $data['bank_brach'] = $this->Esal_order_model->GetFromFr_settings(16);

            $data['username'] = $this->Esal_order_model->getUserName($_SESSION['user_id']);
            $data['all_banks'] = $this->Esal_order_model->select_all_by_condition(array("society_main_banks_account.type" => 2), "society_main_banks_account.bank_id_fk");
            ...
            $data['gathering_place'] = $this->Esal_order_model->GetFromFr_settings(14);
            ...
            $data['titles'] = $this->Esal_order_model->GetFromFr_settings(8);
            $data['people_arr'] = $this->Esal_order_model->select_all_by_fr_all_pills(array('person_type' => 0));
            $data['last_id'] = $this->Esal_order_model->select_last_id();
            $data['greetings'] = $this->Esal_order_model->GetFromFr_settings(9);

            $data['devices_points'] = $this->Esal_order_model->select_all_devices_points();
            $data['esal_type'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(array('esal' => 0));
            $data['all_pills_today'] = $this->Esal_order_model->select_all_by_fr_all_pills_all();
//$this->test( $data['all_pills_today']);

            $data['title'] = 'إضافة طلب ';
            $data['subview'] = 'admin/all_Finance_resource_views/nmazg/esal_orders/esal_order_view';
            $this->load->view('admin_index', $data);
        }
    }


    public function make_esal($id)
    {    // all_Finance_resource/nmazg/esal_orders/Esal_order/make_esal
...
//devices_points
        if ($this->input->post('save') != '') {
            ...
            $last_id_print = $this->Esal_order_model->insert_esal();
            $this->message('success', 'إضافة  بيانات الإيصال');
...
//            $id = base64_decode($id);
            ...
            $data['title'] = 'عمل ايصال  ';
            $data['subview'] = 'admin/all_Finance_resource_views/nmazg/esal_orders/make_esal_view';
            $this->load->view('admin_index', $data);
        }
    }
    ...
    public function get_emps()
    {
        ...
        $edara = $this->input->post('edara_id');
       $con=array( 'administration'=>$edara);
        $data['emps'] = $this->Esal_order_model->get_emps($con);
        echo json_encode($data['emps']);
    ...
    public function getConnection($Fe2aType)
    {
        ...
        /********* خاص بالكفلاء *********************************************/
        if ($Fe2aType == 1) {
            $all_Sponsors = $this->Esal_order_model->getMembersSponsors();
            $arr_sponsors = array();
            ...
            $all_Donors = $this->Esal_order_model->getMembersDonors();
            $arr_donors = array();
            ...
            $all_general_assembly = $this->Esal_order_model->getMembersGeneral_assembly();
                    ...
                    $arr_general_assembly['data'][] = array(
                        '<input type="radio" name="choosed" value="' . $row_general_assembly['id'] . '"
                          ondblclick="GetMemberName(' . $row_general_assembly['id'] . ')"   id="member' . $row_general_assembly['id'] . '" data-name="' . $row_general_assembly['name'] . '" data-id="' . $row_general_assembly['id'] . '"
                                data-mob="' . $row_general_assembly['mob'] . '"  />',
    ...
    public function GetByArray()
    {
        ...
        if ($_POST['type'] === 'getAccount') {
            $data = $this->Esal_order_model->select_all_by_condition(array('society_main_banks_account.bank_id_fk' => $_POST['id']), '');
        } elseif ($_POST['type'] === 'getAccountNum') {
            $data = $this->Esal_order_model->select_all_by_condition(
    ...
    public function GetData()
    {
        ...
        if ($_POST['type'] === 'tabro3') {
            $data = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
    ...
    public function Print_Pill2($id)
    {
        if (!empty($id)) {
            ...
            $type_esal = $this->Esal_order_model->get_type_print($_SESSION['emp_code']);
            if (!empty($type_esal)) {
                if ($type_esal == 1) {
//                $this->test($_SESSION);
                    $this->load->view('admin/all_Finance_resource_views/all_pills/PrintPill', $data);
    ...
    {
        $bands = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(array('band' => 0, 'type' => 4));
    ...
    public function get_search_pills()
    {
        ...
        $data['details'] = $this->Esal_order_model->get_all_pill_search($field, $valu);
...
//                $this->test($_SESSION);
    ...
    public function GetDetails()
    {
        ...
        $data['all_banks'] = $this->Esal_order_model->select_all_by_condition(array("society_main_banks_account.type" => 2)
        ...
        $data['emp_data']=$this->Esal_order_model->get_emps(array( 'emp_code'=>$data['result']->emp_code_fk));
        $data['edara_data']=$this->Esal_order_model->edara_data(array( 'id'=>$data['result']->edara_id_fk));
        $data['project_data']=$this->Esal_order_model->project_data(array( 'id'=>$data['result']->moshro3_fk));
        $this->load->view('admin/all_Finance_resource_views/nmazg/esal_orders/GetDetails', $data);
    ...
    public function deletePillImg($id, $link)
    {
        ...
        $this->Esal_order_model->delete_attaches($id);
        redirect('all_Finance_resource/nmazg/esal_orders/Esal_order/add_esal_orders', 'refresh');
    }
    ...
    public function getSearchResults()
    {
        ...
        $data['records'] = $this->Esal_order_model->getSearchResults($array_search_colum, $input_search_value);
//        $this->test( count($data['records']));
    ...
    {
        $id = base64_decode($id);
        $this->Esal_order_model->DeletePill($id);
    ...
    public function GetDeviceData()
    {
        if ($_POST['type'] === 'all') {
            ...
            $data['all_data'] = $this->Esal_order_model->select_all_by_DeviceData(array('fr_devices_points.device_id_fk' => $_POST['id']), 'bank_id_fk');

            $data['account_code'] = $this->Esal_order_model->select_all_by_condition(
    ...
    public function GetData2()
    {
        ...
        $data['fe2a_type2_arr'] = $this->Esal_order_model->select_fr_bnod_pills_setting_by_condition(
...
//        $file_name=$this->uri->segment(5);
        $file_path = 'uploads/files/fr/esal_order/'.$file_name;
    ...
    public function GetBandType2()
    {
     ...
     public function get_pills() //all_Finance_resource/all_pills/AllPills/get_pills
    {
        $data['all_pills'] = $this->Esal_order_model->get_pills();

        $this->load->view('admin/all_Finance_resource_views/nmazg/esal_orders/get_all_esalat',$data);
