<?php
class Membering extends MY_Controller{
    ...
    public function add_member(){ // public_relations/website/pr_membering/Membering/add_member

        if ($this->input->post('ADD')){
            $this->Membering_model->insert();
            $this->message('success','تم الاضافة بنجاح');
            redirect('public_relations/website/pr_membering/Membering/add_member','refresh');
        }
        $data['membring']=$this->Membering_model->display_membring();

        $data['title']= 'اضافة العضوية للجمعية';
        $data['subview'] = 'admin/public_relations/website/pr_membering/membring_view';
        $this->load->view('admin_index', $data);

    }

    public function Update($id){
    $data['get_member']= $this->Membering_model->get_by_id($id);

    if ($this->input->post('ADD')){
        $this->Membering_model->update($id);
    ...
    public function Delete($id){ // public_relations/website/pr_membering/Membering/Delete

        $this->Membering_model->delete($id);
