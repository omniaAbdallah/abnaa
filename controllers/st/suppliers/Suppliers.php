...
class Suppliers extends MY_Controller{
        ...
        if(!empty($suppliers)){
            ...
            foreach ($suppliers as $row){
                ...
                $arr['data'][] = array(
                    ...
                    $method_name,
                    '
                    <a data-toggle="modal" data-target="#detailsModal" class="btn btn-info btn-xs" style="padding: 1px 5px;" onclick="load_page('.$row->id.');" title="تفاصيل">
    ...
    public function Print_supplier(){ // st/suppliers/Suppliers/Print_supplier
        $data['title']="طباعة الموردين" ;
        $row_id = $this->input->post('row_id');
        $data['get_all']=$this->Suppliers_model->get_by_id($row_id)[0];
        $data['tele_other'] = unserialize($data['get_all']->tele_other);
        $this->load->view('admin/st/suppliers/print_supplier', $data);
