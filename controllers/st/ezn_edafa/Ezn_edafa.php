...
//            $this->test($_POST);
        ...
        if ($this->input->post('save')) {
            $data['last_id'] = $this->Ezn_edafa_model->insert_tabr3();
            $ezn_id = base64_encode($data['last_id']);
            redirect('st/ezn_edafa/Ezn_edafa/print_ezn_edafa/' . $ezn_id, 'refresh');
        }
...
//                        $this->test($_POST);
        ...
        $data['fe2a_type_arr'] = $this->Ezn_edafa_model->select_st_bnod_setting_by_condition(
            array('fe2a' => 0, 'band' => 0, 'erad_tabro3' => $data['one_data']['no3_tabro3_from_id']));

        $data['bnd_type_arr'] = $this->Ezn_edafa_model->select_st_bnod_setting_by_condition(
            array('type' => 4, 'fe2a' => $data['one_data']['fe2a_from_id']));

        $data['bnd_type_arr2'] = $this->Ezn_edafa_model->select_st_bnod_setting_by_condition(
            array('fe2a' => $data['one_data']['fe2a_from_id2']));
        $data['bnd_type_arr3'] = $this->Ezn_edafa_model->select_st_bnod_setting_by_condition(
            array('fe2a' => $data['one_data']['fe2a_from_id3']));
        $data['bnd_type_arr4'] = $this->Ezn_edafa_model->select_st_bnod_setting_by_condition(
            array('fe2a' => $data['one_data']['fe2a_from_id4']));



//        $this->test($data);
        $data['title'] = '  تعديل إذن الإضافة  ';
        $data['subview'] = 'admin/st/ezn_edafa/ezn_edafa_view';
        $this->load->view('admin_index', $data);
    }
...
//        $this->test($data['one_data']);
   ...
   public function get_detailes()
    {
        $data['one_data'] = $this->Ezn_edafa_model->get_one_tabr3($this->input->post('id'));
        $this->load->view('admin/st/ezn_edafa/load_details_tbr3ayni_view', $data);
 ...
 public function add_attach()
    {
        ...
        if ($this->input->post('save')){
        $images = $this->upload_muli_file("file");
        $this->Ezn_edafa_model->insert_attach( $data['id'], $images);
        //  $this->messages('success','تمت إضافة المرفقات');
        messages('success', 'تم اضافة المرفقات ');
        redirect('st/ezn_edafa/Ezn_edafa', 'refresh');
        }
       $data['get_supplier']=$this->Ezn_edafa_model->get_by_id_morfq($data['id'])[0];
    
     //   $data['result'] = $this->Ezn_edafa_model->select_all_by_st_esalat_estlam(array('id' => $id))[0];
        $this->load->view('admin/st/ezn_edafa/ezn_edafa_morfaq_load', $data);
    ...
    { // st/ezn_edafa/Ezn_edafa/get_code
        $id = $this->input->post('id');
        $code = $this->Ezn_edafa_model->get_code($id);
        $json = json_encode($code);
        echo $json;
    ...
    public function get_tabr3()
    {  // st/ezn_edafa/Ezn_edafa/get_tabr3
    ...
    {  // st/ezn_edafa/Ezn_edafa/get_storage
  ...
  /*  public function add_esal_ezn($id)
    {
        ...
        $result  = $this->Esalat_estlam_model->select_all_by_st_esalat_estlam(array('id' => $id))[0];

        $data['result'] = $this->Ezn_edafa_model->get_esal_data($result);
        $data['last_id'] = $this->Ezn_edafa_model->select_last_id();
        $id = base64_encode($data['last_id']);

           if ($this->input->post('save')) {
            $data['last_id'] = $this->Ezn_edafa_model->insert_tabr3();
            $this->Ezn_edafa_model->update_esal($id);
...
//        $this->test($arr);
    ...
    public function GetData2()
    {
        ...
        $data['fe2a_type2_arr'] = $this->Ezn_edafa_model->select_st_bnod_setting_by_condition(
    ...
    /*
    public function print_ezn_edafa()
    {
        $data['title'] = "طباعة إذن الاضافة ";
        $row_id = $this->input->post('row_id');
        $data['get_all'] = $this->Ezn_edafa_model->get_by_id($row_id)[0];
        $data['UserName'] = $this->Ezn_edafa_model->getUserName($_SESSION['user_id']);
        $this->load->view('admin/st/ezn_edafa/ezn_edafa_print_view', $data);

    }
    */
    public function print_ezn_edafa($row_id = false)
    {
        $data['title'] = "طباعة إذن الاضافة ";
       ...
       public function all_ozonat_edafa_deported()
    {

        $data['all_data'] = $this->Ezn_edafa_model->get_all_tabr3_finance_deported();
       
        $data['title'] = 'أذونات الإضافة المرحلة ';
        $data['subview'] = 'admin/st/ezn_edafa/all_ozonat_edafa_deported_view';
        $this->load->view('admin_index', $data);
    }
