 ...
 */    
 public function add_level(){
  ...
  //--------------------------------------
   public function DeleteLevel($code){
     ...
     $this->Difined_model->delete("accounts_group",array("id"=>$code));
 ...
 public function select_code(){
  ...
  public function view_daled(){
       $this->load->model('finance_accounting_model/Reports');

        $data['records'] = $this->Reports->tree();    

            $data['title']='الدليل الحسابى';
        $data['subview'] = 'admin/finance_accounting/add_levels/view_dalel';
     $this->load->view('admin_index', $data); 
     }
 ...
 */  
 public function sand_sarf(){
    ...
    $data['select_max_id']=55;
     ...
     $data['disabls'] = $this->Add_level->select_disabled();
        $data['title']="اضاافه سند صرف";
...
/*************************************************/
 ...
 public function edit_sand_sarf($vouchnum,$type){
   $this->load->model('Difined_model');
        if($this->input->post('id'))
        {
            $this->Difined_model->delete("vouchers",array("id"=>$this->input->post('id')));
          return;
        }
        ...
        $data["accout_groups"]=$this->Add_level->select_all();
        $data['disabls'] = $this->Add_level->select_disabled();
        $data['all_vouchers']=$this->Sarf->getById($vouchnum);
        
        
        $data['title']='تعديل سند الصرف';
 ...
 */    
 public function sand_qabd(){
    ...
    if($this->input->post("ADD") =="ADD"){
     ...
     $data['disabls'] = $this->Add_level->select_disabled();
     $data['title']="اضاافه سند قبض";

     $data['subview'] = 'admin/finance_accounting/qabd/sanad_qabd';
        $this->load->view('admin_index', $data); 
 } 
    
    public function get_all_sand_qabd()
    {
        $data['title']="جميع سندات القبض";
        $this->load->model("finance_accounting_model/Qabd");
         $data['records']=$this->Qabd->get_all();
        $data['subview'] = 'admin/finance_accounting/qabd/all_sand';
        $this->load->view('admin_index', $data);
...
public function R_accounts_group_details_period(){
 ...
 //===================================ahmed zidan===================================//
    ...
    public   function all_sand_qabd(){
        $this->load->model("finance_accounting_model/Qabd");
        $data['records']=$this->Qabd->query_vouchers_qabd();

        $data['title']='تعديل سند القبض';
        $data['subview'] = 'admin/finance_accounting/qabd/all_sand_qabd';
        $this->load->view('admin_index', $data);
    }
    /*

    public function sand_qabd_edit($num)
    {
        $this->load->model("finance_accounting_model/Qabd");
        $data['boxs']=$this->Settings->get_form_id($this->Settings->setting_id(3));
        $data['banks']=$this->Settings->get_form_id($this->Settings->setting_id(4));
        ...
        $data['sand_item']=$this->Qabd->get_sand_item($num);

       

        $data['disabls'] = $this->Add_level->select_disabled();

        $data['subview'] = 'admin/finance_accounting/qabd/edit_sanad_qabd';
        $this->load->view('admin_index', $data);
    }
public function delete_sand($id)
{
    $this->load->model("Difined_model");
    $this->Difined_model->delete('vouchers',array('id'=>$id));
    redirect('admin/Finance_accounting/all_sand_qabd');
}

    public   function all_sand_qabd(){
        $this->load->model("finance_accounting_model/Qabd");
        $data['records']=$this->Sarf->query_vouchers_sarf();
        $data['title']='تعديل سند صرف';
        $data['subview'] = 'admin/finance_accounting/sarf/all_sand_sarf';
        $this->load->view('admin_index', $data);
    }
   */
    ...
    public   function sand_qabd_edit($vouchnum){
        $this->load->model('Difined_model');
        if($this->input->post('id'))
        {
            $this->Difined_model->delete("vouchers",array("id"=>$this->input->post('id')));
        ...
        $data["accout_groups"]=$this->Add_level->select_all();
        $data['disabls'] = $this->Add_level->select_disabled();
        $data['all_vouchers']=$this->Qabd->getById($vouchnum);
        
        
        $data['title']='تعديل سند قبض';
        $data['subview'] = 'admin/finance_accounting/qabd/edit_sanad_qabd';
        $this->load->view('admin_index', $data);
    }
    ...
    //===================================ahmed zidan===================================//
    public   function all_sand_sarf(){
        $this->load->model("finance_accounting_model/Sarf");
        $data['records']=$this->Sarf->query_vouchers_qabd();
    ...
    public function delete_sand_sarf($id,$num){
        $this->load->model('Difined_model');
        $this->Difined_model->delete("vouchers",array("id"=>$id));
        $this->message('success','تم الحذف بنجاح');
        redirect('admin/Finance_accounting/sand_sarf_edit/'.$num, 'refresh');
    }

    public   function sand_sarf_edit($vouchnum){
        $this->load->model("finance_accounting_model/Sarf");
        $data['boxs']=$this->Settings->get_form_id($this->Settings->setting_id(3));
        $data['banks']=$this->Settings->get_form_id($this->Settings->setting_id(4));
        $data["accout_groups"]=$this->Add_level->select_all();
    ...
    public function edit_sanad_item()
    {
         $this->load->model("finance_accounting_model/Qabd");
      $this->Qabd->edit_qabd();
     ...
     public function edit_sand_sarf_item()
    {
         $this->load->model("finance_accounting_model/Sarf");
          $this->Sarf->edit_sarf();
...
{
    $this->load->model("finance_accounting_model/Sarf");
    $this->Sarf->sand_sarf_delete($id);
    redirect('admin/finance_accounting/all_sand_sarf');

}
/*********************************************************/

    public   function all_settings(){  //   admin/Finance_accounting/all_settings

            $this->load->model("finance_accounting_model/Settings");
            $data['records']=$this->Settings->select_all_settings();
            //$this->test($data['records']);

            $data['title']='الاعدادات';
            $data['subview'] = 'admin/finance_accounting/settings/all_settings';
            $this->load->view('admin_index', $data);
        }
    ...
    public   function update_settings($id){
        ...
        $this->Settings->updateSettingCode($id);
    //   var_dump($_POST);
        redirect('admin/Finance_accounting/all_settings');
    }



public function get_sand_between_peroid()
    {
       
       
        $data['subview'] = 'admin/finance_accounting/reports/R_sands_types';
        $this->load->view('admin_index', $data);
        ...
        public function Vouchers_Peroid(){ 

     $this->load->model('finance_accounting_model/Reports');

    if($this->input->post("date_from") && $this->input->post("date_to") 
                  && $this->input->post("sanad_id")  ){
        ...
        $sanad_id =$this->input->post("sanad_id");
         ...
         $data_load['data_table']=$this->Reports->select_between_dates($from,$to,$sanad_id);
        $this->load->view('admin/finance_accounting/reports/get_sanad_types',$data_load);
    }else{
      
        $data['title'] = 'تقرير السندات ';
        $data['metakeyword'] = 'تقرير السندات ';
        $data['metadiscription'] = 'تقرير السندات  ';
        $data['subview'] = 'admin/finance_accounting/reports/R_sands_types';
        $this->load->view('admin_index', $data);
    }
}  

    public function ReprtQuedByTime(){ // Reports/ReprtQuedByTime     
  ...
  $this->load->model('finance_accounting_model/Reports');  
         ...
         $data_load['from']= $from;  
         $data_load['to']=$to;
		 $data_load['tables']=$this->Reports->select_qued(array("date <="=>strtotime($to) ,"date >="=>strtotime($from) ));
         $this->load->view('admin/finance_accounting/qued/get_R_qued_by_time',$data_load);
	   }else{
		$data['subview'] = 'admin/finance_accounting/qued/R_qued_by_time';
        $data['title'] = 'تقرير القيود خلال فترة ';
        $this->load->view('admin_index', $data);   
	   }    
    }
