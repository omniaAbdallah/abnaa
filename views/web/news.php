                   ...
                   <div class="col-xs-12 col-sm-6 col-md-4">
                    <article class="post clearfix mb-sm-30 bg-silver-light">
                        <div class="entry-header">
                           ...
                           <?php if(isset($news->photos)){
                            $src='"'.base_url().'"asisst/web_asset/img/news/1.jpg"';
                           }else{
                            $src ='"'.base_url().'uploads/images/'.$news->photos[0]->img.'"';
                           } ?>
                        ...
                        <div class="entry-content p-20 pr-10">
                            <div class="entry-meta media">
                                <div class="entry-date media-left text-center flip bg-theme-colored ">
                                    <ul>
                                        <li class=""><?php echo date('d',$news->date);?></li>
                                        <li class=""><?php echo $ar_months[date('n',$news->date)]; ?></li>
                                    </ul>
                                </div>
                                <div class="media-body">
                                    <div class="event-content pull-left flip">
                                        <h4 class="entry-title text-white text-uppercase"><a href="<?php echo base_url().'Web/news_details/'.$news->id ?>"> <?php  echo $news->title?></a></h4>
                                    </div>
                                </div>
                            </div>
                            <p class="mt-10"><?php echo character_limiter($news->content,40) ; ?></p>
                            <a href="<?php echo base_url().'Web/news_details/'.$news->id ?>" class="btn btn-default btn-sm btn-theme-colored mt-10">قراءة الخبر</a>
                        </div>
                    </article>
                </div>
              <?php 
