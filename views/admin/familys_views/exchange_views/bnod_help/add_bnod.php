            ...
            <table id="example" class=" display table table-bordered   responsive nowrap" cellspacing="0" width="100%">
                ...
                foreach($records as $row){?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#hesabatEdit<?=$row->id?>">
                                اضافة رقم حساب
                            </button>
                        </td>
...
<div class="modal fade " id="hesabatEdit<?=$row->id?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-success modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">اضافة حساب</h5>

            </div>
            <div class="modal-body">
                <div class="panel-body">
               <?php echo form_open_multipart('family_controllers/Exchange/addBnodHelpHesab/'.$row->id); ?>
                <div class="col-sm-12 ">
                <?php

                if( $row->id == 1){ ?>
                    <input type="hidden" value="1" name="type_rqm">
                    <div class="col_md_100 col-sm-12 padding-4">
                        <div class="col-md-4 col-sm-6 no-padding">
                            <h6 class="label_title_2">الارامل  </h6>
                        </div>
                        <div class="col-md-3 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_armal?>" class="form-control input_style_2 testButton" name="rkm_hesab_armal" id="account_num<?=$row->id.'1'?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$row->id.'1'?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getVouchQbdAccountName($(this).val(),<?=$row->id?>);">
                        </div>
                        <div class="col-md-5 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_armal_name?>" class="form-control input_style_2" name="rkm_hesab_armal_name" id="account<?=$row->id.'1'?>" data-validation="required" aria-required="true" readonly="" >
                        </div>
                    </div>

                    <div class="col_md_100 col-sm-12 padding-4">
                        <div class="col-md-4 col-sm-6 no-padding">
                            <h6 class="label_title_2">الايتام  </h6>
                        </div>
                        <div class="col-md-3 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_yatem?>" class="form-control input_style_2 testButton" name="rkm_hesab_yatem" id="account_num<?=$row->id.'2'?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$row->id.'2'?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getVouchQbdAccountName($(this).val(),<?=$row->id?>);">
                        </div>
                        <div class="col-md-5 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_yatem_name?>" class="form-control input_style_2" name="rkm_hesab_yatem_name" id="account<?=$row->id.'2'?>" data-validation="required" aria-required="true" readonly="" >
                        </div>
                    </div>
                    ...
                    <div class="col_md_100 col-sm-12 padding-4">
                        <div class="col-md-4 col-sm-6 no-padding">
                            <h6 class="label_title_2">المستفيدين  </h6>
                        </div>
                        <div class="col-md-3 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_mostafed?>" class="form-control input_style_2 testButton" name="rkm_hesab_mostafed" id="account_num<?=$row->id.'3'?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$row->id.'3'?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getVouchQbdAccountName($(this).val(),<?=$row->id?>);">
                        </div>
                        <div class="col-md-5 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_mostafed_name?>" class="form-control input_style_2" name="rkm_hesab_mostafed_name" id="account<?=$row->id.'3'?>" data-validation="required" aria-required="true" readonly="" >
                        </div>
                    </div>
                ...
                <?php }else{
                ?>
                    <input type="hidden" value="2" name="type_rqm">
                    <div class="col_md_100 col-sm-12 padding-4">
                        <div class="col-md-4 col-sm-6 no-padding">
                            <h6 class="label_title_2">اسم الحساب  </h6>
                        </div>
                        <div class="col-md-3 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab?>" class="form-control input_style_2 testButton" name="rkm_hesab" id="account_num<?=$row->id?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$row->id?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getVouchQbdAccountName($(this).val(),<?=$row->id?>);">
                        </div>
                        <div class="col-md-5 col-sm-6 no-padding">
                            <input type="text" value="<?=$row->rkm_hesab_name?>" class="form-control input_style_2" name="rkm_hesab_name" id="account<?=$row->id?>" data-validation="required" aria-required="true" readonly="" >
                        </div>
                    </div>
                     <?php  } ?>
                </div>
                ...
                <button type="submit" class="btn btn-labeled btn-success pull-right" name="save" value="save" id="myBtn">
