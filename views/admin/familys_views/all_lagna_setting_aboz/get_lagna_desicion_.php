...
<?php if(isset($all_orders)&&!empty($all_orders)){?>
    <div class="col-sm-9 col-md-9 col-xs-9" style="padding-top: 0;">
        <div class="top-line"></div>
        <div class="panel panel-bd lobidrag" style="padding-top: 0;">
            <div class="panel-heading" style="">
                <h6 style="margin: 0">بنود الجلسة</h6>
            </div>
            <div class="panel-body">
                <br>
                <div class="form-group col-sm-6">
                    <label class="label label-green  half">رقم اللجنة</label>
                    <input type="text" class="form-control half input-style" placeholder="" data-validation="required" readonly="readonly"
                           name="" value="<?php echo $session_member[0]->lagna_number;?>">
                </div>
                <div class="form-group col-sm-6">
                    <label class="label label-green  half"> رقم الجلسة </label>
                    <input type="text" class="form-control half input-style" placeholder="" readonly="readonly" data-validation="required" id="glsa_num" value="<?php echo $session_member[0]->session_number;?>">
                </div>
                ...
                <?php $m=0; foreach($all_orders as $row) {
                    $id=$row->id;
                                            ...
                                            <?php  }else { ?>
                                                ...
                                                <div class="form-group col-sm-4 col-xs-12">
                                                    <label class="label label-green  half">صلة القرابة<strong
                                                            class="astric">*</strong><strong></strong> </label>
                                                    <select name="m_relationship" id="m_relationship"  class="form-control half">
                                                        <option value="">إختر</option>
    ...
    <div class="col-sm-3 col-md-3 col-xs-3">
        <div class="panel panel-bd lobidrag" style="padding-top: 0;">
            <div class="panel-heading" style="">
                <h6 style="margin: 0">الأعضاء الحاضرون</h6>
            </div>
            <div class="panel-body">
                <ol>
                    <?php
            ...
            <div class="panel-footer">
                <span class="label label-info" style="font-size: 13px"> <strong>إجمالى الحاضرون :<?php echo count($session_member);?></strong></span>
            </div>
        </div>
    </div>
<?php } else{?>
...
<div class="col-sm-12 col-md-12 col-xs-12">
    <table id="" class=" display table table-bordered   responsive nowrap" cellspacing="0" width="100%">
        ...
        $x=1;
        $arr=array(1=>"تم قبول الطلب",2=>" رفض الطلب",0=>"لم يتم النظر ففي الطلب");
        foreach($all_orders_end as $row2)
        {?>
            <tr>
                <td><?php echo $x;?></td>
                <td><?php echo $row2->mother_national_num;?></td>
    ...
    <?php $m=0; foreach($all_orders_end as $row) {
        $id=$row->id;
        $result=$all_orders_end[$m]->mother;
        $father=$all_orders_end[$m]->father;
        $member_data=$all_orders_end[$m]->abnaa;
        $devices=$all_orders_end[$m]->devices;
        $home_needs=$all_orders_end[$m]->home_needs;
        $financial_data_income=$all_orders_end[$m]->financial_data_income;
        $financial_data_duties=$all_orders_end[$m]->financial_data_duties;
        $sakn=$all_orders_end[$m]->sakn;
        $searcher=$all_orders_end[$m]->searcher_opinion;
        $m++;
        $mother_last_account = $row->mother_last_account;
        $person_account = $row->person_account;
        $agent_bank_account = $row->agent_bank_account;
...
<script>
    ...
    {
        var reason=$('.txt2').val();
        var glsa_num=$('#glsa_num').val();
        $.ajax({
            type:'post',
            url:"<?php echo base_url();?>family_controllers/LagnaSetting/change_aproved",
            data:{value:value,row:row,reason:reason,glsa_num:glsa_num},
            success:function(d){
                alert(d);
                location.reload();
            }
