...
<div class="tab-content">
 ...
 <div class="form-group col-sm-4 padding-4 form-group">
    <label class="label label-green  "> الخدمات التي يرغب الإستفادة منها </label>
    <select name="kadmat" class="form-control  " id="kadmat" 
            data-show-subtext="true" data-live-search="true" onchange="get_disable()"
            required="required" aria-required="true">
        <option value="">اختر</option>
          <?php
                            $arrxx = array(1 => 'جميع الخدمات التي تقدمها الجمعية ', 2 => 'الإستفادة من خدمات البرامج والأنشطة');
                            foreach ($arrxx as $key => $value) {
                                $select = '';
                                if (isset($result["kadmat"] )&&$result["kadmat"] != '') {
                                    if ($key == $result["kadmat"]) {
                                        $select = 'selected';
                                    }
                                } ?>
                                 <option
 ...
 <!--<div class="form-group col-sm-4 padding-4 form-group">
    ...
    <select name="kadmat" class="form-control  " id="kadmat" 
     ...
     <div class="form-group col-sm-3 padding-4 form-group">
                                    ...
                                    <input class="textbox form-control" type="text" name="retirement" id="retirement"
                                           value="<?php if (isset($result)) {
                                               echo $result["retirement"];
                                           } ?>" placeholder="تقاعد" style="width: 33.33%;float: right;">
                                    <input class="textbox form-control" type="text" name="insurance" id="insurance" 
                                           value="<?php if (isset($result)) {
                                               echo $result["insurance"];
                                           } ?>" placeholder="تأمينات" style="width: 33.33%;float: right;">
                                    <input class="textbox4 form-control" type="text" name="guarantee" id="guarantee"
                                           value="<?php if (isset($result)) {
...
<!--
    ...
    <div class=" col-md-3  col-sm-6  col-md-6 padding-4 form-group">
        ...
        -->
                        ...
                        <div class="setup-foot col-md-12 no-padding text-center">
                            ...
                            <button style="direction: ltr;" class="btn btn-success nextBtn  btn-labeled" type="button">
                                <span class="btn-label" style="right: auto;left: -14px;">
        ...
        if (cal2_mother.isHidden())
            ...
            cal2_mother.hide();
                        ...
                        <div class="setup-foot col-md-12 no-padding text-center">
                            ...
                            onclick="window.location='<?php echo base_url() . 'family_controllers/Family/AddNewRegister/screen-tab2' ; ?>'">
   ...
   }elseif($record->kadmat == 1){ // جميع الخدمات التي تقدمها الجمعية 
     $display_name = '';
      $eqrar_display_name = 'none';
   }elseif($record->kadmat == 2){ //  الإستفادة من خدمات البرامج والأنشطة
     $display_name = 'none';
     $eqrar_display_name = '';
   }else{
     $display_name = '';
...
<div class="btn-group">
    ...
    <ul class="dropdown-menu">
        <li style="display:<?php echo $display_name; ?>"><a target="_blank"
...
href="<?php echo base_url(); ?>family_controllers/Family_details/printEqrar_khdma/<?php echo $record->mother_national_num; ?>">إقرار الخدمات</a></li>
...
<ul class="dropdown-menu">
    ...
    <li style="display:<?php echo $display_name; ?>" >
        <a target="_blank" 
...
<td style="color: black;">
   ...
   -->
                                <td style="color: black;">
                                    ...
                                    <button data-toggle="modal" data-target="#modal-process-procedure"
        onclick="GetTransferPage(<?php echo $record->mother_national_num; ?>)"
        class="btn btn-sm btn-info" style="padding: 0 4px;">
    <i class="fa fa-list btn-info"></i> تحويل الطلب
</button>
                                </td>
                ...
                if (isset($adminastration) && !empty($adminastration)) {
                    foreach ($adminastration as $row_adminastration) {
                        $all_adminastration .= '<option value="' . $row_adminastration->id . '">' . $row_adminastration->title . '</option>'; ?>
                    <?php }
                } ?>
                                    ...
                                    <div class="col-sm-7">
                                        <div class="col-sm-12">
                                            <label style="float: right; width: 25%!important;"
                                                   class="label label-green  "> المسمي الوظيفي </label>
                                            <select name="" class="form-control  selectpicker"
                                                ...
                                                <?php if (isset($adminastration) && !empty($adminastration)) {
                                                    foreach ($adminastration as $row_adminastration) {
                                                        $selected = "";
                                                        if ($row_adminastration->code == $edara_id) {
                                                            $selected = 'selected=""';
                                                        } ?>
                                                        <option value="<?= $row_adminastration->code ?>" <?= $selected ?> >
                                                            <?= $row_adminastration->title ?></option>
                                                    <?php }
                                                } ?>
                                            </select>
...
<script>
    function GetTransferPage(value) {
        if (value != 0 && value != '') {
            ...
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>family_controllers/Family_transformation/GetTransferPage',
                data: {value: value},
                dataType: 'html',
                cache: false,
                beforeSend: function () {
...
<?php $tab_show = $this->uri->segment(4, 0);
if ($tab_show != '') {
?>
$(document).ready(function () {
    $('a[href="#<?php echo $tab_show; ?>"]').tab('show');
});
<?php } ?>
</script>
<script>
...
if(val==2)
{
    ...
    $('#guarantee').attr("disabled","disabled");    
    $('#building_type').attr("disabled","disabled"); 
    $('#h_rent_amount').attr("disabled","disabled"); 
}
else
{
    
    $('#retirement').removeAttr("disabled","disabled");    
    $('#insurance').removeAttr("disabled","disabled"); 
    $('#guarantee').removeAttr("disabled","disabled");    
    $('#building_type').removeAttr("disabled","disabled"); 
    $('#h_rent_amount').removeAttr("disabled","disabled"); 
}
