...
//
                var start=Unix_timestamp(event.start);
                var end=Unix_timestamp(event.end);



               

                var num = event.order_num;
                if(event.file_num != 0) {
                    num = event.file_num;
                }
                element.find('.fc-title').append("<br/>إسم الأب: " + event.father_name + "<br/>الغرض من الزيارة: " + event.purpose + "<br/>الوقت من: " +start + " إلى: " + end);
            },
            editable: true,
            ...
            eventDrop: function(event, delta){
                $.ajax({
                    ...
                    data: 'start='+(event.start).format('DD-MM-YYYY')+'&end='+(event.end).format('DD-MM-YYYY')+'&id='+event.id+'&s_time='+(event.start).format('HH:mm')+'&e_time='+(event.end).format('HH:mm'),
                    type: "POST",
            ...
            select: function(start, end, allDay) {
                bootbox.confirm("<form id='infos' method='post' action='<?=base_url()?>family_controllers/FamilyCalender/add_events'>\
                    <div class='row'>\
                        ...
                        <div class='form-group col-sm-6'>\
                            <label class='label label-green half'>وقت البدء<strong class='astric'>*</strong> </label>\
                            <input id='start_time' name='start_time' type='time' class='form-control confirmForm half input-style' value='"+start.format('HH:mm')+"' data-validation='required'>\
                        </div>\
                        <div class='form-group col-sm-6'>\
                            <label class='label label-green half'>وقت الإنتهاء<strong class='astric'>*</strong> </label>\
                            <input id='end_time' name='end_time' type='time' class='form-control confirmForm half input-style' value='"+end.format('HH:mm')+"' data-validation='required'>\
                        </div>\
                    ...
                    <input type='hidden' id='mother_national_num_fk' name='mother_national_num_fk' value='<?=$this->uri->segment(4)?>' >\
                        ...
                        if(result){
                            ...
                            if(require == true){
                              ...
                              //  alert($('#start_time2').val());
                                $.ajax({
                                    url: '<?=base_url()?>family_controllers/FamilyCalender/add_events',
                                    data: 'start_time='+start.format('DD-MM-YYYY')+'&end_time='+end.format('DD-MM-YYYY')+'&mother_national_num_fk='+$('#mother_national_num_fk').val()+'&manager_notes='+$('#manager_notes').val()+'&researcher_id_fk='+$('#researcher_id_fk').val()+'&visit_purpose_fk='+$('#visit_purpose_fk').val()+'&visit_search_type='+$('#visit_search_type').val()+'&s_time='+$('#start_time').val()+'&e_time='+$('#end_time').val(),
                                    type: "POST",
                                    success: function(json) {
                                        record = JSON.parse(json);
                                        render = record.events[0];
                                        ...
                                        calendar.fullCalendar('renderEvent',
                                            {
                                                ...
                                                father_name: render.father_name,
                                                file_num: render.file_num,
                                                order_num: render.order_num,
            ...
            <form id='' method='post'>\
                <div class='modal-body row'>\
                    <div class='col-sm-12 '><br>\
                        ...
                        <div class='form-group col-sm-6'>\
                            <label class='label label-green half'>وقت البدء<strong class='astric'>*</strong> </label>\
                            <input id='start_time' name='start_time' type='time' class='form-control confirmForm"+calEvent._id+" half input-style' data-validation='required'>\
                        </div>\
                        <div class='form-group col-sm-6'>\
                            <label class='label label-green half'>وقت الإنتهاء<strong class='astric'>*</strong> </label>\
                            <input id='end_time' name='end_time' type='time' class='form-control confirmForm"+calEvent._id+" half input-style' data-validation='required'>\
                        </div>\
                        <div class='form-group col-sm-12'>\
                            <label class='label label-green half' style='width: 23.5% !important;'>ملاحظات<strong class='astric'>*</strong> </label>\
                ...
                modal.find('#start_time').val(calEvent.start_time);
                modal.find('#end_time').val(calEvent.end_time);
                modal.find('form').on('submit', function(ev){
                    ...
                    if(require == true){
                        ...
                        calEvent.start_time = $(this).find("#start_time").val();
                        calEvent.end_time = $(this).find("#end_time").val();
                        ...
                        $.ajax({
                            ...
                            data: 'manager_notes='+calEvent.notes + '&id=' + calEvent._id + '&visit_search_type=' + calEvent.visit_search_type + '&visit_purpose_fk=' + calEvent.visit_purpose_fk + '&researcher_id_fk=' + calEvent.researcher_id_fk + '&start_time=' + calEvent.start_time + '&end_time=' + calEvent.end_time,
                            type: "POST",
                            success: function(json) {
                            }
                        });
