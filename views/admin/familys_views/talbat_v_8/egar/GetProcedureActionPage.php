    ...
    <div class="col-md-12 no-padding">
        <div class="col-md-8 no-padding">
            <div class="panel panel-default">
                <div class="panel-heading">تفاصيل طلب سداد إيجار</div>
                <div class="panel-body">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <td class="back">تاريخ الطلب</td>
                            <td><?= $result['talab_date_ar'] ?></td>
                        </tr>
                        <tr>
                            <td class="back">مقدم الطلب</td>
                            <td><?= $result['mokadem_name'] ?></td>
                            <td class="back">توقيت الطلب</td>
                            <td><?= $result['talab_time'] ?></td>
                        </tr>
                           <tr>
                                    <td class="back">إسم الأسرة</td>
                                    <td><?php echo $result['osra_name'] ?></td>
                                    <td class="back">رقم الملف</td>
                                    <td><?php echo $result['file_num'] ?></td>

                                </tr>
                           <tr>
                                      <td class="back"> الفئة</td>
                                    <td><?php echo $result['fe2a_n'] ?></td>
                        
                                   <td class="back"> الحي</td>
                                    <td><?php echo $result['hai_name'] ?></td>
                        </tr>
                        
                       
                            <tr>
                                      <td class="back"> تاريخ التسجيل</td>
                                    <td><?php echo $result['tasgel_date'] ?></td>
                        
                                   <td class="back"> عدد أفراد الأسرة</td>
                                    <td><?php echo $result['num_afrad'] ?></td>
                        </tr>
                       
                       
                        <tr>
                     
                                    <td class="back"> جوال التواصل</td>
                                    <td><?php echo $result['contact_mob'] ?></td>
                                </tr>
            ...
            if (isset($one_data) && !empty($one_data)) {
                ...
                $folder = 'uploads/family_attached/osr_talbat_egar_files';
                ?>
                <table class="table table-bordered">
                    <thead>
                    <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if (in_array($ext, $image)) {
                                    ...
                                    ?>
                                    <?php if (!empty($morfaq->file)) {
                                        $url = base_url() . 'family_controllers/talbat/Talb_service/read_file/' . $morfaq->file;
                                    } else {
                                        $url = base_url('asisst/fav/apple-icon-120x120.png');
                                    } ?>
        ...
        <div class="col-md-4 no-padding">
            <div class="panel panel-default">
                <div class="panel-heading">إجراءات علي طلب الإيجار</div>
                <div class="panel-body">
                    <?php
                    /*
                    echo'<pre>';
                    ...
                    if ($result['level'] == 0 and $result['current_to_user_id'] == null) { ?>
                        <form action="<?= site_url('family_controllers/talbat/Talb_egar/process_transform_to_mowazf_mokhtas') ?>"
                              method="post" id="transform_form">
                            <input type="hidden" name="save" value="save">
                            <input type="hidden" name="talab_id" value="<?= $result['id'] ?>"/>
                                            ...
                                            <div class="radio">
                                                <label style="color: green;">
                                                    <input type="radio" value="accept" checked="checked"
                                                           name="option_choosed">تم مراجعة الطلب وجميع البيانات 
                                                </label>
                                            </div>
                                        </div>
                                    </td>
                    ...
                    <?php } elseif ($result['level'] == 1 and $result['current_to_user_id'] == $_SESSION['user_id']) { ?>
                        <form action="<?= site_url('family_controllers/talbat/Talb_egar/process_transform_to_moder_edara') ?>"
                              method="post" id="transform_form">
                            <input type="hidden" name="save" value="save">
                            <input type="hidden" name="talab_id" value="<?= $result['id'] ?>"/>
                            <input type="hidden" name="rkm_talab" value="<?= $result['rkm_talab'] ?>"/>
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <td>
                                        <div class="form-group">
                                            <label for="exampleFormControlTextarea1">توصيات مدير الإدارة</label>
                                            <select class="form-control form-control-sm" name="to_person_id"
                                                    id="to_person_id">
                    ...
                    <?php } elseif ($result['level'] == 2 and $result['current_to_user_id'] == $_SESSION['user_id']) { ?>
                        <form action="<?= site_url('family_controllers/talbat/Talb_egar/process_transform_moder') ?>"
                              method="post" id="transform_form">
                            <input type="hidden" name="save" value="save">
                            <input type="hidden" name="talab_id" value="<?= $result['id'] ?>"/>
                            <input type="hidden" name="rkm_talab" value="<?= $result['rkm_talab'] ?>"/>
