...
<div class="col-md-12 no-padding">
    <div class="col-md-8 no-padding">
        <div class="panel panel-default">
            <div class="panel-heading">تفاصيل طلب خدمة رعاية</div>
            <div class="panel-body">
                <table class="table table-bordered">
                    ...
                    <tr>
                        <td class="back">نوع الخدمة</td>
                        <td><?php
                            foreach ($service as $cat) {
                                if ($result['service'] == $cat->id) echo $cat->khdma_name;
                            }
                            ?></td>
                        <td class="back">الوصف</td>
                        <td><?= $result['wasf'] ?></td>
                    </tr>
                    <tr>
                        <table class="table table-bordered ">
        ...
        if (isset($one_data) && !empty($one_data)) {
            ...
            $folder = 'uploads/family_attached/osr_talbat_service_files';
            ?>
            <table class="table table-bordered">
                <thead>
                <tr>
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                if ($result['level'] == 0 and $result['current_to_user_id'] == null) { ?>
                    <form action="<?= site_url('family_controllers/talbat/Talb_service/process_transform_to_bahth') ?>"
                ...
                <?php } elseif ($result['level'] == 1 and $result['current_to_user_id'] == $_SESSION['user_id']) { ?>
                    <form action="<?= site_url('family_controllers/talbat/Talb_service/process_transform_to_moder_edara') ?>"
                          method="post" id="transform_form">
                        <input type="hidden" name="save" value="save">
                        <input type="hidden" name="talab_id" value="<?= $result['id'] ?>"/>
                        <input type="hidden" name="rkm_talab" value="<?= $result['rkm_talab'] ?>"/>
                ...
                <?php } elseif ($result['level'] == 2 and $result['current_to_user_id'] == $_SESSION['user_id']) { ?>
                    <form action="<?= site_url('family_controllers/talbat/Talb_service/process_transform_moder') ?>"
                          method="post" id="transform_form">
                        <input type="hidden" name="save" value="save">
                        <input type="hidden" name="talab_id" value="<?= $result['id'] ?>"/>
                        <input type="hidden" name="rkm_talab" value="<?= $result['rkm_talab'] ?>"/>
