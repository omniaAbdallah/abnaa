...
<?php if (isset($buttun_roles) && !empty($buttun_roles) && $buttun_roles != null) { ?>
   ...
   <!--  <div class="col-sm-6">
    ...
    <tbody>
      <tr>
        <td><button type="submits" class="btn btn-danger w-md m-b-5" data-toggle="modal" data-target="#modal-sm-2">
                    <span><i class="fa fa-window-close" aria-hidden="true"></i></span>  رفض الطلب                </button></td>
        <td><button type="submits" class="btn btn-danger w-md m-b-5" data-toggle="modal" data-target="#modal-sm-2">
                    <span><i class="fa fa-window-close" aria-hidden="true"></i></span>  رفض الطلب                </button></td>
        <td><button type="submits" class="btn btn-danger w-md m-b-5" data-toggle="modal" data-target="#modal-sm-2">
                    <span><i class="fa fa-window-close" aria-hidden="true"></i></span>  رفض الطلب                </button></td>
    ...
    <tbody>
      <tr> 
        ...
        <?php $arr_prosess = array("1" => " قبول الطلب", "2" => " رفض الطلب", "3" => " تحويل الطلب", "4" => "تفعيل الطلب", "5" => "تحويل الى لجنة", 6 => 'سجل الزيارات') ?>
        <?php $arr_but_color = array("1" => "success", "2" => "danger", "3" => "primary", "4" => "add", "4" => "add", "5" => "purple", 6 => 'purple') ?>
        <?php $arr_but_icon = array("1" => "check-square-o", "2" => "window-close", "3" => "paper-plane", "4" => "floppy-o", "5" => "floppy-o", 6 => 'user') ?>
        <?php foreach ($buttun_roles as $row => $value) { ?>
            <td>
            <div class="col-sm-2">
            
            
                <button type="submits" class="btn btn-<?= $arr_but_color[$value] ?> w-md m-b-5"
                        data-toggle="modal"   data-target="#modal-sm-<?= $value ?>">
                    <span><i class="fa fa-<?= $arr_but_icon[$value] ?>"
                             aria-hidden="true"></i></span> <?= $arr_prosess[$value] ?>
                </button>
            </div>
            </td>
        <?php } ?>
       <!--  <button type="submits" class="btn btn-<?= $arr_but_color[$value] ?> w-md m-b-5"
                        data-toggle="modal"   data-target="#modal-sm-6">
                    <span><i class="fa fa-<?= $arr_but_icon[$value] ?>"
                             aria-hidden="true"></i></span> ا
                </button>-->
              </tr>
...
<?php if (isset($buttun_roles) && !empty($buttun_roles) && $buttun_roles != null) { ?>
    <?php
    foreach ($buttun_roles as $row => $value) {
        ?>
                                    ...
                                    if (isset($visitData) && $visitData != null) {
                                        foreach ($visitData as $id => $visit) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <select name="note[]" class="form-control" data-validation="required" >

                                                        <option value=""> اختر</option>
                                                        <?php if(isset($procedures_visit) &&!empty($procedures_visit)){
                                                            foreach($procedures_visit as $row){?>

                                                                <option value="<?php echo $row->id_setting;?>" <?php if($row->id_setting==$visit->note){
                                                                    echo 'selected' ;}?> >
                                                ...
                                                <td>
                                                    <input type="text" class="form-control bottom-input"
                                                           name="researcher_note[]" id="researcher_note<?php echo $len;?>"
                                                           onkeyup="get_length($(this).val(),<?php echo $len;?>);"data-validation=""
                                                           value="<?= $visit->researcher_note ?>"/>
                                                </td>
                                                <td>
                                                    <select name="visit_status[]" class="form-control bottom-input"
                                                            data-validation="required" id="visit_status<?php echo $len;?>">
                                                        <option value="">إختر</option>
                                                        <?php
