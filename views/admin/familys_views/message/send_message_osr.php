...
<div class="form-group  col-md-3">
    <label class="label ">  نموذج الرسالة </label>
    <select name="namozag_message" id="namozag_message" 
    onchange="get_namozag_message();"
            class="form-control "  >
        <option value="">إختر</option>
        <?php $x=1; foreach ($namazeg as $row){
            $selected="";
            if(isset($sarf_data)){
                if($sarf_data["namozag_message"]== $row->id ){$selected="selected";}
            }?>
            <option value="<?=$row->id?>" <?=$selected?>><?=$row->namozeg_name?></option>
            <?php $x++;} ?>
    </select>
</div>  
</div>
...
<div class="col-xs-12 no-padding"  id="load_message">
  ...
  type="text" name="countdown" size="3" value="255"> الحروف المتبقية .</font>
    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body">
                ...
                {
                    echo 'مجموعة ('.$record->family_group_name.')';
                }
...
<script>
    function get_namozag_message() {
      var namozag_message=$('#namozag_message').val();
      
      $.ajax({
            url: "<?php echo base_url() ?>family_controllers/message/Message_c/get_namozag_message",
            type: "Post",
            dataType: "html",
            data: {namozag_message: namozag_message},
            success: function (data) {

              
                $('#load_message').html(data);
                limitText(this.form.message,this.form.countdown,255);
