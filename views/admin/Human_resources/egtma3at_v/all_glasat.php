...
<div class="col-xs-12 fadeInUp wow">
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body">
            ...
            $alert_swal = 2;
            ?>
            <form action="<?php echo base_url(); ?>human_resources/egtma3at/Egtma3at_c/update_galsa/<?php echo $this->uri->segment(5); ?>" method="post" id="form1">
                <?php } else{
                $alert_swal = 1;
                ?>
                <form action="<?php echo base_url(); ?>human_resources/egtma3at/Egtma3at_c" method="post" id="form1">
                    <?php } ?>
                    <div class="col-sm-12">
                        ...
                        <div class="col-sm-3  col-md-2 padding-4 ">
                            ...
                            <input type="date" name="glsa_date" id="glsa_date" class="form-control "
                                   ...
                                   value="<?php if (isset($last_magls_update) && !empty($last_magls_update->glsa_date_ar)) {
                                       echo $last_magls_update->glsa_date_ar;
                                   }else{
                                       echo date('Y-m-d');
                                   } ?>">
                        </div>
                        <div class="col-sm-3  col-md-1 padding-4 ">
                            ...
                            <input type="text" name="glsa_day" id="glsa_day" class="form-control " data-validation="required"
                                   value="<?php if (isset($last_magls_update) && !empty($last_magls_update->glsa_day)) {echo $last_magls_update->glsa_day; }else{
...
value="<?php if (!empty($last_magls_update->glsa_reviser_id)) {
    echo $last_magls_update->glsa_reviser_id;
                             ...
                             <button type="button" data-toggle="modal" data-target="#myModal_emps"
                                ...
                                <!-- <button type="button" data-toggle="modal" data-target="#myModal_emps"
                                    class="btn btn-success btn-next" style=""
                                    onclick="GetDiv_magles('myDiv_emp')">
                                <i class="fa fa-plus"></i>اعضاء مجلس الادارة</button>  -->
                            </div>
                        ...
                        <br>
                            <div class="col-md-12 text-center padding-4 ">
                                ...
                                <button type="button" <?= $disabled ?>
                                        ...
                                        background-color: #3c990b;border-color: #12891b;padding-top: 0;padding-bottom: 0;border-radius:2px; ">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                <?php if ($row->glsa_finshed == 0) { ?>
                                                    <li><a target="_blank"
                                                           href="<?= base_url() . 'human_resources/egtma3at/Egtma3at_c/update_galsa' . '/' . $row->glsa_rkm ?>">
                                                            تعديل بيانات الجلسة </a></li>
                                                <?php } ?>
                                                <li><a target="_blank"
                                                       href="<?= base_url() ?>human_resources/egtma3at/Egtma3at_c/a3da_glsa/<?= $row->glsa_rkm ?>">
                                                        أعضاء الجلسه </a></li>
                                                <li><a target="_blank"
                                                       href="<?= base_url() ?>human_resources/egtma3at/Egtma3at_c/mehwr_glsa/<?= $row->glsa_rkm ?>">
                                                        محاور الجلسه </a></li>
                                                 <li><a target="_blank"
                                                       href="<?= base_url() ?>human_resources/egtma3at/Egtma3at_c/add_galsat_mon24a/<?= $row->glsa_rkm ?>">
                                                        مناقشة الجلسة </a></li>
                                                <li><a target="_blank"
                                                       href="<?= base_url() ?>human_resources/egtma3at/Egtma3at_c/add_attach/<?= $row->glsa_rkm ?>">
                                                        إضافة مرفقات </a></li>
                                                <li><a target="_blank"
                                                       href="<?= base_url() ?>human_resources/egtma3at/Egtma3at_c/add_video/<?= $row->glsa_rkm ?>">
                                                        مكتبة الفيديوهات </a></li>
                                                <li>
                                                    <a href="<?= base_url() ?>human_resources/egtma3at/Egtma3at_c/add_image/<?= $row->glsa_rkm ?>">
                                                        مكتبة الصور </a></li>
                                            </ul>
                                        </div>
                                        <?php
                                        if ($row->glsa_finshed == 0) { ?>
                                            <a onclick=' swal({
                                                    ...
                                                    setTimeout(function(){window.location="<?php echo base_url() . 'human_resources/egtma3at/Egtma3at_c/delete_galsa/' . $row->id . '/' . $row->glsa_rkm ?>";}, 500);
                                                    });'>
    ...
    <script>
        function send_da3wat(value) {
            if (value != 0 && value != '') {
                ...
                $.ajax({
                    type: 'post',
                    url: '<?php echo base_url() ?>human_resources/egtma3at/Egtma3at_c/send_da3wat',
                    data: dataString,
                    dataType: 'html',
                    cache: false,
    ...
    <script>
        function change_status(valu, id) {
            $.ajax({
                type: 'post',
                url: "<?php echo base_url();?>human_resources/egtma3at/Egtma3at_c/change_status",
...
<script>
            ...
            function GetDiv_emps(div) {
                ...
                var oTable_usergroup = $('#js_table_members2').DataTable({
                    dom: 'Bfrtip',
                    "ajax": '<?php echo base_url(); ?>human_resources/egtma3at/Egtma3at_c/getConnection_emp/',
                    aoColumns: [
                        {"bSortable": true},
                        {"bSortable": true},
                        {"bSortable": true},
