                    ...
                    <table id="example" class=" display table table-bordered   responsive nowrap" cellspacing="0"
                        ...
                        foreach ($records as $row) {
                            ...
                            <td><?php echo $x; ?></td>
                                ...
                                <td><?php
                                // echo date('A H:i', strtotime($row->attend_time)+$row->season_frq);//+$row->season_frq);
                                $date = strtotime($row->attend_time);
                                $date = strtotime("+$row->season_frq minute", $date);
                                echo date('A H:i:s', $date); 
                                ?></td>
                                <td><?php //echo date('A H:i', strtotime($row->leave_time)+$row->season_frq);
                                  $date = strtotime($row->leave_time);
                                  $date = strtotime("+$row->season_frq minute", $date);
                                  echo date('A H:i:s', $date); 
                                ?></td>


                                <td><?php echo $row->late_min+$row->season_frq; ?></td>
                                <td><?php echo $row->leave_early_min+$row->season_frq; ?></td>
                                <td><?php
                                // echo date('A H:i', strtotime($row->start_enter)+$row->season_frq); 
                                $date = strtotime($row->start_enter);
                                $date = strtotime("+$row->season_frq minute", $date);
                                echo date('A H:i:s', $date); 
                                ?></td>
                                <td>
                                <?php //echo date('A H:i', strtotime($row->end_enter)+$row->season_frq);
                                $date = strtotime($row->end_enter);
                                $date = strtotime("+$row->season_frq minute", $date);
                                echo date('A H:i:s', $date); 
                                 ?></td>
                                <td><?php
                                // echo date('A H:i', strtotime($row->start_out)+$row->season_frq);
                                $date = strtotime($row->start_out);
                                $date = strtotime("+$row->season_frq minute", $date);
                                echo date('A H:i:s', $date); 
                                 ?></td>
                                <td><?php 
                                //echo date('A H:i', strtotime($row->end_out)+$row->season_frq); 
                                $date = strtotime($row->end_out);
                                $date = strtotime("+$row->season_frq minute", $date);
                                echo date('A H:i:s', $date); 
                                ?></td>
