...
<div class="col-sm-12  col-xs-12 no-padding" >
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body">
            <?=$head?>
            <?php echo form_open_multipart('human_resources/Dwam_settings/add_dwam/'.$this->uri->segment(4).'',array("class"=>"dwam_details_form"));?>
            <div class="col-sm-12 col-xs-12 no-padding">
...
<?php    $work_types = array("full" => "دوام كلي", "half" => "دوام جزئي"); ?>
                        <select name="no3_dawam" id="no3_dawam" 
                                data-validation="required" class="form-control  ">
                            <option value=" " selected="">اختر</option>
                            <?php foreach ($work_types as $key => $value) { ?>
                                <option value="<?php echo $key; ?>" <?php if ($result['no3_dawam']== $key) {
                                    echo 'selected';
...
<script>
    $(document).ready(function () {
        load_dwam_table(<?=$this->uri->segment(4)?>);
    });
</script>
<script>
    function load_dwam_table(emp_id) {
        $.ajax({
            type: 'post',
            url: '<?php echo base_url()?>human_resources/Dwam_settings/dwam_table/'+emp_id,
            data: {},
            dataType: 'html',
            cache: false,
            beforeSend: function () {
