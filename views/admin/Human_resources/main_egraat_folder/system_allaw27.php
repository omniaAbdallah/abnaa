<link rel="stylesheet" href="<?php echo base_url()?>asisst/admin_asset/css/style111.css">
<style>
.row {
margin-right: -15px;
margin-left: -15px;
...
<p><span class="icon"><img src="<?= base_url()?>asisst/admin_asset/img/requlations-icon.png" alt="">
</span><?=$sys->title?></p></a><div class="row"><div class="col-sm-7">
<span class="date"><i class="fa fa-calendar" aria-hidden="true"></i> <?=$sys->publishe_date?></span>
</div><div class="col-sm-5 card-actions">
<a href="<?= base_url()."human_resources/Human_resources/downloads/".$sys->file?>"
class="fa fa-download" title="تحميل المرفقات"></a>
<?php
$mehwar_morfaq = $sys->file;
...
if (in_array($type, $arry_images)) {
    ...
    <?php if (!empty($mehwar_morfaq)) {
        ...
        $url =base_url().'human_resources/Human_resources/read_file/'.$mehwar_morfaq;
    } else {
        $url = base_url('asisst/fav/apple-icon-120x120.png');
    } ?>
...
<a href="<?= base_url()."human_resources/Human_resources/downloads/".$record->file?>"
class="fa fa-download" title="تحميل المرفقات"></a>
<?php
