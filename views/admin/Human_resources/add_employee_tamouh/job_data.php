...
?>
<?php
$out['form'] = 'human_resources/tamouh_emp/Tamouh/add_job_data/' . $this->uri->segment(5);
?>
    ...
    <div class=" panel panel-bd lobidisable lobipanel lobipanel-sortable ">
            ...
            <div class="form-group col-md-4 col-sm-6 padding-4">
                ...
                <select name="degree_id" id="degree_id3" class="form-control " data-validation="required"
                    ...
                    <?php foreach ($job_role as $one_job_role) { ?>
                        <option value="<?php echo $one_job_role->id; ?>"<?php if ($one_job_role->id == $degree_id) {
                            echo 'selected';
                        } ?>><?php echo $one_job_role->name; ?></option>';
                    <?php } ?>
                </select>
            </div>
                    ...
                    <?php if (isset($direct_all_emps) && $direct_all_emps != null) {
                        foreach ($direct_all_emps as $value) { ?>
                            <option value="<?= $value->id ?>"
                                    data-type="<?= $value->emp_type ?>" <?php if ($value->id == $manger) {
                                echo 'selected';
        ...
        $this->load->view('admin/Human_resources/add_employee_tamouh/sidebar_person_data', $data_load); ?>
        <!----------------------------------------------------------------------------------------------------------------->
...
<script>
    function lood(num) {
        if (num > 0 && num != '') {
            ...
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>human_resources/tamouh_emp/Tamouh',
                data: dataString,
                dataType: 'html',
                cache: false,
       ...
       <script>
            function get_direct_manger(depart_id) {
                ...
                if (depart_id > 0 && depart_id != '') {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?php echo base_url() ?>human_resources/tamouh_emp/Tamouh/get_direct_manger',
