...
if (isset($get_ezn) && !empty($get_ezn)) {
    ...
    <table class="table" style="table-layout: fixed">
        ...
        <?php
        if (!empty($get_ezn->no3_ezn)) {
            if ($get_ezn->no3_ezn == 2) {
           ?>
        <tr>
            <td><strong> جهة الزيارة </strong></td>
            <td><strong>:</strong></td>
            <td><?php echo $get_ezn->geha_name; ?></td>
            <td><strong>نوع الزيارة </strong></td>
            <td><strong>:</strong></td>
            <td><?php
                if (!empty($get_ezn->no3_ezn)) {
                    if ($get_ezn->visit_type == 1) {
                        echo "ميداني";
                    } elseif ($get_ezn->visit_type == 2) {
                        echo "  تواصل";
                    }
                }
                ?></td>
            <td><strong>الغرض من الزيارة </strong></td>
            <td><strong>:</strong></td>
            <td><?php echo $get_ezn->visit_goal; ?></td>

        </tr>
        <tr>
            <td><strong> موضوع الزيارة </strong></td>
            <td><strong>:</strong></td>
            <td colspan="2"><?php echo $get_ezn->visit_sub; ?></td>
            <td><strong>تفاصيل الزيارة </strong></td>
            <td><strong>:</strong></td>
            <td colspan="3"><?php echo $get_ezn->visit_detailes; ?></td>
        ...
        <tr>
            ...
            <td><strong>:</strong></td>
            <td colspan="7"><?= $get_ezn->reason ?></td>
