<input type="hidden" id="current_from_id" value="<?php echo $row->current_to_id; ?>"><input type="hidden"
                                                                                            id="emp_user_id_fk"
                                                                                            value="<?php echo $row->emp_id_fk; ?>">
<input type="hidden" id="current_from_user_name" value="<?php echo $row->current_to_user_name; ?>"><input type="hidden"
                                                                                                          id="last_from_id"
                                                                                                          value="<?php echo $row->emp_user_id; ?>">
<input type="hidden" id="last_to_id" value="0"><input type="hidden" id="last_from_user_name" value="0"><input
        type="hidden" id="last_to_user_name" value="0"><input type="hidden" id="last_procedure" value="0"><input
        type="hidden" id="last_procedure_title" value="0"><input type="hidden" id="level"
                                                                 value="<?php echo $row->level; ?>"><input type="hidden"
                                                                                                           id="ezn_id"
                                                                                                           value="<?php echo $row->id; ?>">
<input type="hidden" id="ezn_rkm" value="<?php echo $row->ezn_rkm; ?>"><input type="hidden" id="message_accept"
                                                                              value="<?php echo $mess->msg_accept; ?>">
<input type="hidden" id="message_refuse" value="<?php echo $mess->msg_refuse; ?>"><input type="hidden" id="emp_name"
                                                                                         value="<?php echo $row->emp; ?>">
<input type="hidden" id="emp_job" value="<?php echo $row->emp_job; ?>"><input type="hidden" id="emp_img"
                                                                              value="<?php echo $row->personal_emp_img; ?>"><?php $manager = $row->direct_manager; ?><?php if ($level == 1) { ?>
    <div class="col-md-8">
        ...
        <div class="form-group col-md-6 col-sm-6 padding-4 has-success accept"><label class="label">الموظف </label>
            <select name="gender" onchange="get_emp_data($(this).val());" id="employee_id" data-validation="required"
                ...
                <option value=""> اختر</option> <?php if (isset($employee) && !empty($employee)) {
                    foreach ($employee as $item) { ?>
                        <option value="<?php echo $item->person_id; ?>"> <?php echo $item->person_name; ?></option>                        <?php }
                } ?>            </select></div>
    </div>
    <div class="col-md-4 ">
    <div class="col-md-12 profile">
        <table class="table table-bordered s col-md-12" style="">
            ...
            <tr>
                <td colspan="2">                        <?php if ((isset($row->personal_emp_img)) && (!empty($row->personal_emp_img)) && (file_exists( 'uploads/human_resources/emp_photo/thumbs/' . $row->personal_emp_img))) { ?>
                        <img id="empImg"
                             ...
                             alt="">                        <?php } else { ?>                            <img
                            id="empImg" src="<?php echo base_url(); ?>asisst/admin_asset/img/user.png"
                            alt="">                        <?php } ?>                    </td>
            </tr>
            </thead>
            <tbody>
            ...
            <tr>
                <td id="name-emp"
                    class="text-center"><?php if ((isset($row->emp)) && (!empty($row->emp))) echo $row->emp; ?></td>
            </tr>
            <tr class="greentd">
            ...
            <tr>
                <td id="job-title"
                    class="text-center"> <?php if ((isset($row->emp_job)) && (!empty($row->emp_job))) echo $row->emp_job; ?></td>
            </tr>
            </tbody>
        </table>
    </div>    </div><?php }
...
if ($level == 3) { ?>
    ...
    <div class="col-md-8">
        ...
        <!-- <div class="form-group col-md-6 col-sm-6 padding-4 has-success accept">                <label class="label">الاداره </label>                <select name="gender" onchange="get_emps($(this).val())" id="edara"                        data-validation="required"   class="form-control valid"                        aria-required="true">                    <option value="">إختر</option>                    <?php if (isset($admin) && !empty($admin)) {
            foreach ($admin as $row) { ?>                            <option value="<?php echo $row->id; ?>"> <?php echo $row->name; ?></option >                            <?php }
        } ?>                </select>            </div> -->
        <div class="form-group col-md-6 col-sm-6 padding-4 has-success accept"><label class="label">الموظف </label>
            <select name="gender" onchange="get_emp_data($(this).val());" id="employee_id" data-validation="required"
                    class="form-control valid" aria-required="true">
...
<script>    function make_suspend() {
        valu = $(".decision:radio:checked").val();
        if (valu != 1 && valu != 2) {
            swal('تنبيه!', 'من فضلك حدد قراراك سواء القبول او الرفض');
            return;
        }
        if (valu == 1) {
            ...
            if (level_check == 1 || level_check == 2 || level_check == 3) {
                if ($('#employee_id').val() == '') {
                    swal('تنبيه!', 'من فضلك قم باختيار الموظف ');
                    return;
                }
            }
//موافق      
        ...
        if (valu == 2) {
            ...
            $.ajax({
                ...
                success: function (d) {
                    swal('بنجاح!', 'تم رفض الطلب وتحويله الي الموظف مقدم الطلب');
                    $('#detailsModal2').modal('hide');
                    $('#ezn_table').hide();
                    location.reload();
                }
...
<script>    function get_emp_data(emp_id) {
        $.ajax({
            type: 'post',
...
<script>    function show_hide(valu) {
        if (valu == 2) {
            $('.accept').hide();
