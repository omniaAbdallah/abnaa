<div class="col-sm-12 no-padding">
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
        ...
        <div class="panel-body">
            ...
            {
                echo form_open_multipart('human_resources/employee_forms/late_emp/Late_emp_c/update/'.$result->id, array('id' => 'myform'));
                ...
                $late_value=$result->late_value;
             
                $remove_late_value=$result->remove_late_value;
            }else{

            
                echo form_open_multipart('human_resources/employee_forms/late_emp/Late_emp_c/insert', array('id' => 'myform'));
...
<div class="col-sm-2  col-md-1 padding-4 ">
                    <label class="label  "> عدد الدقائق </label>
                    <input type="number" value="<?= $late_min;?>"
                    
                     name="late_min" id="num"
...
<div class="col-sm-2  col-md-1 padding-4 ">
                    ...
                    <input type="number" value="<?= $late_value;?>" name="late_value" id="new_value"
                readonly
                    data-validation="required"
                           class="form-control ">
                </div>
                <div class="form-group col-sm-1 col-xs-12">
                                        ...
                                        <input  name="remove_late_value" type="checkbox" money="0"
                                        class="form-control"
                                                       

                                                       value="1"<?php if (!empty($remove_late_value) && $remove_late_value == 1) {
                ...
                <div class="col-sm-4  col-md-3 padding-4 ">
                    <label class="label  ">سبب التاخير </label>
...
<?php if (isset($records) && $records != null) { ?>
    <div class="col-sm-12 no-padding">
        <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
            ...
            <div class="panel-body">
                <table id="" class="example table table-bordered table-striped" role="table">
                    ...
                    <tr class="greentd">
                        <th>م</th>
                        <th class="text-center"> عدد الدقائق</th>
                    ...
                    foreach ($records as $value) {
                        ...
                        <tr>
                            ...
                            <td>
                            <?php 
                            if(isset($value->late_min)&&!empty($value->late_min)) 
                            echo $value->late_min ;
                            else echo 'غير محدد';
                            ?>
                            </td>
                            ...
                            <td>
                            <?php 
                            if(isset($value->late_value)&&!empty($value->late_value)) 
        ...
        setTimeout(function(){window.location="<?= base_url() . 'human_resources/employee_forms/late_emp/Late_emp_c/Delete_namozeg/' . $value->id ?>";}, 500);
        });'>
    ...
    <a onclick=' swal({
        ...
        window.location="<?= base_url() . 'human_resources/employee_forms/late_emp/Late_emp_c/update_gezaa/' . $value->id ?>";
        });'>
...
<script>
            function GetDiv_emps(div) {
                ...
                var oTable_usergroup = $('#js_table_members2').DataTable({
                    dom: 'Bfrtip',
                    "ajax": '<?php echo base_url(); ?>human_resources/employee_forms/late_emp/Late_emp_c/getConnection_emp/',
                    aoColumns: [
                        {"bSortable": true},
                        {"bSortable": true},
                        {"bSortable": true},
...
<script>
    function change_status(valu, id) {
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>human_resources/employee_forms/late_emp/Late_emp_c/change_status",
...
function get_all_data()
{
    $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>human_resources/employee_forms/late_emp/Late_emp_c/get_all_data",
            beforeSend: function () {
...
function get_pop_detailes(emp_id) {
    var emp_id=emp_id.value;
        $.ajax({
            type: 'post',
            url: '<?php echo base_url() ?>human_resources/employee_forms/late_emp/Late_emp_c/get_pop_detailes',
            data: {emp_id: emp_id},
            cache: false,
  ...
  /*  function get_value()
    {
       ...
       var sa3at_amal=$('#sa3at_amal').val();
       if(total_bdlat !='' ){
       var result=(total_bdlat*num)/(ayam_amal*sa3at_amal*60);
