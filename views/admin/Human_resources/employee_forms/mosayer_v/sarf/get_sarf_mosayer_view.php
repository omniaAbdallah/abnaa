...
class="btn btn-warning"
                onclick="tahwel_sarf()" title="تحويل"><i class="fa fa-archive"> </i>  تحويل الصرف </button>
  ...
  foreach ($all_mosayer_data as $emp){
   ?>
   <input type="hidden" id="mosayer_rkm_fk" value="<?=$emp->mosayer_rkm_fk;?>"/>
   <input type="hidden" id="choosed_pay_method_id_fk" value="<?=$choosed_pay_method_id_fk;?>"/>
        ...
        <tr>
              ...
              <?php 
              if($choosed_pay_method_id_fk == 2){
                $pay_method_title = 'الشيك';
              }elseif($choosed_pay_method_id_fk == 3){
                 $pay_method_title = 'التحويل'; 
              }else{
                  $pay_method_title = ''; 
              }
              ?>
  <strong></strong> تم إرسال موظفي <?=$pay_method_title ?> إلي الصرف
</div> 
</div> 
           <?php  }
            ?>
            ...
            <script>
        function tahwel_sarf() {
            var id=$('#mosayer_rkm_fk').val();
            var choosed_pay_method_id_fk =$('#choosed_pay_method_id_fk').val();
            
        swal({
  title: "هل انت متاكد من ارسال الصرف؟",
  type: "warning",
...
function(isConfirm) {
  if (isConfirm) {
    $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>human_resources/employee_forms/Employee_salaries/tahwel_sarf',
                data: {id: id,choosed_pay_method_id_fk:choosed_pay_method_id_fk},
                dataType: 'html',
                cache: false,
                beforeSend: function()
                {
