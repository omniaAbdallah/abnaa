...
<div class="col-md-12 no-padding">
    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
            <div class="panel-heading">
                <h3 class="panel-title">طلب تأجيل سلفة</h3>
            </div>
 <?php
...
$fe2a_tagel=$items->fe2a_tagel;
$tagel_reason=$items->tagel_reason;
...
$tagel_reason='';
                ...
                <div class="col-sm-12 no-padding">
                    ...
                    if (isset($items) && !empty($items)){
                    ?>
                    <form action="<?php echo base_url() . 'human_resources/employee_forms/solaf/Solaf/edit_tagel_solaf/' . $items->id .'/' . $items->solfa_rkm; ?>"
                          method="post" id="form1">

                        <?php } else{ ?>
                        <form action="<?php echo base_url(); ?>human_resources/employee_forms/solaf/Solaf/tagel_solaf"
                              method="post" id="form1">
                            ...
                            <input type="hidden" id="emp_id_fk" name="emp_id_fk"
                                   value="<?php if (!empty($emp_data->id)) {
                                    ...
                                    <input type="hidden" id="solfa_rkm" name="solfa_rkm"
                                   value="<?php 
                                       echo $item->id;
                                   ?> ">
                            ...
                            <div class="col-md-12 no-padding">
                                ...
                                <?php $arr = array(1 => 'أضافة للشهر القادم ', 2 => '    أضافة لاخر شهر', 3 => '    اعادة الجدوله علي الاشهر المتبقية',4=>'زيادة شهر جديد'); ?>
                                <div class="form-group col-md-3 col-sm-6 padding-4">
                                    <label class="label ">  طريقة التأجيل </label>
                                    <select id="fe2a_tagel" name="fe2a_tagel"   data-validation="required"
                                            class="form-control">
                                ...
                                <div class="form-group col-md-2  col-sm-6 col-xs-6  padding-4">
                                    ...
                                    readonly
                                           value="<?php if ($item->sadad_solfa == 3) {
                                               echo $item->qemt_solaf / $item->qst_num;
                                ...
                                <div class="form-group col-md-5  col-sm-6 col-xs-6  padding-4">
                                    ...
                                    <label class="label "> سبب التأجيل </label>
                                    <input type="text" id="tagel_reason" name="tagel_reason"  data-validation="required"
                                           value="<?= $tagel_reason; ?>" class="form-control "
                                           data-validation-has-keyup-event="true">
        ...
        <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
            ...
            <table id="example" class=" display table table-bordered   responsive nowrap" cellspacing="0" width="100%">
                ...
                <tr class="greentd">
                    ...
                    <th style="width: 140px;">فئه التأجيل</th>
                    <th style="width: 75px;"> التأجيل لشهر</th>
                    <th> الاجراء</th>
                    <th> حالة الطلب</th>
                ...
                if (isset($records) && !empty($record)) {
                    ...
                    foreach ($records as $row) {
                           ...
                           $link_update = 'window.location="' . base_url() . 'human_resources/employee_forms/solaf/Solaf/edit_tagel_solaf/' . $row->id . '/' . $row->solfa_rkm .'";';
                            $link_delete = 0;
                        ...
                        <tr>
                            ...
                            <td><?php if ($row->fe2a_tagel == 1) {
                                    echo ' أضافة للشهر القادم ';
                                } elseif ($row->fe2a_tagel == 2) {
                                    echo '  أضافة لاخر شهر';
                                } elseif ($row->fe2a_tagel == 3) {
                                    echo 'اعادة الجدوله علي الاشهر المتبقية';
                                }
                                elseif ($row->fe2a_tagel == 4) {
                                    echo 'زيادة شهر جديد';
                                }
                                ?></td>
                            ...
                            <a class="option_td<?= $row->id ?>" onclick='swal({
                                    ...
                                    <a class="option_td<?= $row->id ?>" onclick=' swal({
                                            ...
                                            setTimeout(function(){window.location="<?= base_url() . 'human_resources/employee_forms/solaf/Solaf/delete_tagel_solfa/' . $row->id . '/' . $row->solfa_rkm ?>";}, 500);
                                            });'>
                                        ...
                                        <a onclick="make_transformation_direct(<?= $row->solfa_rkm ?>,<?=$row->fe2a_tagel?>,<?=$row->for_month?>,<?=$row->qemt_qst?>,<?=$row->emp_code_fk?>)"
...
<script>
    ...
    function make_transformation_direct(solf_id,fe2a,month,qemt_qst,emp_code_fk) {
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>human_resources/employee_forms/solaf/Solaf/make_tagel_transformation_direct",
            data: {
                solf_id: solf_id,
                fe2a:fe2a,
                month:month,
