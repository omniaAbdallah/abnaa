...
if (isset($result) && !empty($result)) {
    ...
    $mokdm_emp_id = $result->mokdm_emp_id;
    $moshrf_emp_id = $result->moshrf_emp_id;
    $moshrf_name = $result->moshrf_name;
    $moshrf_jwal = $result->moshrf_jwal;
    $moshrf_job = $result->moshrf_job;
    $moshrf_email = $result->moshrf_email;
    $moshrf_tele = $result->moshrf_tele;
...
<div class="col-sm-12 no-padding ">
 ...
 <div class="form-group col-md-3 col-sm-6 col-xs-6 padding-4">
    <label class="label">مقدم الطلب </label>
    <input type="text" name="mokdm_talab" value="<?= $mokdm_talab ?>"
           class="form-control" data-validation="required" readonly/>
    <input type="hidden" name="mokdm_emp_id" id="mokdm_emp_id" value="<?= $mokdm_emp_id ?>" />

</div>  
<div class="form-group col-md-3 col-sm-6 col-xs-6 padding-4">
    <label class="label">اسم المشرف </label>
    <input type="hidden" name="moshrf_name" id="moshrf_name" value="<?= $moshrf_name ?>" >
    <input type="hidden" name="moshrf_job" id="job" value="<?= $moshrf_job ?>" >
    <input type="hidden" name="moshrf_tele" id="tele" value="<?= $moshrf_tele ?>" >
    <input type="hidden" name="moshrf_email" id="email" value="<?= $moshrf_email ?>" >
    <select name="moshrf_emp_id" id="responsible_load" class="form-control  " onchange="get_ms2ol_data()" data-validation="required" aria-required="true">
        <option value="">إختر</option>
        <?php
        if (!empty($responsibles)):
            foreach ($responsibles as $record):
                $select = '';
                if ($moshrf_emp_id == $record->id) {
                    $select = 'selected';
                } ?>
                <option
