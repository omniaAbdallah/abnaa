...
<?php echo form_open_multipart('human_resources/Managers_and_authorities_settings/Update_authorities_managers');?>
...
<tr>
     <td>
         <select class="form-control   "  name="job_title_id_fk" data-validation="required" aria-required="true"  >
             ...
             <?php foreach ($jobtitles as $value){
                 $select='';
                 if($result['job_title_id_fk'] == $value->defined_id){

                     $select='selected';
                 }
                 ?>
                 <option value="<?php echo $value->defined_id; ?>"  <?php echo$select;?>><?php echo  $value->defined_title;?> </option>
             <?php }?>
         </select> </td>
    <td>
