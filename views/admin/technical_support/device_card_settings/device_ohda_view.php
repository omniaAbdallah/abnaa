        ...
        <div class="panel-body">
            <?php
              ...
              //  $device_code = $get_card->device_code;
                ...
                if($ohda_type==1){
                $from_ohda_date=$get_card->from_ohda_date;
                $to_ohda_date=$get_card->to_ohda_date;
                $hidden='';
                }else{
                    $from_ohda_date='';
                    $to_ohda_date='';
                ...
                ///////
                $notes='';
                $from_ohda_date=date('Y-m-d');
                $to_ohda_date=date('Y-m-d');
                $hidden="none";
                    }
                    ?>
...
<div id="result_type" style="display:<?=$hidden?>;">

                <div class="form-group col-md-3  padding-4">
                    <label class="label">   من تاريخ </label>
                    <input type="date" name="from_ohda_date" id="ohda_date" value="<?= $from_ohda_date ?>"
                           class="form-control "
                           data-validation="required"
                        >
                </div>
                ...
                <label class="label">   الي تاريخ </label>
                    <input type="date" name="to_ohda_date" id="ohda_date" value="<?= $to_ohda_date ?>"
                           class="form-control "
                           data-validation="required"
                        >
                </div>
  ...
  <script>
    function print_card(row_id) {
        var request = $.ajax({
            ...
            url: "<?=base_url() . 'technical_support/device_card/Device_card_c/print_ohda'?>",
            type: "POST",
            data: {row_id: row_id},
        });
