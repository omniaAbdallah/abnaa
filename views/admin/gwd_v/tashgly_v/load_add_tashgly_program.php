...
<div class="col-sm-12 no-padding ">
    ...
    if(!empty($record)){
        ...
        $program_year_money=$record->program_year_money;
        $prog_reviser_name=$record->prog_reviser_name;
       $prog_ms2ol_name=$record->prog_ms2ol_name;
       
       $prog_from_date=$record->prog_from_date;
	   $prog_to_date=$record->prog_to_date;
    }else{
    ...
    $prog_reviser_name='';
       $prog_ms2ol_name='';
       $prog_from_date=date('Y-m-d');
	   $prog_to_date=date('Y-m-d');
    }
    ?>
    ...
    height: 59px;" class="form-control" name="program_wasf" id="program_wasf">
      ...
      <div class="col-xs-8 r-input">
                            ...
                            onchange="Checkcode(<?php echo $plan_id; ?>);"
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12 inner-side r-data">
                    <div class="col-sm-4">
           <label class="">    ميزانية البرنامج السنوية </label>
     </div> 
      <div class="col-xs-8 r-input">
                            <input type="text" name="program_year_money" id="program_year_money" value="<?php echo $program_year_money; ?>"
                                   class="form-control "
                        ...
                        <!-- <div class="col-md-12 col-sm-12 col-xs-12 inner-side r-data">
                            <div class="col-sm-4">
                                    <label class="">    معتمد ميزانية البرنامج </label>
                            </div>
                            <div class="col-xs-8 r-input">
                <select id="prog_reviser_name" name="prog_reviser_name" data-validation="required"
                title="    اختر    معتمد ميزانية البرنامج  "
                class="form-control selectpicker"
                data-show-subtext="true"
                data-live-search="true">
                <option value="" >اختر    معتمد ميزانية البرنامج</option>
                <?php
                if (isset($all_Emps) && (!empty($all_Emps))) {
                foreach ($all_Emps as $key) {
                    $select = '';
                    if (isset($prog_reviser_name) && (!empty($prog_reviser_name))) {
                        if ($key->employee == $prog_reviser_name) {
                            $select = 'selected';
                        }
                    }
                    ?>
                ...
                <?php }
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12 inner-side r-data">
                            <div class="col-sm-4">
                                    <label class="">    مسؤول التنفيذ </label>
                            </div>
                            <div class="col-xs-8 r-input">
                <select id="prog_ms2ol_name" name="prog_ms2ol_name" data-validation="required"
                title="    اختر    مسؤول التنفيذ   "
                class="form-control selectpicker"
                data-show-subtext="true"
                data-live-search="true">
                ...
                foreach ($all_Emps as $key) {
                    $select = '';
                    if (isset($prog_ms2ol_name) && (!empty($prog_ms2ol_name))) {
                        if ($key->employee == $prog_ms2ol_name) {
                            $select = 'selected';
                        }
                    }
                    ?>
                ...
                <?php }
                      ...
                      <div class="col-md-12 col-sm-12 col-xs-12 inner-side r-data">
                      <label class="">فترة التنفيذ</label>
                            <div class="col-sm-2">
                                    <label class="">من</label>
                            </div>
                           
                            <div class="col-xs-4 r-input">
                            <input type="date" name="prog_from_date" id="prog_from_date" value="<?=$prog_from_date;?>" data-validation="required"
                            class="form-control ">
                            </div> 
                            <div class="col-sm-2">
                                    <label class="">الي</label>
                            </div>
                            <div class="col-xs-4 r-input">
                            <input type="date" name="prog_to_date" id="prog_to_date" value="<?=$prog_to_date;?>" data-validation="required"
                            class="form-control ">
                            </div>
                            </div> 
<!-- yara7-4-2020 -->
...
<!-- yara -->
<script>
    function Checkcode(plan_rkm) {
    //    var national_id_type = $('#national_id_type').val();
      var  prog_code= $('#program_order').val();
        $.ajax({
            type: 'post',
            url: '<?php echo base_url() ?>gwd/Tashgly/Checkcode',
            data: {prog_code:prog_code,plan_rkm:plan_rkm},
            dataType: 'html',
            cache: false,
            success: function (html) {
                if (html == 1) {
