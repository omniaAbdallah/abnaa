    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body">
                ...
                <table id="table_anf" class="table-bordered table table-responsive tb-table"
                    ...
                    if ((isset($get_rasid->details)) && (!empty($get_rasid->details)) && ($get_rasid->details != 0)) {
                        ...
                        foreach ($get_rasid->details as $sanfe) {
                            ...
                            <tr id="row_<?= $z ?>">
                                ...
                                <td><?php if (!empty($sanfe->sanf_salahia_date_ar)) {
                                        $type = 'date';
                                        $read = 'data-validation="required"';

                                    } else {
                                        $type = 'text';
                                        $read = 'readonly';
                                    } ?>
                                    <input type="<?= $type ?>" name="sanf_salahia_date[]" data-validation="required"
                                           class="form-control testButton inputclass" id="sanf_salahia_date<?= $z ?>"
                                           value="<?= $sanfe->sanf_salahia_date_ar ?>" <?= $read ?>/></td>
                                <td><input type="text" name="lot[]" class="form-control" id="lot<?= $z ?>" value="<?= $sanfe->lot ?>"
                                           /></td>
                                ...
                                <td style="padding: 8px 0;"><a id="1"
                                                               onclick=" $(this).closest('tr').remove();set_sum();"><i
                        ...
                        <tr id="row_1">
                            ...
                            <td><input type="text" name="lot[]" data-validation="required" class="form-control testButton inputclass" id="lot1"
                                       value=""/></td>
                            <td><input readonly type="text" name="rased_hali[]"
                                       class="form-control testButton inputclass"
            ...
            <div class="col-xs-12 text-center">
                ...
                <button type="submit" class="btn btn-labeled btn-success " name="<?= $submitAdd ?>" value="save"
                ...
                <button type="button" class="btn btn-labeled btn-inverse " id="attach_button" data-target="#searchModal"
                        data-toggle="modal">
                        ...
                        <tr>
                            ...
                            <td>
                                <a type="button" class="btn btn-xs btn-primary" data-toggle="modal"
                                   ...
                                   onclick="load_page(<?= $all->id ?>)" ;><i class="fa fa-list"></i></a>
                            ...
                            <tr ondblclick="get_asnaf(<?= $asnaf->code ?>,<?= $asnaf->code_br ?>,'<?= $asnaf->name ?>','<?= $asnaf->wehda_name ?>',<?= $asnaf->price ?>,<?= $asnaf->slahia ?>);">
                                <td>
                                    <input type="radio" name="r"
                                           ondblclick="get_asnaf(<?= $asnaf->code ?>,<?= $asnaf->code_br ?>,'<?= $asnaf->name ?>','<?= $asnaf->wehda_name ?>',<?= $asnaf->price ?>,<?= $asnaf->slahia ?>);">
                                </td>
                                <td><?= $asnaf->code ?></td>
                                <td><?= $asnaf->name ?></td>
...
<script>
    ...
    function Get_sanfe_Name(obj, id) {
        ...
        if (parseInt(slahia) == 1) {
            ...
            $('#sanf_salahia_date' + id).attr('data-validation', 'required');
    ...
    function add_row_sanfe() {
        ...
        var row = ' <tr id="row_1" >\n' +
            '                        <td>' + (len + 1) + '</td>\n' +
            '                        <td> <input type="text" class="form-control testButton inputclass" data-validation="required" name="sanf_code[]" id="sanf_code' + (len + 1) + '" value=""  ondblclick="$(\'#myModal\').modal(\'show\'); GetDiv_sanfe(\'myDiv_sanfe\',' + (len + 1) + ')" readonly/></td>\n' +
            ...
            '                        <td> <input type="text" class="form-control testButton inputclass" data-validation="required" name="sanf_amount[]" oninput="get_total(this,' + (len + 1) + ')" id="sanf_amount' + (len + 1) + '" value="" /></td>\n' +
            ...
            '                        <td> <input type="text" class="form-control testButton inputclass" data-validation="required" name="sanf_salahia_date[]" id="sanf_salahia_date' + (len + 1) + '" value="" /></td>\n' +
            '                        <td> <input type="text" class="form-control testButton inputclass" data-validation="required" name="lot[]" id="lot' + (len + 1) + '" value="" /></td>\n' +
