...
<tr>
    ...
    <?php 
      ...
      echo '<pre>';    
    print_r($all_emp_allowances);
    echo '<pre>';   
    if(isset($all_emp_allowances) && $all_emp_allowances != null){ 
             foreach( $all_emp_allowances as $record){?>
     <th class="text-center"><?php echo $record->defined_title ; 
     echo '--';
     echo $record->defined_id;
      ?></th>
    <?php }  } ?>
    ...
    <?php 
    echo '<pre>';    
    print_r($all_emp_deduction);
    echo '<pre>';    
    if(isset($all_emp_deduction) && $all_emp_deduction != null){ 
        foreach( $all_emp_deduction as $record){?>
        <th class="text-center"><?php echo $record->defined_title;?></th>
    <?php }  } ?>

    <!--sa-->
    <?php if(isset($penalty_names) && $penalty_names != null){
             foreach( $penalty_names as $record){?>
        <th class="text-center"><?php echo $record->title;
             echo '--';
     echo $record->id;
    ...
    <th class="text-center">صافي</th>
        ...
        <?php $val=0;  $ALL_bdl= 0; if(isset($all_emp_allowances) && $all_emp_allowances != null){ 
                   foreach( $all_emp_allowances as $record){?>
        <td><?php if( !empty($allowances[$record->defined_id]) && $allowances[$record->defined_id] !='' && $allowances[$record->defined_id] !=null){ 
                 echo $allowances[$record->defined_id]; 
                  $ALL_[$record->defined_id] += $allowances[$record->defined_id];
                   }else{ echo 0;} ?></td>
         <?php if( !empty($allowances[$record->defined_id]) && $allowances[$record->defined_id] !='' && $allowances[$record->defined_id] !=null){
            $val +=$allowances[$record->defined_id];
   ...
   <?php     }
    ...
    <td><?php echo(($income+$all_rewards) -($penalty_values +$punishment));
       $ALL_safy +=(($income+$all_rewards) -($penalty_values +$punishment));?></td>
</tr>
<?php  $a++;endforeach;?>
