...
if(isset($_SESSION["order_addtion".$session_id]) && isset($_POST["session2"]) && $_POST["session2"] == 1){  //if session var already exist
    unset($_SESSION["order_addtion".$session_id]); //unset old item     			
}
if(!(isset($_POST["session2"]))){

if((!(isset($_SESSION["order_addtion".$session_id])) || $_SESSION["order_addtion".$session_id] == null) && !(isset($_POST["remove_code"]))){
    ...
    $mode = reset($_SESSION["order_addtion".$session_id]);
...
if(isset($_POST["order_num"])){
    ...
    if(isset($_SESSION["order_addtion".$session_id])){  //if session var already exist
        if(isset($_SESSION["order_addtion".$session_id][$new_product['product_code_fk']])) //check item exist in products array
        {
            unset($_SESSION["order_addtion".$session_id][$new_product['product_code_fk']]); //unset old item
        }			
    }

    $_SESSION["order_addtion".$session_id][$new_product['product_code_fk']] = $new_product;	//update products with new item array
}
...
if(isset($_POST["remove_code"]) && isset($_SESSION["order_addtion".$session_id]))
{
	$product_code   = filter_var($_POST["remove_code"], FILTER_SANITIZE_STRING); //get the product code to remove
   
	if(isset($_SESSION["order_addtion".$session_id][$product_code]))
	{
		unset($_SESSION["order_addtion".$session_id][$product_code]);
	}
...
if(isset($_POST["load"]) && $_POST["load"]==10){
    ...
    if(isset($_SESSION["order_addtion".$session_id]) && count($_SESSION["order_addtion".$session_id])>0){ //if we have session variable
        $mode = reset($_SESSION["order_addtion".$session_id]);
                    ...
                    <div class="panel-body">
                        <div class="fade in active">
                          <table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                            ...
                            <tbody>';
        $all_value = 0;
        for($x = 0 ; $x < count($_SESSION["order_addtion".$session_id]) ; $x++){
            $table .= '<tr>
                            <td>'.($x+1).'</td>          
        ...
        $mode = next($_SESSION["order_addtion".$session_id]);
        }
    ...
    echo form_open_multipart('Exchange_orders/index/'.$this->uri->segment(3).'');
    echo $table;
    echo form_close();
    }
}
...
if(isset($_POST["remove_code"]) && isset($_SESSION["order_addtion".$session_id]))
{
    ...
    if($_SESSION["order_addtion".$session_id] == null){
        echo '<script>
                  document.getElementById("date").removeAttribute("disabled");
...
$("#results").on('click', 'span.off', function(e) {
		...
		commentContainer.fadeOut(); 
          ...
          var remove_code = 'remove_code=' + pcode + '&load=10';
            ...
            $.ajax({
                type:'post',
                url: '<?php echo base_url() ?>Exchange_orders/index/0',
                data:remove_code,
                dataType: 'html',
                cache:false,
