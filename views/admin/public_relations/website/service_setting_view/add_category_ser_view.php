    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body">
            ...
            if ((isset($f2at)) && (!empty($f2at))) {
                echo form_open_multipart(base_url() . 'public_relations/website/setting_service/Setting_service/cat_service_setting_uptate/' . base64_encode($f2at->id));
                $cat_serv = $f2at->cat_name;
                $ser = $f2at->ser_id_fk;
            } else {
                $cat_serv = '';
                $ser = 0;
                echo form_open_multipart(base_url() . 'public_relations/website/setting_service/Setting_service/cat_service_setting');
            } ?>
                ...
                <table class="table table-bordered">
                    ...
                    <?php foreach ($cat_service as $key => $value) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="#"
                                   onclick="confirm_delete('<?=base64_encode($value->id)?>');"><i class="fa fa-trash"></i></a>
                                <a href="<?= base_url() . 'public_relations/website/setting_service/Setting_service/cat_service_setting_uptate/' . base64_encode($value->id) ?>"><i
...
<script>
    function confirm_delete(id){
        ...
        if(confirm('هل متأكد من حذف هذة الفئة وما فيها من وصف لها  ؟!!')){
            window.location='<?= base_url() . 'public_relations/website/setting_service/Setting_service/cat_service_setting_delete/' ?>'+id;
        }
