...
if(isset($get_project) && $get_project!=null){
    ...
    $form = form_open_multipart("public_relations/website/projects/Project_controller/Update/".$get_project->id);
} else{
    $disp = 'none';
    $form =form_open_multipart("public_relations/website/projects/Project_controller/add_project");
}
?>
<?php echo  $form;?>
    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
        ...
        <div class="panel-body">
          ...
          <div class="col-md-2 form-group padding-4">
                    <label class=""> من تاريخ </label>
                    <input type="date" class="form-control"  name="from_date" value="<?php if(isset($get_project) && !empty($get_project->from_date)){ echo $get_project->from_date;} else{echo date('Y-m-d');} ?>">

                </div>
                <div class="col-md-2 form-group padding-4">
                    <label class=""> الي تاريخ </label>
                    <input type="date" class="form-control"  name="to_date" value="<?php if(isset($get_project) && !empty($get_project->to_date)){ echo $get_project->to_date;} else{echo date('Y-m-d');} ?>">
                ...
                <div class="col-md-4 form-group padding-4">
                    <label class="">الصورة الرئيسية للبرنامج <small>(مقاس الصورة  200بيكسل * 400 بيكسل)</small></label>
                    ...
                    <table class="table table-bordered"   id="details_table">
                        ...
                        foreach ($get_items as $d){ ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?php echo base_url().'public_relations/website/projects/Project_controller/DeleteDetails/'.$d->id."/".$d->project_id_fk?>" onclick="return confirm('هل انت متأكد من عملية الحذف ؟');">
                    ...
                    <table class="table table-bordered"   id="mostafed_table">
                        ...
                        foreach ($get_mostafed as $m){ ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?php echo base_url().'public_relations/website/projects/Project_controller/DeleteMostafed/'.$m->id."/".$m->project_id_fk?>" onclick="return confirm('هل انت متأكد من عملية الحذف ؟');">
                                        <i class="fa fa-trash" aria-hidden="true" title="حذف"></i> </a>
                                </td>
                        ...
                        foreach ($get_photos as $row){
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo base_url().'public_relations/website/projects/Project_controller/downloads_new/'.$row->image ?>" download>
                                        <i class="fa fa-download" title="تحميل"></i> </a>
                                    <a  data-toggle="modal" data-target="#myModal-view-<?=$row->id?>" >
                                        <i class="fa fa-eye" title=" قراءة"></i> </a>
                                    <a id="delete_img" href="<?php echo base_url().'public_relations/website/projects/Project_controller/DeletePhoto/'.$row->id."/".$row->project_id_fk?>" >
                                        <i class="fa fa-trash" aria-hidden="true" title="حذف"></i> </a>
                                </td>
                    ...
                    <table class="table table-bordered"   id="videotable">
                        ...
                        foreach ($get_videos as $video){
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="<?php echo base_url().'public_relations/website/projects/Project_controller/DeleteVideo/'.$video->id."/".$video->project_id_fk?>" onclick="return confirm('هل انت متأكد من عملية الحذف ؟');">
                                        <i class="fa fa-trash" aria-hidden="true" title="حذف"></i> </a>
                                </td>
                   ...
                   // $project_details = word_limiter($project->project_details,20);
                    ...
                    <tr>
                        ...
                        <td>
                           ...
                           <input type="hidden" id="status"  value="<?= $project->web_display ?>">
                            <?php
                            if ($project->web_display == 1) {
                                ?>
                                <button id="unactive<?= $project->id ?>" class="btn btn-success btn-sm"
                                        onclick="update_status(<?= $project->id ?>)">تم العرض
                                </button>

                                <?php
                            } elseif ($project->web_display == 0) {
                                ?>
                                <button id="unactive<?= $project->id ?>" class="btn btn-warning btn-sm"
                                        onclick="update_status(<?= $project->id ?>)"> عرض علي الموقع
                                </button>
                                <?php
                            }
                            ?>
                            <a href="<?=base_url()."public_relations/website/projects/Project_controller/Delete/".$project->id?>"  onclick="return confirm('هل انت متأكد من عملية الحذف ؟');">
                                <i class="fa fa-trash" aria-hidden="true" title="حذف"></i> </a>
                            <a href="<?=base_url()."public_relations/website/projects/Project_controller/Update/".$project->id?>" title="تعديل" >
...
<script>
    function update_status(id) {
        ...
        $.ajax({
            type: 'post',
            url: '<?php echo base_url() ?>public_relations/website/projects/Project_controller/update_web_display',
            ...
            success: function () {
                ...
                if (value==1){
                    ...
                    $('#unactive'+id).removeClass('btn-warning');
                    $('#unactive'+id).addClass('btn-success');
                    $('#unactive'+id).text('  تم العرض');

                }  else if(value==0){
                    $('#unactive'+id).removeClass('btn-success');
                    $('#unactive'+id).addClass('btn-warning');
                    $('#unactive'+id).text('    عرض علي الموقع');
                }
...
<script>
    function add_row(){
        ...
        var dataString   ='length=' + len;
            ...
            $.ajax({
                type:'post',
                url: '<?php echo base_url() ?>public_relations/website/projects/Project_controller/get_project_details',
                data:dataString,
                dataType: 'html',
...
<script>
    function add_details(){
       ...
       if (len <4){
        $.ajax({
            type:'post',
            url: '<?php echo base_url() ?>public_relations/website/projects/Project_controller/get_mostafed',
            data:dataString,
            dataType: 'html',
...
<script>
    function add_row_member(){
        ...
        $.ajax({
            type:'post',
            url: '<?php echo base_url() ?>public_relations/website/projects/Project_controller/get_photos',
            data:dataString,
            dataType: 'html',
...
<script>
    function add_row_video(){
        ...
        $.ajax({
            type:'post',
            url: '<?php echo base_url() ?>public_relations/website/projects/Project_controller/get_videos',
            data:dataString,
            dataType: 'html',
