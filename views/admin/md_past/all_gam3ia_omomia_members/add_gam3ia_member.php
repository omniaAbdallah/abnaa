...
<div class="form-group col-md-3 col-sm-6 padding-4">
                        ...
                        <select name="mehna_fk" id="job_name_fk" class="form-control " <?php if($hya_3elmia_fk !=1){ ?>disabled="disabled" <?php } ?>
                            ...
                            <?php foreach($job_role as $one_job_role){
                                $select ='';      if(!empty($mehna_fk)){ if($one_job_role->id_setting == $mehna_fk){
                                    $select = 'selected'; }  }?>
...
<!--
                            ...
                            if (isset($morfaqs) && !empty($morfaqs)) {
                                foreach ($morfaqs as $key=>$item1) {
                                    ...
                                    <tr>
                                        ...
                                        <input type="hidden" id="morfaq_name<?=++$key?>" name="morfaq_name[]" value="<?=$item1?>">
                                        </td>
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            if (isset($phones) && !empty($phones)) {
                                foreach ($phones as $item1) { ?>
                                    <tr>
                                        <td><input class="form-control" type="text" name="morfaq[]"
                                                   value="<?= $item1->tele ?>"></td>
                                        <td><a href="#" onclick="remove(this)"><i class="fa fa-trash"
                ...
                <div class="col-md-12">
                    ...
                    <span class="btn-label"><i class=" glyphicon glyphicon-map-marker"></i></span>     الموقع على الخريطة 
                        </button>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <li><a target="_blank"
	   href=" <?php echo base_url(); ?>md/all_gam3ia_omomia_members/Gam3ia_omomia_members/update_gam3ia_member/<?php echo $record->id; ?>">البيانات الأساسية</a></li>
    ...
    function apen(id, name_input, type, max_len) {
        ...
        if (parseInt(current_len) <= parseInt(max_len)) {
            var html = '<tr>' + '<td> <input class="form-control" type="' + type + '" name="' + name_input + '[]" ' +
                ...
                '  <input type="hidden" id="morfaq_name'+current_len+'" name="morfaq_name[]" value="">' +
                '</td>' +
