                ...
                if (isset($my_email) && !empty($my_email)) {
                    foreach ($my_email as $row) {
                        ...
                        <tr class="<?= $unread ?>" title="<?= $row->email_from_n; ?> - <?= $row->title; ?>"
                            ...
                            <td class="inbox-small-cells" onclick="make_star(<?= $row->id ?>,<?= $row->starred ?>,1)"><i
                                        id="satr_<?= $row->id ?>"
                                        class="fa fa-star <?= $start ?>"></i>
                            </td>
                            <td onclick="get_my_emails('page_content','message_details',<?= $row->id ?>,1)"
                            ...
                            <td class="view-message  text-right">
                                ...
                                <span class="label label-warning pull-right" style="color: black;"><i
                                            class="fa fa-clock-o"
                                            aria-hidden="true"></i><?= date('g:ia', $row->ttime-3600); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    ...
                    <?php /*for ($i = 1; $i <= round($pages); $i++) { */ ?>
                        <li class="<?php /*if ($i==1) echo 'active'*/ ?>" onclick="get_data_pagination(this,<? /*= $i */ ?>)"><a ><? /*= $i */ ?></a></li>
                    <?php /*}  */ ?>
    ...
    function get_select_check_box(action) {
        ...
        if (emailes_ids.length >= 1) {
            $.ajax({
                ...
                data: {emailes_ids: emailes_ids, action: action,type:1},
                dataType: 'html',
                cache: false,
