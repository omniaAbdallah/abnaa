...
if (isset($emp) && !empty($emp)) {
    ...
    $test_num_month = 0;
    /*7-7-om*/
    if (!empty($emp->start_work_date_m)) {
        $start_work_date = date('Y-m-d', strtotime($emp->start_work_date_m));
    }
    if (!empty($emp->end_contract_date_m)) {
        $end_contract_date = date('Y-m-d', strtotime($emp->end_contract_date_m));
    }
    if (!empty($emp->end_service_date_m)) {
        $end_service_date = date('Y-m-d', strtotime($emp->end_service_date_m));
    }
    if (!empty($emp->end_test_date_m)) {
        $end_test_date = date('Y-m-d', strtotime($emp->end_test_date_m));
    }
    if (!empty($emp->start_tamin_date_m)) {
        $start_tamin_date = date('Y-m-d', strtotime($emp->start_tamin_date_m));
    }
    if (!empty($emp->tamin_date_m)) {
        $tamin_date = date('Y-m-d', strtotime($emp->tamin_date_m));
    }
    if (!empty($emp->test_num_month)) {
        $test_num_month = $emp->test_num_month;
    }
    /*7-7-om*/
...
?>
<?php
$out['form'] = 'maham_mowazf/basic_data/Maham/add_job_data' ;
?>
<div class=" panel panel-default ">
    ...
    <div class="panel-body">
        <div class="col-sm-10 no-padding">
            ...
            >
                <label class="label  ">الدرجه العلميه</label>
                <input type="text" name="employee_degree_name" id="employee_degree_name"
                       value="<?php echo $employee_degree_name ?>"
                       class="form-control testButton inputclass"
            ...
            >
                ...
                <input type="text" name="moahel_3elmi_name" id="moahel_3elmi_name"
                       value="<?php echo $qualification_name ?>"
                       class="form-control testButton inputclass"
                       style="cursor:pointer; width:85%;float: right;" autocomplete="off"
            ...
            <div class="form-group col-md-3 col-sm-6 padding-4">
                ...
                <select name="department" class="form-control selectpicker" id="department3"
                        onchange="get_depart_data(this)" aria-required="true" data-live-search="true">
                    <option value="0">إختر</option>
                    <?php
                    if (isset($departs) && !empty($departs)) {
                        foreach ($departs as $row) {
                            ?>
                            <option value="<?= $row->id ?>" data-from_id="<?= $row->from_id ?>"
                                    data-from_name="<?= $row->name ?>"
                                    data-from_code="<?= $row->code ?>"
                                    data-main_type="<?= $row->main_type ?>"
                                    data-level="<?= $row->level ?>" <?php if ($row->id == $qsm_id) {
                                echo 'selected';
                            } ?>
                            ><?= $row->name ?></option>
                            <?php
                ...
                <input type="hidden" name="department_name" id="department_name" value="<?= $qsm_n ?>">
            </div>
            <div class="form-group col-md-3 col-sm-6 padding-4">
                <label class="label ">الاداره </label>
                <select  id="administration3_select" class="form-control "
                         disabled  data-validation="required" aria-required="true">
                    <option value="">إختر</option>
                    <?php
                    if (isset($departs) && !empty($departs)) {
                        foreach ($departs as $row) {
                            ?>
                            <option value="<?= $row->id ?>" data-from_id="<?= $row->id ?>"
                                    data-from_name="<?= $row->name ?>"
                                    data-from_code="<?= $row->code ?>"
                                    data-main_type="<?= $row->main_type ?>"
                                    data-level="<?= $row->level ?>" <?php if ($row->id == $edara_id) {
                                echo 'selected';
                            } ?>
                            ><?= $row->name ?></option>
                            <?php
                ...
                <input type="hidden" name="administration_name" id="administration_name" value="<?= $edara_n ?>">
                <input type="hidden" name="administration" id="administration3" value="<?= $administration ?>">
            </div>
            <div class="form-group col-md-4 col-sm-6 padding-4">
                <label class="label ">المسمي الوظيفي </label>
                <select name="degree_id" id="degree_id3" class="form-control selectpicker" data-validation="required"
                        onchange="($('#job_name').val($('option:selected',this).text()))" aria-required="true" data-live-search="true">
                    <option value="">إختر</option>
                    <?php foreach ($job_data as $one_job_role) { ?>
                ...
                <input type="hidden" name="job_name" id="job_name" value="<?= $mosma_wazefy_n ?>">
            </div>
           <!--<div class="form-group col-md-3 col-sm-6 padding-4">
                <label class="label ">الاداره </label>
                <select name="administration" id="administration3"
                    ...
                    if (!empty($admin)):
                        foreach ($admin as $record):
                            ?>
                            <option
                                    value="<?php echo $record->id; ?>-<?php echo $record->name; ?>" <?php if ($record->id == $administration) {
                                echo 'selected';
                ...
                <select name="department" class="form-control " id="department3"
                    ...
                    if (isset($departs[$administration]) && !empty($departs[$administration])) {
                        foreach ($departs[$administration] as $row) {
                            ?>
                            <option value="<?php echo $row->id; ?>-<?php echo $row->name; ?>" <?php if ($row->id == $department) {
                                echo 'selected';
                            } ?>>
                                <?php echo $row->name; ?></option>
                            <?php
            ...
            <div class="form-group col-md-4 col-sm-6 padding-4">
                ...
                <select name="degree_id" id="degree_id3" class="form-control " data-validation="required"
                    ...
                    <?php foreach ($job_role as $one_job_role) { ?>
                        <option value="<?php echo $one_job_role->defined_id; ?>-<?php echo $one_job_role->defined_title; ?>"<?php if ($one_job_role->defined_id == $degree_id) {
                            echo 'selected';
                        } ?>><?php echo $one_job_role->defined_title; ?></option>';
                    <?php } ?>
                </select>
          ...
          <!--  <div class="form-group col-md-2  col-sm-6 padding-4">
                ...
                <input name="start_work_date_m" onchange="get_date()" id="start_work_date_m" class="form-control   "
                       type="date" value="<?=$start_work_date?>"/>
            </div>-->
            <div class="form-group col-md-1  col-sm-6 padding-4">
                <label class="label text-center">مدة الاختبار
                    <span class="text-danger"> بالشهر</span>
                </label>
                <input name="test_num_month" id="test_num_month" class="form-control" oninput="get_date()" value="<?=$test_num_month?>"
                       type="number"/>
            </div>
            <div class="form-group col-md-2  col-sm-6 padding-4">
                <label class="label text-center">
                    تاريخ إنتهاء الاختبار
                </label>
                <input name="end_test_date_m" id="end_test_date_m" class="form-control   " type="date" value="<?=$end_test_date?>"/>
            </div>
            <div class="form-group col-md-3  col-sm-6 padding-4">
                <label class="label text-center">
                    <img style="width: 18px;float: right;"
           ...
           <!-- <div class="form-group col-md-2  col-sm-6 padding-4">
                ...
                <input name="end_contract_date_m" class="form-control   " type="date" value="<?=$end_contract_date?>" <?= $disabled_end_date ?>/>
            </div>
            <div class="form-group col-md-2  col-sm-6 padding-4">
                <label class="label text-center">
                    تاريخ انتهاء الخدمه
                </label>
                <input name="end_service_date_m" class="form-control   " value="<?=$end_service_date?>"
                       type="text" <?= $disabled_end_service ?>/>
            </div>-->
            <div class="form-group col-md-2 col-sm-6 padding-4">
            ...
            <div class="form-group col-md-2  col-sm-6 padding-4">
                ...
                <input  name="start_tamin_date_m" class="form-control   " value="<?=$start_tamin_date?>"
                       type="date" <?php if ($type_tamin != 1) {
                    echo 'disabled';
                } ?> />
            </div>
            <div class="form-group col-md-2 col-sm-6 padding-4">
            ...
            >
                ...
                <input type="text" name="tamin_company_name" id="tamin_company_name"
                       value="<?php echo $tamin_company_name ?>"
                       class="form-control testButton inputclass"
                       style="cursor:pointer; width:88%;float: right;" autocomplete="off"
            ...
            <div class="form-group col-md-2  col-sm-6 padding-4">
                ...
                <input name="tamin_date_m" class="form-control   "  value="<?=$tamin_date?>"
                       type="date" <?php if ($type_tamin__medicine != 1) {
                    echo 'disabled';
                } ?>/>
            </div>
                    ...
                    <div class="col-sm-12 form-group">
                        ...
                        <div class="col-sm-12 no-padding form-group">
                            <div id="geha_input" style="display: none">
                                <div class="col-sm-3  col-md-5 padding-2 ">
                                    <label class="label   ">الدرجه العلميه </label>
                                    <input type="text" name="employee_degree_fk" id="employee_degree_fk"
                    ...
                    <div class="col-sm-12 form-group">
                        ...
                        <div class="col-sm-12 no-padding form-group">
                            <div id="geha_input111" style="display: none">
                                <div class="col-sm-3  col-md-5 padding-2 ">
                                    <label class="label   ">مكتب العمل </label>
                                    <input type="text" name="work_maktb_fk" id="work_maktb_fk"
                                           data-validation="required"
                                           value=""
                                           class="form-control ">
                                    <input type="hidden" class="form-control" id="s_id" value="">
                                </div>
                                <div class="col-sm-3  col-md-2 padding-4" id="div_add_work_maktb"
                                     style="display: block;">
                                ...
                                <div class="col-sm-3  col-md-3 padding-4" id="div_update_work_maktb"
                                     style="display: none;">
                                    <button type="button"
                                            class="btn btn-labeled btn-success " name="save" value="save"
                                            id="update_work_maktb">
                                        <span class="btn-label"><i class="glyphicon glyphicon-floppy-disk"></i></span>حفظ
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="col-sm-12 form-group">
                        ...
                        <div class="col-sm-12 no-padding form-group">
                            <div id="geha_input1111" style="display: none">
                                <div class="col-sm-3  col-md-5 padding-2 ">
                                    <label class="label   ">شركه التأمين </label>
                                    <input type="text" name="tamin_company_fk" id="tamin_company_fk"
                                           data-validation="required"
                                           value=""
                                           class="form-control ">
                                    <input type="hidden" class="form-control" id="s_id" value="">
                                </div>
                                <div class="col-sm-3  col-md-2 padding-4" id="div_add_tamin_company"
                                     style="display: block;">
                                ...
                                <div class="col-sm-3  col-md-3 padding-4" id="div_update_tamin_company"
                                     style="display: none;">
                                    <button type="button"
                                            class="btn btn-labeled btn-success " name="save" value="save"
                                            id="update_tamin_company">
                                        <span class="btn-label"><i class="glyphicon glyphicon-floppy-disk"></i></span>حفظ
                                    </button>
                                </div>
                            </div>
...
<script>
    function get_date() {
        var test_num_month = parseInt($('#test_num_month').val());
        console.warn("test_num_month ::" + test_num_month);
        if (isNaN(test_num_month)) {
            test_num_month = 0;
        }
        var date = new Date(document.getElementsByName("start_work_date_m")[0].value);
        console.log("start_work_date_m ::" + document.getElementsByName("start_work_date_m")[0].value);
        day = date.getDate();
        console.warn("day :: " + day);
        month = date.getMonth() + 1 + test_num_month;
        ...
        document.getElementsByName("end_test_date_m")[0].value = new_date;
        console.log("date :: " + new_date);
        // $('#end_test_date_m').val(new_date);
    }
</script>
<script type="text/javascript">
    jQuery(function ($) {
...
<script>
    function changeAqdStatus(value) {
        if (value == 1) {
            document.getElementById('date-Hijri4').removeAttribute("disabled", "disabled");
            document.getElementById('date-Miladi3').removeAttribute("disabled", "disabled");
            document.getElementById('date-Hijri4').setAttribute("data-validation", "required");
            document.getElementById('date-Miladi3').setAttribute("data-validation", "required");
        } else if (value == 0) {
            document.getElementById('date-Hijri4').setAttribute("disabled", "disabled");
            document.getElementById('date-Miladi3').setAttribute("disabled", "disabled");
            document.getElementById('date-Hijri4').removeAttribute("data-validation", "required");
            document.getElementById('date-Miladi3').removeAttribute("data-validation", "required");
            $('#date-Miladi3').val('');
            $('#date-Hijri4').val('');
        } else {
            document.getElementById('date-Hijri4').setAttribute("disabled", "disabled");
    ...
    function change_service_end(value) {   /// 1 active
        console.log(value);
        if (value == 1) {
            document.getElementById('date-Miladi5').setAttribute("disabled", "disabled");
            document.getElementById('date-Hijri6').setAttribute("disabled", "disabled");
            document.getElementById('date-Miladi5').removeAttribute("data-validation", "required");
            document.getElementById('date-Hijri6').removeAttribute("data-validation", "required");
        } else if (value == 2) {
            document.getElementById('date-Miladi5').removeAttribute("disabled", "disabled");
            document.getElementById('date-Hijri6').removeAttribute("disabled", "disabled");
            document.getElementById('date-Miladi5').setAttribute("data-validation", "required");
            document.getElementById('date-Hijri6').setAttribute("data-validation", "required");
        }
    }
</script>
<script>
    function get_halt_tamin(valu) {
        if (valu == 2) {
            ...
            document.getElementById('date-Hijri8').setAttribute("disabled", "disabled");
            document.getElementById('date-Miladi7').setAttribute("disabled", "disabled");
            $('#work_maktb').val('');
            $('#tamin3').val('');
            $('#date-Miladi7').val('');
            $('#date-Hijri8').val('');
        ...
        if (valu == 1) {
            ...
            document.getElementById('date-Hijri8').removeAttribute("disabled", "disabled");
            document.getElementById('date-Miladi7').removeAttribute("disabled", "disabled");
        }
    }
</script>
<script>
    function get_medicine_tamin(valu2) {
        if (valu2 == 2) {
            ...
            document.getElementById('date-Hijri10').setAttribute("disabled", "disabled");
            document.getElementById('date-Miladi9').setAttribute("disabled", "disabled");
            $('#tamin_company').val('');
        ...
        if (valu2 == 1) {
            ...
            document.getElementById('date-Hijri10').removeAttribute("disabled", "disabled");
            document.getElementById('date-Miladi9').removeAttribute("disabled", "disabled");
        }
    }
</script>
<script>
...
<script>
    function lood(num) {
        if (num > 0 && num != '') {
            ...
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>human_resources/human_resources',
                data: dataString,
                dataType: 'html',
                cache: false,
...
<script>
    function get_direct_manger(depart_id) {
        if (depart_id > 0 && depart_id != '') {
            ...
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>human_resources/human_resources/get_direct_manger',
...
<script>
    function get_depart_data(depart_elem) {
        var from_id = $('option:selected', $('#department3')).data('from_id');
        console.log("from_id text::" + ($('option:selected', $('#department3')).text()));
        console.log("from_id ::" + (from_id));
        $('#administration3').val(from_id);
        $('#administration3_select').val(from_id);
        $('#administration_name').val($('option:selected', $('#administration3_select')).text());
        $('#department_name').val($('option:selected', $('#department3')).text());
        $.ajax({
            type: 'post',
            url: '<?php echo base_url() ?>human_resources/Human_resources/get_job_data/',
            data: {depart_id: depart_elem.value},
            cache: false,
            success: function (response) {
                var recodes = JSON.parse(response);
                if (recodes.length > 0) {
                    var options = ' <option value="0" > - إختر -</option>';
                    for (var i = 0; i < recodes.length; i++) {
                        options += '<option value="' + recodes[i].defined_id + '" >' + recodes[i].defined_title + '</option>';
                    }
                    $('#degree_id3').html(options);
                    $('.selectpicker').selectpicker("refresh");
                }else {
                    var options = ' <option value=" " > - لايوجد مسمى وظيفي -</option>';
                    $('#degree_id3').html(options);
                    $('.selectpicker').selectpicker("refresh");
                }
            }
