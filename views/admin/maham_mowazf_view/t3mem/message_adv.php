            ...
            <table class="table table-inbox table-hover">
                ...
                if (isset($my_t3mem) && !empty($my_t3mem)) {
                    foreach ($my_t3mem as $row) {
                        if(isset($row->t3mem_data->type)&&!empty($row->t3mem_data->type)&&$row->t3mem_data->type=='adv'){
                        if ($row->seen == 0) {
                            $unread = 'unread';
                        } else {
                            $unread = 'read';
                        ...
                        ?>
                        <tr class="<?= $unread ?>" title="<?= $row->emp_name; ?> - <?= $row->t3mem_data->ta3mem_title; ?>"
                            data-id="<?= $row->id ?>">
                            ...
                            <td onclick="check_t3mem(<?= $row->id ?>)"
                                class="view-message "><?= $row->t3mem_data->ta3mem_title; ?></td>
                            <td class="view-message  inbox-small-cells"><i class="fa fa-paperclip"></i></td>
                            <td class="view-message  text-right">
                                <span class="label label-warning pull-right" style="color: black;"><i
                                            class="fa fa-calendar"
                                            aria-hidden="true"></i><?=  $row->t3mem_data->ta3mem_date; ?></span>
...
function check_t3mem(id) {
    $.ajax({
        type: 'post',
          url:  "<?=base_url() . 'maham_mowazf/t3mem/T3mem_c/check_d3wa_adv'?>",
          data:{id:id},
