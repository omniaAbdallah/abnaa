...
<div class="col-xs-12 no-padding">
			...
			<div class="col-md-3 form-group">
                <button class="btn btn-success btn-labeled" type="button" name="add" value="add" onclick="update_comment(<?= $get_tazkra->id?>,<?= $get_tazkra->sader_id_fk ?>)"
    ...
    if(id !='')
{
        $.ajax({
            type:'post',
            url:"<?php echo base_url();?>all_secretary/archive/sader/Add_sader/update_comment",
              type: "POST",
            data:{id:id,comment:comment},
            success:function(d){
				$('#detailsModal_comm').modal('hide');
                swal({
