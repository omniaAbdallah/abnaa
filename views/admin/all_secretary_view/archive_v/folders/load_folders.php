...
continue;                            }                 
    ...
    <td> 
    <?php if ($row->type != 0) { ?>   
    <a data-toggle="modal" data-target="#myModal"   onclick="$('#ar_title_update').val('<?= $row->ar_title ?>');$('#id_update').val('<?= $row->id ?>')"  
    <i class="fa fa-pencil"> </i> </a>
    <a  onclick='swal({title: "هل انت متأكد من الحذف ؟",text: " الملف رئيسي ويوجد به ملفات داخليه اذا قمت بالحذف ستحذف جميع الملفات الداخليه",
    type: "warning",showCancelButton: true,confirmButtonClass: "btn-warning",confirmButtonText: "حذف",cancelButtonText: "إلغاء",closeOnConfirm: true},function(){window.location="<?= base_url() . 'all_secretary/archive/folders/Folder/delete_folder/' . base64_encode($row->id) ?>";});'>  
    ...
    <div class="alert-danger col-md-12 text-center "><h4> لا توجد بيانات ......</h4></div><?php } ?>    
    </div>        </div>    </div> <?php } ?>
    <script> var table = $('.example').DataTable({  
