...
<div class="tab-content" id="myTabContent">
  <div class="tab-pane fade " id="answers" role="tabpanel" aria-labelledby="answers-tab">
            <div class="panel panel-success" style="overflow-y: scroll; height: 250px;">
           
                <div class="panel-body" id="dive_replays<?= $message->email_rkm ?>">
                    <?php if (isset($replays) && !empty($replays)) { ?>
                        <?php foreach ($replays as $item) { ?>
                            <div class="panel panel-default">
                                <div class="panel-heading">
      <span class="bg-green badge avatar-text">
      <i class="fa fa-paper-plane" aria-hidden="true"></i>
      المرسل  </span>
      <span style="color: black;"><?php echo $item->emp_name; ?></span>
...
<div class="inbox-avatar">
    <img src="<?php if (isset($item->emp_photo) && !empty($item->emp_photo) ) {
        echo base_url() . 'uploads/human_resources/emp_photo/thumbs/' . $item->emp_photo;
    } else {
        echo base_url() . 'asisst/admin_asset/arch/default_img.png';
    }
    ?>"
         class="border-green hidden-xs hidden-sm" alt="">
    <div class="inbox-avatar-text">
...
if ((replay != '') ) {
  ...
  /*  function get_email_replay() {
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>all_secretary/email/Emails/get_email_replay/<?=$message->email_rkm?>",
            success: function (d) {
                $('#replay_pop').val('');
                $('#dive_replays').html(d);
            }
    ...
    var min = 1;
    setInterval(get_email_replay, (1000 * 60*min));
    */
    
    
    function get_email_replay(email_rkm) {
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>all_secretary/email/Emails/get_email_replay/" + email_rkm,
            success: function (d) {
                $('#dive_replays<?=$message->email_rkm?>').html(d);
            }
        });
    }
    ...
    setInterval(function () {
        console.warn(' majjk <?=$message->email_rkm?> ' + $('#dive_replays<?=$message->email_rkm?>').length)
