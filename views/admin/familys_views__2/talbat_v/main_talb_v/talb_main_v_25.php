...
<div class="col-sm-12 no-padding">
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
        ...
        <div class="panel-body">
            ...
            if (!empty($main_data)) {
    echo form_open_multipart('family_controllers/talbat/Talb_main/update/' . $id, array('id' => 'myform', 'autocomplete' => "off"));
    ...
    $f2a_service_n = $main_data['f2a_service_n'];
    $type_sevice_n = $main_data['type_sevice_n'];

            } else {
                echo form_open_multipart('family_controllers/talbat/Talb_main/insert', array('id' => 'myform', 'autocomplete' => "off"));
    ...
    <select type="text" class="form-control " <? //= $script ?> onchange="$('#f2a_service_n').val($('#f2a_service_id_fk option:selected').text());get_select_data()"
            name="f2a_service" id="f2a_service_id_fk" data-validation="required">
        <option value="">إختر</option>
        <?php
        if (isset($categories) && $categories != null) {
            foreach ($categories as $value) {
                ?>
                <option value="<?= $value->id ?>"><?= $value->fe2a_title ?></option>
                <?
            }
        }
        ?>
    </select>
    <input type="hidden" name="f2a_service_n" id="f2a_service_n">
</div>
<div class="col-sm-4  col-md-4 padding-4 ">
    <label class="label  "> نوع الخدمة </label>
    <select type="text" class="form-control " onchange="$('#type_sevice_n').val($('#type_service_id_fk option:selected').text());get_form_data(this.value)"
            name="type_sevice" id="type_service_id_fk"
            data-validation-depends-on="f2a_service"
            data-validation-depends-on-value="1">
        <option value="">إختر</option>
    </select>
    <input type="hidden" name="type_sevice_n" id="type_sevice_n">

</div>
                   <!-- <div class="col-sm-3  col-md-4 padding-4 ">
                        <label class="label  "> فئة الخدمة </label>
                        <select type="text" class="form-control " <? //= $script ?> onchange="get_select_data()"
                                name="f2a_service"
                            ...
                            if (isset($categories) && $categories != null) {
                                foreach ($categories as $value) {
                                    ?>
                                    <option value="<?= $value->id ?>"><?= $value->khdma_name ?></option>
                                    <?
                                }
                            }
                            ?>
                ...
                <table id="" class="example table table-bordered table-striped" role="table">
                    ...
                    foreach ($records as $value) {
                        ...
                        <tr>
                            ...
                            <td style="background: #74b9b1;"><?= $value->f2a_service_n ?></td>
                            <td style="background: #74b9b1;"><?php echo $value->type_sevice_n; ?></td>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <a class="btn btn-sm btn-warning" onclick='swal({
                                    ...
                                    <?php if ($_SESSION['role_id_fk'] == 1) { ?>
                                        <a class="btn btn-sm btn-danger" onclick=' swal({
                                    ...
                                    <?php } else { ?>
                                    <?php } ?>
                                    <a class="btn btn-sm btn-black" target="_blank" onclick="print_contrat(<?= $value->id ?>)"
                                       title="طباعه"><i class="fa fa-print"></i></a>
                                    <!-- attached -->
                                    <a class="btn btn-sm btn-blue" onclick="get_attach_data(<?= $value->id ?>);"
...
<?php if (isset($main_data) && !empty($main_data)) {
    ...
    <script>
            ...
            $('#type_sevice_n').val('<?=$type_sevice_n?>');
            $('#f2a_service_n').val('<?=$f2a_service_n?>');
            var promises = [];
            setTimeout(function () {
                var request = $.ajax({
                    type: 'post',
                    url: '<?php echo base_url() ?>family_controllers/talbat/Talb_main_request/get_select_data',
                    data: {f2a_service_id_fk: '<?=$f2a_service?>'},
                    dataType: 'html',
                    cache: false,
                    success: function (resp) {
                        var data = JSON.parse(resp).data;
                        var str = '<option value="">إختر</option>'; // variable to store the options
                        if (data.length > 0) {
                            for (var i = 0; i < data.length; ++i) {
                                str += '<option value="' + data[i].id + '"> ' + data[i].khdma_name + ' </option>'; // Storing options in variable
                            }

                        }
                        var my_list = document.getElementById("type_service_id_fk");
            ...
            setTimeout(function () {
                var request = $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        if (data.length > 0) {
                            for (var i = 0; i < data.length; ++i) {
                                str += '<option value="' + data[i].id + '"> ' + data[i].fe2a_title + ' </option>'; // Storing options in variable
                            }
                            var my_list = document.getElementById("type_service_id_fk");
                            my_list.innerHTML = str;
                        }
...
<script>
  function get_select_data() {
        var f2a_service_id_fk = $('#f2a_service_id_fk').val();
        if (f2a_service_id_fk != 0 && f2a_service_id_fk != '') {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>family_controllers/talbat/Talb_main_request/get_select_data',
                data: {f2a_service_id_fk: f2a_service_id_fk},
                dataType: 'html',
                cache: false,
                success: function (resp) {
                    var data = JSON.parse(resp).data;
