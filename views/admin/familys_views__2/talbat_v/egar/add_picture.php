                        ...
                        <div class="modal fade" id="myModal-pdf-<?= $morfaq->id ?>" tabindex="-1"
                            ...
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                             <iframe src="<?=base_url()."family_controllers/talbat/Talb_egar/read_file/".$morfaq->file?>" style="width: 100%; height:  640px;" frameborder="0">
                                               </iframe>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal">
...
<script>
    ...
    closeOnConfirm: false
        ...
        if(error == '') {
            $.ajax({
                url:"<?php echo base_url(); ?>family_controllers/talbat/Talb_egar/upload_img", //base_url() return http://localhost/tutorial/codeigniter/
                method:"POST",
                data:form_data,
                contentType:false,
                cache:false,
...
function(isConfirm) {
  if (isConfirm) {
      $.ajax({
          type: 'post',
          url: '<?php echo base_url() ?>family_controllers/talbat/Talb_egar/delete_morfq',
          data: {id: id},
          dataType: 'html',
          cache: false,
    ...
    function get_details(id) {
       // $('#pop_rkm').text(rkm);
        $.ajax({
            type: 'post',
            data: {id: id},
            url: "<?php echo base_url();?>family_controllers/talbat/Talb_egar/load",
            beforeSend: function () {
...
<script>
    function val_id(val_id)
    {
        ...
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>family_controllers/talbat/Talb_egar/get_fatora_attach",
            data: {
                val_id: val_id,
            },
            success: function (d) {
...
<script>
    function get_details_title(row_id) {
        ...
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>family_controllers/talbat/Talb_egar/load_details",
            data: {row_id: row_id},
            beforeSend: function () {
