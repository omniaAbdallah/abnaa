                ...
                <tr id="<?=$value->id?>" data-from_code="<?=$value->from_code?>">
                    ...
                    <td>
                        <a href="#" onclick='swal({
                                ...
                                function(){
                                load_edit(<?=$value->id?>,"<?=$value->title?>","<?=$value->color?>",<?=($value->num_days?$value->num_days:0)?>,<?=$value->from_code?>,<?=$num_row?>);
                        ...
                        <!--<a href="<?php echo base_url().'services_orders/setting/Egar_setting/UpdateSetting/'.$value->id."/".$value->from_code  ?>" title="تعديل">
                            <i class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>
                        <span> </span>
                        <a href="<?=base_url()."services_orders/setting/Egar_setting/DeleteSetting/".$value->id."/".$value->from_code?>" onclick="return confirm('هل انت متأكد من عملية الحذف ؟');">
    ...
    <script>
        function load_edit(row_id,title,color,num_days,from_code,num_row) {
            ...
            if(from_code == 501) {
                $('#num_days_' + from_code).val(num_days);
            }
    ...
    <script>
        function delete_setting(row_id,from_code) {
            ...
            $.ajax({
                type: 'post',
                url: '<?php echo base_url()?>services_orders/setting/Egar_setting/delete_setting',
