...
if($id == '') {
	echo form_open_multipart('Storage/Client_vouchers');
}
?>
                    ...
                    <div class="col-xs-12 r-inner-details">
			    		...
			    		<div class="col-md-3 col-sm-12 col-xs-12 inner-side r-data no-padding">
			    			...
			    			<div class="col-md-12 padd">
	                            ...
	                            <div class="col-xs-6 r-input">
	                            	<input type="text" id="client_phone" name="client_phone" class="r-inner-h4 dt" placeholder="الهاتف" readonly />
								</div>
			                </div>
			                ...
			                <div class="col-md-12 padd">
	                            ...
	                            <div class="col-xs-6 r-input">
	                            	<input type="text" id="client_address" name="client_address" class="r-inner-h4 dt" placeholder="العنوان" readonly />
								</div>
			                </div>
			    		</div>

					    <div class="form-group col-sm-12 col-xs-12">
				            <button type="submit" name="add" id="add" value="حفظ" class="btn btn-purple w-md m-b-5" onclick="if($('#client_code').val() == '') {alert('برجاء قم بإختيار إسم المورد أولا'); return false;} if($('#remain').val() < 0){alert('لا يمكن أن يكون إجمالي المدفوع أكبر من إجمالي المديونية ..!'); return false;}" ><span><i class="fa fa-floppy-o" aria-hidden="true"></i></span> حفظ </button>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
    function ajaxSearch(client_code)
    {
        var inputs = $('.dt');
        ...
        if(client_code){
            ...
            $.ajax({
                type:'post',
                url: '<?php echo base_url() ?>Storage/Client_vouchers/client_total',
                data:dataString,
                cache:false,
                success: function(ajax_data){
                    ...
                    var total = parseFloat(data.client_dayen) + parseFloat(data.remain) - parseFloat(data.paid);
                    $('#total').val(total);
                    $('#remain').val(total);
                    if(total <= 0){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>Storage/Client_vouchers/Get_client_vouchers",
                data: dataString,
                dataType: 'html',
                cache:false,
