    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body"><!-- Nav tabs -->
            <ul class="nav nav-tabs">
                <li class="active"><a href="#details" data-toggle="tab"><i class="fa fa-plus  fa-2x"
                                                                           style="display: block;text-align: center"></i>اضافه
                        مراكز التكلفه</a></li>
                <li><a href="#morfqat" data-toggle="tab"><i class="fa  fa-reply-all  fa-2x"
                                                            style="display:block;text-align: center"></i> ربط الحساب
                        بمراكز التكلفه </a></li>
            </ul><!-- Tab panels -->
            <div class="tab-content">
                <div class="tab-pane fade in active" id="details">
                    <div class="col-xs-12 no-padding">
                        <div class="col-md-3 col-sm-3 col-xs-6"><label class="label">مركز التكلفه</label>
                            <input type="hidden"  name="id" id="id">
                            <input type="text" class="form-control" name="name" id="name"/></div>
                        <div class="col-md-3 col-sm-3 col-xs-6"><label class="label"> اللون</label>
                            <input type="color" class="form-control" name="color" id="color"/>
                        </div>
                        <div class="col-md-3 col-sm-3 col-xs-6" style="    margin-top: 26px;">
                            <div id="div_add_markz" style="display: block;">
                                <button type="button" onclick="add_markz()" class="btn btn-labeled btn-success "
                    ...
                    <div id="my_markz">
                        <br><br><?php //  echo '<pre>'; print_r($reasons_settings);
                        if (!empty($marakez)) { ?>
                            ...
                            foreach ($marakez as $value) { ?>
                                <tr>
                                    ...
                                    <td><a href="#" onclick="delete_markz(<?= $value->id ?>);"> <i
                                                    class="fa fa-trash"> </i></a><a
                                                href="#" onclick="update_markz(<?= $value->id ?>);"><i
                ...
                <div class="tab-pane fade " id="morfqat">
                    <div class="col-xs-12 no-padding">
                        <div class="col-sm-3  col-md-2 padding-4  form-group"><label class="label ">رقم
                                الحساب </label>
                            <input type="text" class="form-control  " name="rkm_hesab" id="account_num"
                                   data-validation="required" aria-required="true"
                                   readonly="readonly" ondblclick="$('#myModal').modal('show');"
                                   style="width: 78%;float: right;" style="cursor:pointer;"
                                   autocomplete="off" value=""><input type="hidden" id="hesab_no3"
                                                                      value="">
                        ...
                        <div class="col-sm-4  col-md-4 padding-4  form-group"><label class="label ">إسم
                                الحساب </label><input type="text" class="form-control " name="hesab_name" id="account"
                                                      data-validation="required" aria-required="true" readonly=""
                                                      value="" style="width: 100% !important;"></div>
                        <div class="col-md-3 col-sm-3 col-xs-6"><label class="label "> مركز التكلفه</label>
                            <select id="markez"  class="form-control" data-validation="required" >
                                <option value="">إختر</option><?php foreach ($marakez as $key) {
                                    $select = ''; ?>
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-6" style="    margin-top: 26px;">
                            ...
                            <div id="div_update_hesab" style="display: none;">
                                <button type="button" class="btn btn-labeled btn-yellow " name="save" value="save"
                                        id="update_hesab"><span class="btn-label"><i
                                                class="fa fa-search-plus"></i></span> تعديل
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="my_hesab"><br><br>
                        <?php if (!empty($hesabs)) { ?>
                            <table id="js_table_customer" class="  table table-bordered table-striped"
                                   role="table">
                                <thead>
                                <tr class="greentd">
                                    <th class="text-center"> م</th>
                    ...
                    <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                        <div class="modal-dialog" role="document" style="width: 70%">
                            <div class="modal-content">
                                ...
                                <div class="modal-body"><?php //if(!empty($tree)) {?>
...
<script type="text/javascript">
    ...
    function  set_table() {
        var oTable_usergroup = $('#js_table_customer').DataTable({
            dom: 'Bfrtip',
            "ajax": '<?php echo base_url(); ?>finance_accounting/markz_tklfa/Markz_tklfaa/get_data',
...
<script> function update_markz(id) {
        var id = id;
        $('#my_markz').show();
        $('#div_add_markz').hide();
        ...
        $('#update_markz').on('click', function () {
            ...
            if (name == '' && color == '' && update_id =='')
            {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {id: update_id, name: name, color: color},
                    beforeSend: function () {
                        $('#update_markz').attr('disabled', 'disabled');
                    },
                    success: function (d) {
                        $('#name').val('');
                        $('#color').val('');
                        $('#id').val('');
                        $('#update_markz').removeAttr('disabled');
...
<script> function delete_markz(id) {
...
<script> function get_details_hesab() {
        /* $('#pop_rkm').text(rkm);*/
        $.ajax({
            type: 'post',
...
<script> function update_hesab(id) {
        var id = id;
        $('#my_hesab').show();
        $('#div_add_hesab').hide();
