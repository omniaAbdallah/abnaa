...
function buildTreeTable($tree, $count=1)
{
  ...
  if($record['type'] ==1){
       $rased_sabek_madeen = $record['total_sabeq'][0] -  $record['total_sabeq'][1];
        $rased_sabek_dayen = 0;
  }elseif ($record['type'] ==2){
        $rased_sabek_madeen = 0;
        $rased_sabek_dayen =$record['total_sabeq'][1] -  $record['total_sabeq'][0];
        }
        if (empty($record['children'])) {
            $dayen =$record['all_dayen'];
            $maden =$record['all_maden'];
           if($record['type'] ==1){
       $all_total_sabeq_maden = $rased_sabek_madeen -  $rased_sabek_dayen;
        $all_total_sabeq_dayen = 0;
  }elseif ($record['type'] ==2){
        $all_total_sabeq_maden = 0;
        $all_total_sabeq_dayen =$rased_sabek_dayen -  $rased_sabek_madeen;
        }
        ...
        //   $all_total_sabeq_dayen = $record['all_total_sabeq_dayen'];
                  if($record['type'] ==1){
       $all_total_sabeq_maden = $record['all_total_sabeq_maden'] -  $record['all_total_sabeq_dayen'];
        $all_total_sabeq_dayen = 0;
  }elseif ($record['type'] ==2){
        $all_total_sabeq_maden = 0;
        $all_total_sabeq_dayen =$record['all_total_sabeq_dayen'] -  $record['all_total_sabeq_maden'];
        }
        }
  /*      if($record['type'] ==2){
...
if (isset($records) && !empty($records)) {
    ...
    <div class="table-wrap" id="printMe">
        ...
        <div class=" col-xs-12 visible-print">
            ...
            <div class="col-xs-4 text-center">
                <label>المشرف المالي  </label><br><br>
            </div>
            <div class="col-xs-4 text-center">
                  ...
                  /*  $(win.document.body).find('.tablearea thead tr')
                    ...
                    '</tr>');
                     $(win.document.body).find('.my_table').after('<div class=" col-xs-12">'+
                                        ...
                                        '</div>'+
                                        '<div class="col-xs-4 text-center">'+
                                            '<label> مدير الشئون المالية </label><br><br><p  style="font-size:22px">فهد بن سليمان الطليحان	</p>'+
                                        '</div>'+
                                        '<div class="col-xs-4 text-center">'+
