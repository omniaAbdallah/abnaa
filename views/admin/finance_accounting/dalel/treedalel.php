...
</div>        <div class=" col-md-8 col-sm-12 col-xs-12 padding-4">    
...
onclick="add_editAccounDalel(<?= $record->id ?>,2,this)">  
...
data-parent_code="<?= $record->code ?>" data-ttype="<?= $record->ttype ?>"   
data-level="<?= $record->level ?>"   
onclick="add_editAccounDalel(0,1,this,<?= $record->code ?>);"><i      
...
<!---------------------------------------------------------------------------->
<!---------------------------------------------------------------------------->
<div class="modal fade " id="add_editAccounDalel" tabindex="-1" role="dialog"  
aria-labelledby="myModalLabel">    <div class="modal-dialog " style="width: 80%" role="document">   
...
id="save_account_dalel" 
onclick="add_editAccounDalel_ajex(0)"      
name="add" value="حفظ"> 
<span class="btn-label"><i class="glyphicon glyphicon-floppy-disk"></i>
</span>حفظ                 </button>-->        

<span class=" label-danger" id="save_start_work_span" style="display: none;float: right">  
...
var markz_tklfa_fk = $('#markz_tklfa_fk').val();
var hesab_no3 = $("input[class='hesab_no3']:checked").val();  
var markz_tklfa = $("input[class='markz_tklfa']:checked").val();   
...
valid = 0;               
text_valid += "- مركز التكلفة ";    
$('#markz_tklfa_fk_span').show();   
} else {           
$('#markz_tklfa_fk_span').hide(); 
}      
}  
}  
$.ajax({  
type: 'post',   
url: '<?php echo base_url() ?>finance_accounting/dalel/Dalel/addAccount',
...
function add_editAccounDalel(id, method, obj,code) {
$.ajax({
type: 'post',
url: '<?php echo base_url() ?>finance_accounting/dalel/Dalel/add_editAccounDalel/' + id,  
...
var name = $('#name').val();
var hesab_tabe3a = $('#hesab_tabe3a').val();    
var hesab_report = $('#hesab_report').val();
var markz_tklfa_fk = $('#markz_tklfa_fk').val(); 
var hesab_no3 = $("input[class='hesab_no3']:checked").val();   
var markz_tklfa = $("input[class='markz_tklfa']:checked").val(); 
