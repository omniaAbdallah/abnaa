											...
											foreach ($banks as $value) {
												if ($value->id == $bank_id) {
													foreach ($value->accounts as $account) {
														if ($account->bank_id_fk == $code_account_id) {
															$selected = '';
																$selected = 'selected';
														}
										?>
										<option value="<?=$account->id?>" <?=$selected?>><?=$account->rqm_hesab?></option>
										<?php
													}
													break;
												}
							...
							<div class="elbelad no-padding" id="fisrtDiv"  data-src="path/to/image" data-caching-key="userImage">
								<div class="right-chick  ">
									<div class="top-backs">
										...
										<div class="divider1">
											<div class="col-xs-3 pad-2">
												<p style="margin-right: <?=$sheek_sarf_right?>px; margin-top: <?=$sheek_sarf_top?>px;" class="line" id="data-block7"><?=$condition_text?></p>
											</div>
										</div>
            ...
            if(banks[x].id == id){  
            	...
            	for (var z = 0; z < accounts.length; z++) {
		        	$("#code_account_id").append($("<option></option>")
				                    .attr("value",accounts[z].id)
				                    .text(accounts[z].rqm_hesab)); 
		        }
		        break;
            }
        }
