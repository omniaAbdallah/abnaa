            ...
            <div class="form-group col-sm-12 col-xs-12 no-padding">
                <div class="tab-content">
                    <div class="tab-pane fade in active" id="tab1">
                        <div class="panel-body">
                            <table id="all_no_deported_quods_json" class="table tablegray table-bordered table-striped"
                                   role="table">
                                <thead>
                                <tr class="info">
                                    <th width="2%">م</th>
...
<div class="modal" id="modal-deletes" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <a id="adele" href="">
                    <button type="button" name="save" value="save" class="btn btn-danger remove" id="Delete-Record">
                        نعم
                    </button>
                </a>
            </div>
...
<div class="modal fade" id="myModal_attach" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document" style="width: 50%">
        <div class="modal-content">
            ...
            <div class="modal-body" id="AttachTable">
...
<script type="text/javascript">
    $(document).ready(function () {
        var oTable_usergroup = $('#all_no_deported_quods_json').DataTable({
            dom: 'Bfrtip',
            "ajax": '<?php echo base_url(); ?>finance_accounting/box/quods/Quods/all_not_deported_quodss',
            aoColumns: [
                {"bSortable": true},
                {"bSortable": true},
                {"bSortable": true},
...
<script>
    function getAttach(value, link) {

        if (value != '' && value > 0) {

            var dataString = 'rkm=' + value + '&type=' + link;
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>finance_accounting/box/quods/Quods/getAttach',
                data: dataString,
                dataType: 'html',
                cache: false,
...
<script>
    function tarhel(valu, num) {
        if (num == 1) {
            var title = "هل انت متأكد من مراجعة القيد؟";
            var confirm = "نعم, قم بالترحيل";
            var text2 = "عند الضغط علي نعم سيتم ترحيل القيد إلي القيود التي تمت مراجعتها";
        } else {
