...
<div class="col-sm-12 no-padding">
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
        ...
        <div class="panel-body">
            ...
            if (isset($result) && !empty($result)) {
                ...
                $button2 = '';
                $action_button = '';
                  $print_action='onclick="print_qued('.$rkm.')"';
                ...
                <table class="table tablegray table-bordered tb-table main-table " style="table-layout: fixed;  width:100%;
                    ...
                    if (!empty($result->details)) {
                        foreach ($result->details as $row_detail) {
                            ...
                            <tr id="<?= $row_detail->id ?>" class="tr" onclick="check_tr(<?= $row_detail->id ?>);">
                                <td style="width: 105px;">
                                    <input type="text" value="<?= $row_detail->maden ?>"
                                           onkeypress="return validate_number(event);" class="form-control accountmaden tr<?= $row_detail->id ?>"
                                           step="any"
                                ...
                                <td style="width: 105px;">
                                    <input type="text" value="<?= $row_detail->dayen ?>"
                                           onkeypress="return validate_number(event);" class="form-control accountdayen tr<?= $row_detail->id ?>"
                                ...
                                <td style="width: 300px;">
                                    <input type="text" value="<?= $row_detail->hesab_name ?>" class="form-control tr<?= $row_detail->id ?>"
                                           name="hesab_name[]" id="account<?= $row_detail->id ?>"
                                           data-validation="required" aria-required="true" readonly=""
                                           style="width: 100% !important;">
                                </td>
   <td style="width: 100px;" id="markz_id<?= $row_detail->id ?>">
    <?php if (isset($row_detail->mrakz_taklefa) && (!empty($row_detail->mrakz_taklefa))) {
        if (sizeof($row_detail->mrakz_taklefa) == 1) {
            $row = $row_detail->mrakz_taklefa[0];
            ?>
            <input type="text" class="form-control" step="any"
                   name="markz_name[]"
                   value="<?= $row->name ?>" readonly>
            <input type="hidden" name="markz_id[]"
                   value="<?= $row->id ?>">
            <?php
        } elseif (sizeof($row_detail->mrakz_taklefa) > 1) { ?>
            <select class="form-control" data0
                    -validation="required"
                    step="any"
                    onchange="($('#markz_name<?= $row_detail->id ?>').val($('option:selected',this).text()))"
                    name="markz_id[]">
                <option> اختر</option>
                <?php foreach ($row_detail->mrakz_taklefa as $row) { ?>
                    <option value="<?= $row->id ?>"
                        <?php if ($row_detail->markz_id == $row->id) {
                            echo 'selected';
                        } ?>
                    ><?= $row->name ?></option>
                <?php } ?>
            </select>
            <input type="hidden" id="markz_name<?= $row_detail->id ?>"
                   name="markz_name[]"
                   value="<?= $row_detail->markz_name ?>">
            <?php
        }
    } else { ?>
        <input type="text" class="form-control" step="any"
   ...
   <!-- <td style="width: 100px;" id="markz_id<?= $row_detail->id ?>">
    ...
    <input type="text" class="form-control" step="any"
    name="markz_name[]"
    value="<?= $row_detail->markz_name ?>" readonly>
    <input type="hidden" name="markz_id[]" value="<?= $row_detail->markz_id ?>">
    <?php
    } elseif (sizeof($row_detail->mrakz_taklefa) > 1) { ?>
    ...
    <input type="hidden" id="markz_name<?= $row_detail->id ?>"
    name="markz_name[]" value="<?=$row_detail->markz_name?>">
    ...
    <?php } ?>
                                ...
                                <td style="width: 100px;">
                                    <input type="text" value="<?= $row_detail->marg3 ?>"
                                           onkeypress="return validate_number(event);" class="form-control tr<?= $row_detail->id ?> " step="any"
                                           name="marg3[]">
                                </td>
                               <!-- <td><select class="form-control tr<?= $row_detail->id ?> " name="harka_id[]"
                                    >
                                        <option value="">إختر</option>
                                        <?php
                                    ...
                                    -->
                                <td>
                                    <input type="text" value="<?=  str_replace('"', '', $row_detail->byan); ?>" class="form-control tr<?= $row_detail->id ?>"
                                           name="byan[]" aria-required="true">
                                </td>
                                <td id="TD<?= $row_detail->id ?>" style="width: 58px;">
    ...
    <td style="width: 100px;" id="markz_id<?= $counter+1 ?>">
    <input type="text" class="form-control" step="any" name="markz_name[]"
    value="" readonly>
    <input type="hidden" name="markz_id[]" value="" >
   </td>
                            ...
                            <td id="TD<?= $counter + 1 ?>" style="width: 58px;">
                           <!--<a
          ...
          if ($counter == 1) {
                              //  $byan = 'تغذية بطاقات الأسر لعدد ' . ($sarf['aytam']) . ' لشهر ' . $month[$sarf['mon_melady']] . ' سنة ' . date("Y", $sarf['sarf_date']) .$sarf['about'];
       $byan = 'تغذية بطاقات الأسر لعدد ' . ($sarf['aytam']) . ' يتيم ' .' - ' .$sarf['about'];
             ...
             if($counter == 4 ){
                    ...
                    <tr>
                        ...
                        <input type="hidden" class="total" name="total_value" value="<?= $total_value ?>">
                        <td  colspan="5" style=" text-align:right;font-size:16px;font-weight:bold;color: #002542">الفرق بين الجانبين</td>
            ...
            <div class="col-xs-12 text-center">
                ...
                <button type="button" class="btn btn-labeled btn-inverse " id="attach_button" onclick="checkInputs()"
                   ...
                   <button type="button" class="btn btn-labeled btn-info" data-target="#myModal_search" data-toggle="modal">
