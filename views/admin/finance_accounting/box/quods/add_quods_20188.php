    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
        ...
        <div class="panel-body">
            ...
            if(isset($result) && !empty($result)) {
                ...
                $form_action ='finance_accounting/box/quods/Quods/add_quods/'.$result->rkm;
            ...
            <div class="col-md-12 no-padding">
                ...
                <div class="col_md_25 col-sm-6 padding-4">
                    ...
                    <div class="col-md-7 col-sm-6 no-padding">
                        ...
                        <select name="no3_qued"  class="form-control inputclass" data-validation="required"  aria-required="true">
                            <option value="">إختر</option>
                            <?php foreach ($no3_qued_arr as $key =>$value){ ?>
                                <option value="<?php echo $key.'-'.$value;?>"
                                    <?php if($no3_qued == $key){ echo'selected';} ?>
                                ><?=$value?></option>
                            <?php  } ?>
                        </select>
                    </div>
                ...
                <div class="col_md_25 col-sm-6 padding-4">
                    ...
                    <div class="col-md-7 col-sm-6 no-padding">
                        <?php $halet_qued_arr =array(1=>'مرحل',2=>'غير مرحل');?>
                        <select name="halet_qued"  class="form-control inputclass" data-validation="required"  aria-required="true">
                            <option value="">إختر</option>
                            <?php foreach ($halet_qued_arr as $key =>$value){ ?>
                                <option value="<?php echo $key.'-'.$value;?>"
                                    <?php if($halet_qued == $key){ echo'selected';} ?>><?=$value?></option>
                            <?php  } ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="col-md-12 no-padding">
                <table class="table table-striped table-bordered" style="table-layout: fixed;
                    ...
                    <tr class="info">
                        ...
                        <th style="width: 40px;" >رقم المرجع</th>
                        <th style="width: 250px;" >البيان</th>
                        <th style="width: 23px;">الإجراء</th>
                    </tr>
                    </thead>
                    ...
                    $counter =1; if(!empty($result->details)){
                            ...
                            <tr id="<?=$row_detail->id?>">
                                <td>
                                    <input type="text" value="<?=$row_detail->maden?>"
                                           onkeypress="return validate_number(event);" class="form-control accountmaden" step="any"
                                           name="maden_value[]" data-validation="required" aria-required="true" onkeyup="getTableSum();">
                                </td>
                                <td>
                                    <input type="text" value="<?=$row_detail->dayen?>"
                                           onkeypress="return validate_number(event);" class="form-control accountdayen" step="any" name="dayen_value[]" data-validation="required" aria-required="true" onkeyup="getTableSum();">
                                </td>

                                <td>
                                    <input type="text" value="<?=$row_detail->rkm_hesab?>" class="form-control testButton" name="rkm_hesab[]" id="account_num<?=$row_detail->id?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$row_detail->id?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getAccountName($(this).val(),<?=$row_detail->id?>);">
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="text" value="<?=$row_detail->marg3?>"   onkeypress="return validate_number(event);" class="form-control "  step="any"   name="marg3[]">
                                </td>
                                <td>
                                ...
                                <td id="TD<?=$row_detail->id?>">
                                    <a href="#" <?php if($counter < sizeof($result->details)){?> style="display: none;" <?}?> onclick="addNewRow(); $(this).remove(); $('#deleteRow<?=$row_detail->id?>').show();"><i class="fa fa-plus sspan"></i></a>
                                    <a class="pull-right"  id="deleteRow<?=$row_detail->id?>" href="#" onclick="$('#<?=$row_detail->id?>').remove(); getTableSum();"><i class="fa fa-trash" aria-hidden="true"></i>
                                    </a>
                                </td>
                            </tr>
                        ...
                        <tr id="<?=$counter?>">
                            <td>
                                <input type="text" onkeypress="return validate_number(event);" id="accountmaden<?=$counter?>" class="form-control accountmaden all_total inputclass"   step="any" name="maden_value[]" data-validation="required" aria-required="true" onkeyup="getTableSum();GetZeroFunction('accountdayen<?=$counter?>');">
                            </td>
                            <td>
                                <input type="text" onkeypress="return validate_number(event);" id="accountdayen<?=$counter?>" class="form-control accountdayen all_total inputclass"   step="any" name="dayen_value[]" data-validation="required" aria-required="true" onkeyup="getTableSum();GetZeroFunction('accountmaden<?=$counter?>');">
                            </td>

                            <td>
                                <input type="text" class="form-control testButton inputclass" name="rkm_hesab[]" id="account_num<?=$counter?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$counter?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getAccountName($(this).val(),<?=$counter?>);">
                            </td>
                            <td>
                                <input type="text" class="form-control inputclass" name="hesab_name[]" id="account<?=$counter?>" data-validation="required"
                            ...
                            <td>
                                <input type="text"  class="form-control " step="any"  onkeypress="return validate_number(event);" name="marg3[]">
                            </td>
                            <td>
                                <input type="text" class="form-control" name="byan[]"  aria-required="true">
                            </td>
                            <td id="TD<?=$counter?>">
                                <a class="pull-right" style="display: none;" id="deleteRow<?=$counter?>" href="#" onclick="$('#<?=$counter?>').remove(); getTableSum();"><i class="fa fa-trash" aria-hidden="true"></i>
                                </a>
                            </td>
                        </tr>
                        <tr id="<?=$counter+1?>">
                            <td>
                                <input type="text" onkeypress="return validate_number(event);" class="form-control accountmaden all_total inputclass" id="accountmaden<?=$counter+1?>"   step="any" name="maden_value[]" data-validation="required" aria-required="true" onkeyup="getTableSum();GetZeroFunction('accountdayen<?=$counter+1?>');">
                            </td>
                            <td>
                                <input type="text" onkeypress="return validate_number(event);" class="form-control accountdayen all_total inputclass"  id="accountdayen<?=$counter+1?>"  step="any" name="dayen_value[]" data-validation="required" aria-required="true" onkeyup="getTableSum();GetZeroFunction('accountmaden<?=$counter+1?>');">
                            </td>

                            <td>
                                <input type="text" class="form-control testButton inputclass" name="rkm_hesab[]" id="account_num<?=$counter+1?>" data-validation="required" aria-required="true" readonly="" onclick="$('#modalID').val(<?=$counter+1?>); $(this).removeAttr('readonly');" ondblclick="$(this).attr('readonly','readonly'); $('#myModal').modal('show');" style="cursor:pointer;" autocomplete="off" onkeypress="return isNumberKey(event);" onblur="$(this).attr('readonly','readonly')" onkeyup="getAccountName($(this).val(),<?=$counter+1?>);">
                            </td>
                            <td>
                                <input type="text" class="form-control inputclass" name="hesab_name[]" id="account<?=$counter+1?>"
                                 data-validation="required" aria-required="true" readonly=""
                                   style="width: 100% !important;"
                                  />
                            </td>
                            ...
                            <td id="TD<?=$counter+1?>">
                                <a href="#" onclick="addNewRow(); $(this).remove(); $('#deleteRow<?=$counter+1?>').show();"><i class="fa fa-plus sspan"></i></a>
                                <a class="pull-right" style="display: none;" id="deleteRow<?=$counter+1?>" href="#" onclick="$('#<?=$counter+1?>').remove(); getTableSum();"><i class="fa fa-trash" aria-hidden="true"></i>
                                </a>
                            </td>
                        </tr>
                    <?php } ?>
                    ...
                    <tfoot>
                    <tr>
                        <td id="total" style=""><?=$total_value?>
                        </td>
                        <td id="total2" style=""><?=$total_value?>
                        </td>
                        <input type="hidden" class="total"  name="total_value" value="<?=$total_value?>"  >
                    
                        <td  style=" text-align:center;color: red">الإجمالي</td>
                        <td    colspan="4" style="text-align:right;color: red"  id="total_value_arabic"></td>
        ...
        <div class="panel-body">
            <div class="form-group col-sm-12 col-xs-12">
                ...
                <ul class="nav nav-tabs">
                    <li class="active"><a href="#tab1" data-toggle="tab"> القيود الغير مرحلة</a></li>
                    <li><a href="#tab2" data-toggle="tab"> القيود المرحلة</a></li>
                </ul>
                <!-- Tab panels -->
                <div class="tab-content">
                                        ...
                                        foreach ($QuodsNotTransfered as $value) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="<?=base_url()."finance_accounting/box/quods/Quods/add_quods/".$value->id?>" title="تعديل"><i class="fa fa-pencil"></i></a>

                                                    <a  onclick="$('#adele').attr('href', '<?=base_url()."finance_accounting/box/quods/Quods/deleteQuod/".$value->rkm?>');" data-toggle="modal" data-target="#modal-delete" title="حذف"><i class="fa fa-trash"></i></a>
                                                    <a type="button" onclick="getDetails(<?=$value->id?>)" class="btn btn-sm btn-info" data-toggle="modal" data-target="#detailsModal" style="padding: 3px 5px;line-height: 1;">
                    ...
                    <div class="tab-pane fade" id="tab2">
                        <div class="panel-body">
                            ...
                            <?php  }  else { ?>
                                <div style="color: red; font-size: large;" class="col-sm-12">لم يتم  إضافة قيود مرحلة !!</div>
                        ...
                        <?php if(!empty($result->attaches)){ ?>
                            <?php $x=1;foreach ($result->attaches as $row_img){ ?>
                                <tr class="<?=$x?>">
                                    ...
                                    <td><img id="img_view<?=$x?>" src="<?php  echo base_url().'uploads/images/finance_accounting/box/quods/'.$row_img->file_attached?>"  width="150px" height="150px" /></td>
                                    <td>

                                        <a  onclick="$('#adele').attr('href', '<?php echo  base_url().'finance_accounting/box/quods/Quods/deleteQuodImg/'.$row_img->id.'/'.$row_img->qued_rkm_fk?>');" data-toggle="modal" data-target="#modal-delete" title="حذف"><i class="fa fa-trash"></i></a>
...
<script>
    ...
    function addNewRow(){
        ...
        var html = `<tr id="`+id+`">\
                        ...
                        <td id="TD`+id+`">\
                            <a href="#" onclick="addNewRow(); $(this).remove(); $('#deleteRow`+id+`').show();"><i class="fa fa-plus sspan"></i></a>\
                            <a class="pull-right" id="deleteRow`+id+`" href="#" onclick="addPlusButton(`+id+`); getTableSum();"> <i class="fa fa-trash" aria-hidden="true"></i>
                            </a>\
                        </td>\
                    </tr>`;
    ...
    function addPlusButton(id) {
        if (parseFloat($("#QuodresultTable tr:last").attr('id')) == id) {
            ...
            $('#TD'+$("#QuodresultTable tr:last").attr('id')).append(`<a href="#" onclick="addNewRow(); $(this).remove(); $('#deleteRow`+last+`').show();"><i class="fa fa-plus sspan"></i></a>`);
        }
        else {
            $('#'+id).remove();
        }
    ...
    function getTableSum() {
        ...
        if( total >0 && total2 >0 ) {
            ...
            if (total != total2) {
                ...
                // alert('خطأ فى العملية الحسابية');
                //$(".accountmaden").val('');
                // $(".accountdayen").val('');
                $('#total').attr('style','background-color:red;');
                $('#total2').attr('style','background-color:red;');
                ...
                //$('#total2').removeAttr('style');
                $('#total').attr('style','background-color:#3c990b;');
                $('#total2').attr('style','background-color:#3c990b;');
