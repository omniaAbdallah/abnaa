				...
				<div class="col-xs-12">
                   ...
                   if(!empty($result_details)) {
									foreach ($result_details as $key => $value) {
                                        ...
                                        $number = intval(preg_replace('/[^\d\.]/', '', $value['value']));

                                        $rkm_from = substr($value['from_ayban_rkm_full'], -12);
                                        $rkm_to = substr($value['to_ayban_rkm_full'], -12);
									    ?>
							...
							<tr>
								<td><?php echo $value['value'].''.convertNumber($number).' ريال فقط لا غير ' ;?></td>
								<td><?php echo $rkm_from.'-'.$value['from_general_hesab_name'];?></td>
								<td><?php echo $rkm_to.'-'.$value['to_general_hesab_name'];?></td>
