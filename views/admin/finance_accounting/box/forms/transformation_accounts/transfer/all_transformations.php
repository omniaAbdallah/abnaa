...
تم اعتماد الطلب بالرفض
                                ...
                                foreach ($coming_records as $row) {
                                    ...
                                    <?php echo $halet_eltalab; ?>
                                        ...
                                        <td>
                                            <!------------------ التفاصيل---------->
        <a type="button" class="btn btn-primary btn-xs" style="padding: 1px 6px;"
           data-toggle="modal" data-target="#transform_details" title="التفاصيل"
           onclick="transformDetails(<?php echo $row->process_rkm; ?>)"> <i
                    class="fa fa-list"> التفاصيل </i>
        </a>
        ...
        <a type="button" class="btn btn-success btn-xs" title=" الإجراء "
           ...
           data-toggle="modal" data-target="#transform">
           <i class="fa fa-check-square-o" aria-hidden="true"> الإجراء </i>
   ...
   /* function transformRequest(level, process_rkm, fromUser, toUser) {
    ...
    function transformRequest(level, process_rkm, fromUser, toUser) {
        $.ajax({
            type: 'post',
            url: '<?php echo base_url() ?>finance_accounting/box/forms/transformation_accounts/Transformation_accounts/Get_load_page',
            data: {<?php if ((isset($tab_id)) && (!empty($tab_id)) ) {?>
                from_profile: 1,
                <?php }?>
                level: level, process_rkm: process_rkm, fromUser: fromUser, toUser: toUser
            },
            dataType: 'html',
            cache: false,
            success: function (html) {
