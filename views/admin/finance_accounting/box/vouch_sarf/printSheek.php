...
<div class="col-sm-12 no-padding">
			...
			<body id="page-top" data-spy="scroll" onload="checkEdits()">
				...
				$sheek_date			 = $dataSheek['date_sanad_ar'];
				$benefit_name		 = $dataSheek['person_name'];
				$value_num 			 = $dataSheek['total_value'];
				$value_text			 = $arabicNumber.' ريال  فقط لا غير';
				$byan   			 = $dataSheek['about'];
          ...
          //  echo  $dataSheek['cancel'] ;
              if($dataSheek['cancel'] == 'yes'){
                $display = 'none';
              }else{
                  $display = '';
              }
                ...
                function chickVisibility($top, $right)
		{
    ...
    //   echo $dataSheek['total_value'];
       if($value_num ==0 ){
        ...
        $quotation = '';
                ...
                if(is_float($value_num+0) === true ) {

                    $value_num = sprintf("%.2f",$value_num);
                }
                 $val_data = explode(".", sprintf("%.2f",$value_num));
                 
                 
                 if($value_num == 4096){
                     $value_nums = 'أربعة ألاف وست وتسعون ريال '; 
                 }else{
                   
                   if(isset($val_data[0]) && !empty($val_data[0])){
                            $value_nums = convertNumber($val_data[0]).' ريال  ';
                            }?><?php if(isset($val_data[1]) && !empty($val_data[1]) && $val_data[1]!= 00){
                                $value_nums .= 'و'.convertNumber($val_data[1]).' هلله  ';
                                }               
				...
				<div class="" id="cheque" data-src="path/to/image" data-caching-key="userImage">
					<div class="elbelad no-padding" id="fisrtDiv"  data-src="path/to/image" data-caching-key="userImage">
						<div class="right-chick  ">
							<div class="top-backs">
								<div class="bond-header"style="display:<?=$display?>" >
									<div class="col-xs-6 pad-2">
										<div  class="rightbond-header">	
											<p style="right: <?=$sheek_date_right?>cm; top: <?=$sheek_date_top?>cm; <?=chickVisibility($sheek_date_right,$sheek_date_top)?>" class="line" id="data-block1"><?=$sheek_date?></p>
											<p style="right: <?=$sheek_horer_right?>cm; top: <?=$sheek_horer_top?>cm; <?=chickVisibility($sheek_horer_right,$sheek_horer_top)?>" class="line" id="data-block2"><?=$horer_in?></p>
										</div>
									</div>
									<div class="col-xs-6 pad-2">
										<div class="leftbond-header">
											<p style="right: <?=$ka3b_num_right?>cm; top: <?=$ka3b_num_top?>cm; <?=chickVisibility($ka3b_num_right,$ka3b_num_top)?>" class="line"><span class="right-lang"><strong></strong></p>
				                            	<p style="right: <?=$sheek_sarf_right?>cm; top: <?=$sheek_sarf_top?>cm; <?=chickVisibility($sheek_sarf_right,$sheek_sarf_top)?>" class="line" id="data-block7"><?=$condition_text?></p>
										</div>
									</div>
								</div>
                                
								<div class="divider1"  >
				                	<div class="col-xs-4 pad-2"></div>
									<div class="col-xs-4 pad-2">
									
									</div>
								</div>
								<div class="bond-body" id="updateDiv">
									<div class="col-xs-12 pad-2" style="display:<?=$display?>" >
										<p style="right: <?=$sheek_benifit_right?>cm; top: <?=$sheek_benifit_top?>cm; <?=chickVisibility($sheek_benifit_right,$sheek_benifit_top)?>" class="line line-pay" id="data-block3"><?=$benefit_name?></p>
									</div>
									<div class="col-xs-12 pad-2" style="display:<?=$display?>" >
	<div class="col-xs-3 pad-2">
		<p style="right: <?=$sheek_value_right?>cm; top: <?=$sheek_value_top?>cm; <?=chickVisibility($sheek_value_right,$sheek_value_top)?>" class="line" id="data-block4">
			<?=$quotation.$value_num.$quotation?> 
		</p>
	</div>
										<div class="col-xs-9 no-padding">
											<p style="right: <?=$sheek_text_right?>cm; top: <?=$sheek_text_top?>cm; <?=chickVisibility($sheek_text_right,$sheek_text_top)?>" class="line" id="data-block5">
				                            <?=$value_nums?> فقط لا غير 
				                            </p>
											<p style="right: <?=$sheek_byan_right?>cm; top: <?=$sheek_byan_top?>cm; <?=chickVisibility($sheek_byan_right,$sheek_byan_top)?>" class="line" id="data-block6"><?=$byan?></p>
										</div>
									</div>
									<div class="col-xs-12 pad-2">
										<div class="col-xs-9 no-padding">
											...
											<p style="right: <?=$sheek_sarf_right?>cm; top: <?=$sheek_sarf_top?>cm; <?=chickVisibility($sheek_sarf_right,$sheek_sarf_top)?>" class="hint-bottom"></p>
									
                                    
                                    	</div>
                                        <?php   if($dataSheek['cancel'] == 'yes'){ ?>
                                        	<div class="col-xs-3 no-padding">
                                             <img style="height: 100px; width: 100px;    margin-top: 150px; margin-right: -25px;" src="<?php echo base_url() ?>/asisst/admin_asset/img/pills/cancel.png"/>
                                   <img style="height: 225px; width:225px; margin-right: -523px; " src="<?php echo base_url() ?>/asisst/admin_asset/img/pills/x.png"/>
                                  
                                    <p style="    margin-right: -25px"> تم إلغاء الشيك <?=$dataSheek['cancel_reason']?></p>
                                            </div>
                                            <?php }else{ ?>
					...
					<div class="kaab no-padding" id="secondDiv" style="display: <?=$display?>;">
						<div class="bond-sidebar">
							<p style="right: <?=$ka3b_num_right?>cm; top: <?=$ka3b_num_top?>cm; <?=chickVisibility($ka3b_num_right,$ka3b_num_top)?>" class="line line-pay" id="data-block8"></p>
							<p style="right: <?=$ka3b_date_right?>cm; top: <?=$ka3b_date_top?>cm; <?=chickVisibility($ka3b_date_right,$ka3b_date_top)?>" class="line line-pay" id="data-block9"><?=$sheek_date?></p>
							<p style="right: <?=$ka3b_to_right?>cm;top: <?=$ka3b_date_top?>cm; <?=chickVisibility($ka3b_to_right,$ka3b_date_top)?>" class="line line-pay ft_12" id="data-block10"><?=$benefit_name?></p>
							<p style="right: <?=$ka3b_value_right?>cm; top: <?=$ka3b_value_top?>cm; <?=chickVisibility($ka3b_value_right,$ka3b_value_top)?> font-size:12px !important;" class="line line-pay " id="data-block11"><?=$value_num?></p>
