    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable">
        ...
        <div class="panel-body">
            <div class="col-sm-11 padding-4">
                ...
                if (!empty($result)){
                    $id=$result->id;
                    if ($result->sarf_method == 'from_bank'){
                        $pay_method = 3;

                    }
                    if ($result->sarf_method == 'tahwel_banki'){
                        $pay_method = 4;
                        $type_paid = 1;

                    }if ($result->sarf_method == 'sadad_hkomi'){
                        $pay_method = 4;
                        $type_paid = 2;

                    }if ($result->sarf_method == 'sadad_fwater'){
                        $pay_method = 4;
                        $type_paid = 3;

                    }

                    $person_name = $result->geha_name ;
                    $mob = $result->geha_mob;
                    $about = $result->about;
                ...
                if (!empty($result)) {
                    if (!empty($result->ezn_date_ar)) {
                        $orderdate = explode('-', $result->ezn_date_ar);
                        $month = $orderdate[1];
                        $day = $orderdate[2];
                        $year = $orderdate[0];
                ...
                $sheek_num_bank = '';
                echo form_open_multipart('finance_accounting/box/ezn_sarf/Ezn_sarf/do_sand_sarf/'.$id, array('id' => 'fromVouchSarf'));
