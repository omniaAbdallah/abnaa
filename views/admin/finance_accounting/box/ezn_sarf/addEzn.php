...
<ul class="nav nav-tabs">
                ...
                <li><a href="#tab3" data-toggle="tab">الأذونات  المرفوضة- الملغاة</a></li>
            </ul>
                            ...
                            <table id="example" class="table table-bordered" role="table">
                                ...
                                if (isset($records) && $records != null) {
                                    ...
                                    foreach ($records as $value) {
                                       ...
                                       <td style="background: #ffffff;color: #e40000;"><?=$value->value?></td>
                                            <td style="background: white;color: blue;"><?=$value->geha_name?></td>
...
<div class="btn-group btn-group-sm">
  ...
  <a title="مرفقات" href="<?= base_url() . "finance_accounting/box/ezn_sarf/Ezn_sarf/morfqat_ezn/" . $value->id ?>" class="btn btn-success"><i class="fa fa-cloud-upload"></i></a>
      
    <?php if ($value->suspend == 0) { ?>
        <a title="تعديل" href="<?= base_url() . "finance_accounting/box/ezn_sarf/Ezn_sarf/editeEzn/" . $value->id ?>"
           class="btn btn-warning"><i class="fa fa-pencil"></i></a>

    <?php } ?>

         <a target="_blank" title="طباعة"  href="<?=base_url()."finance_accounting/box/ezn_sarf/Ezn_sarf/print_namozag_ezn/".$value->id?>" class="btn btn-inverse"><i class="fa fa-print"></i></a>
        
        

    
    
   <a target="_blank" href="#"  title="إجراء وتفاصيل علي الإذن"
...
<span class="btn-label"><i class="fa fa-undo"></i></span>
    ...
    <a class="btn btn-danger btn-sm"  onclick="cancel_ezn_pop(<?= $value->id ?>)"

               data-toggle="modal" data-target="#modal_cancel_ezn"

               title="إلغاء الإذن "><i class="fa fa-trash"></i> </a>     
                                ...
                                if (isset($ozonat_suspended) && $ozonat_suspended != null) {
                                    ...
                                    foreach ($ozonat_suspended as $suspend_ezn) {
                                        ...
                                        <tr>
                                            ...
                                            <td style="background: #ffffff;color: #e40000;"><?=$suspend_ezn->value?></td>
                                            <td style="background: white;color: blue;"><?=$suspend_ezn->geha_name?></td>
...
<div class="btn-group btn-group-sm">
  ...
  <a title="مرفقات" href="<?= base_url() . "finance_accounting/box/ezn_sarf/Ezn_sarf/morfqat_ezn/" . $suspend_ezn->id ?>" class="btn btn-success"><i class="fa fa-cloud-upload"></i></a>
...
onclick="GetTransferPage(<?php echo $suspend_ezn->id;?>, <?=$suspend_ezn->level?>)"
 ...
 <a target="_blank" title="طباعة"  href="<?=base_url()."finance_accounting/box/ezn_sarf/Ezn_sarf/print_namozag_ezn/".$suspend_ezn->id?>" class="btn btn-inverse"><i class="fa fa-print"></i></a>
        
  <?php if($suspend_ezn->qed_num !=null){ ?>  
       <span style="    height: 31px;
    border-radius: unset;" title="تم التنفيذ" class="label label-success"><i class="fa fa-check-circle" aria-hidden="true"></i>
</span>
  <?php }else{ ?>
 ...
 <?php } ?>    
    ...
    background: #e691b8;">
  <?php if($suspend_ezn->rqm_sanad !=null){ 
  echo  $suspend_ezn->rqm_sanad;
   }else{
    echo  '--';
  } ?>
  </td>
                    ...
                    <div class="panel-body">
                            ...
                            <table id="example2" class="table table-bordered" role="table">
                                ...
                                <tr class="info">
                                    ...
                                    <th class="text-left">القائم بالإلغاء</th>
                                    <th class="text-left">بتاريخ</th>
                                    <th class="text-left">الساعة</th>
                                    <th class="text-left">السبب</th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php
                                if (isset($ozonat_refused) && $ozonat_refused != null) {
                                    ...
                                    foreach ($ozonat_refused as $refuz_ezn) {
                                        ?>
                                        <tr>
                                            <td style="background: #ffd1d1;"><?=$xxx++?></td>
                                            <td style="background: #ffd1d1;"><?=$refuz_ezn->ezn_rkm?></td>
                                            <td style="background: #ffd1d1;"><?=$refuz_ezn->ezn_date_ar?></td>
                                            <td style="background: #ffd1d1;"><?=$refuz_ezn->emp_name?></td>
                                            <td style="background: #ffd1d1;"><?=$refuz_ezn->edara_n?></td>
                          

                                            <td style="background: #ffd1d1;color: #e40000;"><?=$refuz_ezn->value?></td>
                                            <td style="background: #ffd1d1;color: blue;"><?=$refuz_ezn->geha_name?></td>
...
<div class="btn-group btn-group-sm">
  <a target="_blank" href="#" title="تفاصيل الإذن" 
       data-toggle="modal" data-target="#transferModal"   data-backdrop="static" data-keyboard="false"
onclick="GetTransferPage(<?php echo $refuz_ezn->id;?>, <?=$refuz_ezn->level?>)"
   class="btn btn-purple"><i class="fa fa-list"></i></a>
                                            ...
                                            <td style="background: #ffd1d1;color: blue;"><?=$refuz_ezn->person_cancel?></td>
                                            <td style="background: #ffd1d1;color: blue;"><?=$refuz_ezn->date_cancel?></td>
                                            <td style="background: #ffd1d1;color: blue;"><?=$refuz_ezn->time_cancel?></td>
                                            <td style="background: #ffd1d1;color: blue;"><?=$refuz_ezn->cancel_reason?></td>
                                        </tr>
                                        <?php
                                    }
                                }
...
<div class="modal fade" id="modal_cancel_ezn" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog " style="width: 80%" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel_header"> إلغاء الإذن </h4>
            </div>
            <div class="modal-body">
                <form type="post"action="<?= base_url() . 'finance_accounting/box/ezn_sarf/Ezn_sarf/cancel_ezn' ?>"
                      id="form_cancel_ezn">
                    <div class="container-fluid">
                        <div class="  col-xs-12 no-padding">
                            <div class="col-md-12  form-group">
                                <label class="label"> سبب إلغاء الإذن </label>
                                <input type="text" class="form-control" name="cancel_reason" id="cancel_reason"
                                       value="">
                                <input type="hidden" name="ezn_id" id="ezn_id" value="">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-labeled btn-success " id="" name="add" onclick="make_cancel()"
                        value="تنفيذ الإلغاء">
                    <span class="btn-label"><i class="glyphicon glyphicon-floppy-disk"></i></span>تنفيذ الإلغاء
                </button>
                ...
                <button type="button"
                    ...
                    <span class="btn-label"><i class="glyphicon glyphicon-remove"></i></span>التراجع
                </button>
            </div>
        </div>
    </div>
...
<script>
   function cancel_ezn_pop(ezn_id) {
    ...
    function make_cancel() {
     ...
     //   var sanad_id = $('#sanad_id').val();
      //  var rqm_sanad = $('#rqm_sanad').val();
        ...
        if (!cancel_reason) {
            valid = 0;
            text_valid += "- سبب الغاء الإذن ";
        }

        $.ajax({
            type: 'post',
            url: "<?= base_url() . 'finance_accounting/box/ezn_sarf/Ezn_sarf/cancel_ezn' ?>",
            data: {var_ezn_id: var_ezn_id, cancel_reason: cancel_reason},
            cache: false,
            ...
            success: function (html) {
                swal({
                    title: 'تم إلغاء الإذن',
                    type: 'success',
                    confirmButtonText: 'تم'
                });

                $('#modal_cancel_ezn').modal('hide');
                location.reload();
